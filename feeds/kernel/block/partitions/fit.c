...
char * __initdata cls_rootdisk;
EXPORT_SYMBOL(cls_rootdisk);
static int __init cls_rootdisk_setup(char *str)
{
	cls_rootdisk = str;
	return 1;
}
__setup("cls_rootdisk=", cls_rootdisk_setup);
...
int parse_fit_partitions(struct parsed_partitions *state, u64 fit_start_sector, u64 sectors, int *slot, int add_remain)
{
	...
	page_index = fit_start_sector >> (PAGE_SHIFT - SECTOR_SHIFT);
	...
	if (size > FIT_MAX_PAGES * PAGE_SIZE) {
		put_page(page);
		return 0;
	}
	...
	fit_c = fit;
	while (bytes_left > 0) {
		bytes_to_copy = min_t(size_t, bytes_left, PAGE_SIZE);
		memcpy(fit_c, init_fit, bytes_to_copy);
		fit_c += bytes_to_copy;
