...
void cls_nlevent_notify(struct net_device *dev, unsigned short type, unsigned short len, const char *data)
{
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
	int err = -ENOBUFS;
	...
	net = dev_net(dev);
	evt_len = NLMSG_ALIGN((sizeof(struct clsevt) + len));
	skb = nlmsg_new(evt_len, GFP_ATOMIC);
	if (skb == NULL)
		goto errout;

	nlh = nlmsg_put(skb, 0, 0, RTM_CLSEVENT, evt_len, 0);
	if (!nlh) {
		kfree_skb(skb);
		goto errout;
	}
