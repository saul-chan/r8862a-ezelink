...
NETDEVICE_SHOW_RO(if_port, fmt_dec);
NETDEVICE_SHOW_RO(type, fmt_dec);
NETDEVICE_SHOW_RO(link_mode, fmt_dec);
NETDEVICE_SHOW_RO(hotplug_dev_flow_offload, fmt_dec);
...
static int modify_mcast_to_bcast(struct net_device *dev, unsigned long val)
{
	WRITE_ONCE(dev->mcast_to_bcast, !!val);
	return 0;
}

static ssize_t mcast_to_bcast_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t len)
{
	return netdev_store(dev, attr, buf, len, modify_mcast_to_bcast);
}
NETDEVICE_SHOW_RW(mcast_to_bcast, fmt_dec);

static int modify_af_pkt_sup_vlan(struct net_device *dev, unsigned long val)
{
	WRITE_ONCE(dev->af_pkt_sup_vlan, !!val);
	return 0;
}

static ssize_t af_pkt_sup_vlan_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t len)
{
	return netdev_store(dev, attr, buf, len, modify_af_pkt_sup_vlan);
}
NETDEVICE_SHOW_RW(af_pkt_sup_vlan, fmt_dec);
...
static struct attribute *net_class_attrs[] __ro_after_init = {
	...
	&dev_attr_mcast_to_bcast.attr,
	&dev_attr_af_pkt_sup_vlan.attr,
	&dev_attr_hotplug_dev_flow_offload.attr,
	NULL,
};
