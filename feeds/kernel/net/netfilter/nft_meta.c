...
void nft_register_meta_cls_plugin(int (*get_init)(const struct nft_ctx *ctx, unsigned int *len, int key),
				  int (*set_init)(const struct nft_ctx *ctx, unsigned int *len, int key),
				  int (*get_eval)(u32 *dest, int key, const struct nft_pktinfo *pkt),
				  int (*set_eval)(u32 *dest, int key, const struct nft_pktinfo *pkt),
				  void (*get_destroy)(const struct nft_ctx *ctx, int key),
				  void (*set_destroy)(const struct nft_ctx *ctx, int key))
{
	nft_meta_cls_plugin_get_init = get_init;
	nft_meta_cls_plugin_set_init = set_init;
	nft_meta_cls_plugin_get_eval = get_eval;
	nft_meta_cls_plugin_set_eval = set_eval;
	nft_meta_cls_plugin_get_destroy = get_destroy;
	nft_meta_cls_plugin_set_destroy = set_destroy;
}
EXPORT_SYMBOL(nft_register_meta_cls_plugin);
		       ...
		       const struct nft_pktinfo *pkt)
{
	...
	case NFT_META_SDIFNAME:
		...
		break;
	default:
		if (nft_meta_cls_plugin_get_eval) {
			int ret_val;

			ret_val = nft_meta_cls_plugin_get_eval(dest, priv->key, pkt);
			if (ret_val > 0)
				break;
			else if (ret_val == 0)
				goto err;
...
#endif
	default:
		if (nft_meta_cls_plugin_set_eval &&
		    nft_meta_cls_plugin_set_eval(sreg, meta->key, pkt) >= 0)
			break;
		WARN_ON(1);
	}
}
		      ...
		      const struct nlattr * const tb[])
{
	...
	case NFT_META_TIME_HOUR:
		...
		break;
	default:
		if (nft_meta_cls_plugin_get_init &&
		    nft_meta_cls_plugin_get_init(ctx, &len, priv->key) >= 0)
			break;
		return -EOPNOTSUPP;
	}
		      ...
		      const struct nlattr * const tb[])
{
	...
	case NFT_META_PKTTYPE:
		...
		break;
	default:
		if (nft_meta_cls_plugin_set_init &&
		    nft_meta_cls_plugin_set_init(ctx, &len, priv->key) >= 0)
			break;
		return -EOPNOTSUPP;
	}
...
void nft_meta_get_destroy(const struct nft_ctx *ctx,
			  const struct nft_expr *expr)
{
	const struct nft_meta *priv = nft_expr_priv(expr);

	if (nft_meta_cls_plugin_get_destroy)
		nft_meta_cls_plugin_get_destroy(ctx, priv->key);
}
EXPORT_SYMBOL_GPL(nft_meta_get_destroy);
			  ...
			  const struct nft_expr *expr)
{
	...
	if (nft_meta_cls_plugin_set_destroy)
		nft_meta_cls_plugin_set_destroy(ctx, priv->key);
}
