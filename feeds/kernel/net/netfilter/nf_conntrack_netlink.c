...
static int ctnetlink_dump_clsmark(struct sk_buff *skb, const struct nf_conn *ct)
{
	if (nla_put_be64(skb, CTA_CLSMARK, cpu_to_be64(ct->clsmark), CTA_PAD))
		return -1;
	return 0;
}
...
static int ctnetlink_dump_info(struct sk_buff *skb, struct nf_conn *ct)
{
	if (ctnetlink_dump_status(skb, ct) < 0 ||
	    ctnetlink_dump_clsmark(skb, ct) < 0 ||
...
#endif
	       + nla_total_size(sizeof(u_int64_t)) /* CTA_CLSMARK */
#ifdef CONFIG_NF_CONNTRACK_MARK
...
ctnetlink_conntrack_event(unsigned int events, struct nf_ct_event *item)
{
	...
	if ((events & (1 << IPCT_CLSMARK) || ct->clsmark)
	    && ctnetlink_dump_clsmark(skb, ct) < 0)
		goto nla_put_failure;
...
ctnetlink_alloc_filter(const struct nlattr * const cda[], u8 family)
{
	...
	if (cda[CTA_CLSMARK]) {
		filter->clsmark.val = be64_to_cpu(nla_get_be64(cda[CTA_CLSMARK]));
		if (cda[CTA_CLSMARK_MASK])
			filter->clsmark.mask = be64_to_cpu(nla_get_be64(cda[CTA_CLSMARK_MASK]));
		else
			filter->clsmark.mask = ~0ULL;
	} else if (cda[CTA_CLSMARK_MASK]) {
		err = -EINVAL;
		goto err_filter;
	}
...
{
	return family || cda[CTA_CLSMARK] || cda[CTA_MARK] || cda[CTA_FILTER] || cda[CTA_STATUS];
}
...
static int ctnetlink_filter_match(struct nf_conn *ct, void *data)
{
	...
	if ((ct->clsmark & filter->clsmark.mask) != filter->clsmark.val)
		goto ignore_entry;
...
static const struct nla_policy ct_nla_policy[CTA_MAX+1] = {
	...
	[CTA_CLSMARK]		= { .type = NLA_U64 },
	[CTA_CLSMARK_MASK]	= { .type = NLA_U64 },
...
static void ctnetlink_change_clsmark(struct nf_conn *ct,
				    const struct nlattr * const cda[])
{
	u64 mark, newmark, mask = 0;

	if (cda[CTA_CLSMARK_MASK])
		mask = ~be64_to_cpu(nla_get_be64(cda[CTA_CLSMARK_MASK]));

	mark = be64_to_cpu(nla_get_be64(cda[CTA_CLSMARK]));
	newmark = (ct->clsmark & mask) ^ mark;
	if (newmark != ct->clsmark)
		ct->clsmark = newmark;
}
			   ...
			   const struct nlattr * const cda[])
{
	...
	if (cda[CTA_CLSMARK])
		ctnetlink_change_clsmark(ct, cda);
...
static int __ctnetlink_glue_build(struct sk_buff *skb, struct nf_conn *ct)
{
	...
	if (ct->clsmark && ctnetlink_dump_clsmark(skb, ct) < 0)
		goto nla_put_failure;
