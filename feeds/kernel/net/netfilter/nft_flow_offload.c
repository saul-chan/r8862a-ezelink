				   ...
				   struct nft_flowtable *ft)
{
	...
	if (dev->hotplug_dev_flow_offload)
		return true;
...
static bool check_offload_thresh(struct nf_conn *ct)
{
	...
	packets = (unsigned long long)atomic64_read(&acct->counter[IP_CT_DIR_ORIGINAL].packets) +
		  (unsigned long long)atomic64_read(&acct->counter[IP_CT_DIR_REPLY].packets);

	if (packets > nf_conntrack_offload_thresh)
		return true;
				  ...
				  const struct nft_pktinfo *pkt)
{
	...
	if (ct->cls_no_accel_for_ctl || ct->cls_no_accel_for_qos || ct->cls_no_accel_for_uft)
		goto out;
