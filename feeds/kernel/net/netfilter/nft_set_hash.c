...
EXPORT_SYMBOL(nft_register_cls_match);

static bool cls_set_hash_lookup(struct nft_hash *priv, const u32 *key, u8 klen,
				u8 genmask, const struct nft_set_ext **ext)
{
	...
	for (i = 0; i < priv->buckets; i++) {
		hlist_for_each_entry_rcu(he, &priv->table[i], node) {
			if (nft_cls_match(key, nft_set_ext_key(&he->ext), klen) &&
			    ...
			    const u32 *key, const struct nft_set_ext **ext)
{
	...
	if (nft_cls_match && nft_set_is_cls_set(set->klen))
		return cls_set_hash_lookup(priv, key, set->klen, genmask, ext);
