...
void register_flow_offload_cls_meta(void (*fn)(struct nf_conn *ct, int dir, struct flow_action_entry *entry))
{
	__flow_offload_cls_meta = fn;
}
EXPORT_SYMBOL_GPL(register_flow_offload_cls_meta);
			  ...
			  struct nf_flow_rule *flow_rule)
{
	...
	if (__flow_offload_cls_meta)
		__flow_offload_cls_meta(flow->ct, dir, flow_action_entry_next(flow_rule));
...
static struct flow_offload *flow_offload_find_tbd_flow(struct nf_flowtable *flowtable)
{
	...
	cls_flow.command = FLOW_CLS_FIND_TDB_FLOW;
	...
	return *((struct flow_offload **)extack.cookie);
}
				 ...
				 struct list_head *block_cb_list)
{
	...
	cls_flow.cookie = (unsigned long)flow | dir;
				 ...
				 struct nf_flow_rule *flow_rule[])
{
	...
	ret = flow_offload_tuple_add(offload, flow_rule[0], FLOW_OFFLOAD_DIR_ORIGINAL);
	if (ret)
		set_bit(NF_FLOW_HW_ORIGINAL, &offload->flow->flags);
	else
		clear_bit(NF_FLOW_HW_ORIGINAL, &offload->flow->flags);
	ok_count += ret;

	ret = flow_offload_tuple_add(offload, flow_rule[1], FLOW_OFFLOAD_DIR_REPLY);
	if (ret)
		set_bit(NF_FLOW_HW_REPLY, &offload->flow->flags);
	else
		clear_bit(NF_FLOW_HW_REPLY, &offload->flow->flags);
	ok_count += ret;
...
static void __nf_flow_offload_recall(struct nf_flowtable *flowtable, struct flow_offload *flow);

static bool flow_offload_work_pre_add(struct flow_offload_work *offload)
{
	struct nf_flowtable *flowtable = offload->flowtable;
	struct flow_offload *flow = offload->flow;
	struct flow_offload *tbd_flow;

	if (hw_flow_table_is_full(flowtable)) {
		if (!flow->ct->cls_is_vip)
			return false;
		tbd_flow = flow_offload_find_tbd_flow(flowtable);
		if (!tbd_flow)
			return false;
		__nf_flow_offload_recall(flowtable, tbd_flow);
	}
...
static void flow_offload_work_add(struct flow_offload_work *offload)
{
	...
	if (READ_ONCE(flowtable->dying))
		return;

	if (test_bit(IPS_HW_OFFLOAD_BIT, &flow->ct->status))
		return;

	if (!flow_offload_work_pre_add(offload))
		return;
	...
	set_bit(IPS_HW_OFFLOAD_BIT, &flow->ct->status);
	WRITE_ONCE(flow->hw_lastused, nf_flowtable_time_stamp);

	atomic_inc(&flowtable->hw_offload_cnt);
...
static void flow_offload_work_del(struct flow_offload_work *offload)
{
	...
	if (test_bit(IPS_HW_OFFLOAD_BIT, &flow->ct->status)) {
		atomic_dec(&flowtable->hw_offload_cnt);
		clear_bit(IPS_HW_OFFLOAD_BIT, &flow->ct->status);
	}

	if (offload->is_recall)
		clear_bit(NF_FLOW_HW_DYING, &flow->flags);
	else
...
static void __nf_flow_offload_recall(struct nf_flowtable *flowtable, struct flow_offload *flow)
{
	...
	atomic_inc(&flowtable->tbd_recall_cnt);
...
static void flow_offload_work_stats(struct flow_offload_work *offload)
{
	...
	WRITE_ONCE(offload->flow->timeout,
		   max_t(u64, READ_ONCE(offload->flow->timeout), lastused + flow_offload_get_timeout(offload->flow)));
...
{
	queue_work_on(0, nf_flow_offload_wq, &offload->work);
}
...
void nf_flow_offload_recall(struct nf_flowtable *flowtable,
			    struct flow_offload *flow)
{
	struct flow_offload_work *offload;
	...
	offload->is_recall = 1;
	atomic_inc(&flowtable->gc_recall_cnt);
	set_bit(NF_FLOW_HW_DYING, &flow->flags);
	flow_offload_queue_work(offload);
}
...
static int nf_flow_table_parse_extack(struct nf_flowtable *flowtable, struct netlink_ext_ack *extack)
{
	struct hw_flowtable_cap *hw_cap = (struct hw_flowtable_cap *)extack->cookie;

	if (extack->cookie_len != sizeof(struct hw_flowtable_cap))
		goto parse_failure;

	if (hw_cap->magic != HW_FT_CAP_MAGIC)
		goto parse_failure;

	ft_hw_offload_cap = hw_cap->hw_ft_size;
	if (!ft_hw_offload_max)
		ft_hw_offload_max = ft_hw_offload_cap;

	return 0;

parse_failure:
	ft_hw_offload_cap = UINT_MAX;
	ft_hw_offload_max = UINT_MAX;
				...
				enum flow_block_command cmd)
{
	...
	if (cmd == FLOW_BLOCK_BIND)
		nf_flow_table_parse_extack(flowtable, &extack);
...
{
	nf_flow_offload_wq = create_singlethread_workqueue("nf_ft_offload");
