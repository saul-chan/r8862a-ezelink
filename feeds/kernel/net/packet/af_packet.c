		      ...
		      struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (dev->af_pkt_sup_vlan &&
	    skb->pkt_type == PACKET_OTHERHOST &&
	    skb_vlan_tag_get_id(skb) != 0 &&
	    skb_vlan_tag_get_id(skb) == br_port_get_tagged_pvid(dev) &&
	    ether_addr_equal(dev->dev_addr, eth_hdr(skb)->h_dest))
...
static void packet_push_vlan(struct sk_buff *skb, struct net_device *dev)
{
	u16 pvid;
	...
	pvid = br_port_get_tagged_pvid(dev);
	if (!pvid)
		return;
