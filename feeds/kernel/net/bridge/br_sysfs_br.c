...
static ssize_t terminal_1905_control_show(struct device *d,
				   struct device_attribute *attr,
				   char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%#x\n", br->terminal_1905_control);
}

static int set_terminal_1905_control(struct net_bridge *br, unsigned long val)
{

	br->terminal_1905_control = val;

	return 0;
}

static ssize_t terminal_1905_control_store(struct device *d,
				    struct device_attribute *attr,
				    const char *buf,
				    size_t len)
{
	return store_bridge_parm(d, buf, len, set_terminal_1905_control);
}
static DEVICE_ATTR_RW(terminal_1905_control);
...
static int br_set_loop_detect_state(struct net_bridge *br, unsigned long val)
{
	switch (val) {
	case 0:
	case 1:
	...
	spin_lock_bh(&br->lock);
	br->loop_detect_enabled = val;
	spin_unlock_bh(&br->lock);

	return 0;
}

static ssize_t loop_detect_state_show(struct device *d,
					struct device_attribute *attr,
					char *buf)
{
	struct net_bridge *br = to_bridge(d);

	return sprintf(buf, "%d\n", br->loop_detect_enabled);
}

static ssize_t loop_detect_state_store(struct device *d,
					 struct device_attribute *attr,
					 const char *buf, size_t len)
{
	return store_bridge_parm(d, buf, len, br_set_loop_detect_state);
}
static DEVICE_ATTR_RW(loop_detect_state);
...
static struct attribute *bridge_attrs[] = {
	...
	&dev_attr_terminal_1905_control.attr,
...
#endif
	&dev_attr_loop_detect_state.attr,
	NULL
};
