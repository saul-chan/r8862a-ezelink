...
int br_handle_frame_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	if (p->flags & BR_LEARNING)
		br_fdb_update(br, p, eth_hdr(skb)->h_source, vid, skb->src_port, 0, 0);

	dest = eth_hdr(skb)->h_dest;
	if (skb->dev->mcast_to_bcast && is_multicast_ether_addr(dest) && !is_broadcast_ether_addr(dest))
		ether_addr_copy(dest, skb->dev->broadcast);
...
static void __br_handle_local_finish(struct sk_buff *skb)
{
	...
	if ((p->flags & BR_LEARNING) &&
		...
		br_fdb_update(p->br, p, eth_hdr(skb)->h_source, vid,  skb->src_port, 0, 0);
}
...
static const u8 eth_1905_control_addr[ETH_ALEN] __aligned(2) =
{ 0x01, 0x80, 0xc2, 0x00, 0x00, 0x13 };

inline int br_1905_process(struct net_bridge_port *p, struct sk_buff *skb, const unsigned char *dest)
{
	if (likely(skb->protocol != htons(0x893a)))
		goto out;

	if (unlikely(ether_addr_equal(dest, p->br->dev->dev_addr)))
		return 1;
	if ((p->br->terminal_1905_control) && (unlikely(ether_addr_equal(dest, eth_1905_control_addr))))
		return 1;
out:
	return 0;
}
...
static rx_handler_result_t br_handle_frame(struct sk_buff **pskb)
{
	...
	/* EAPOL or WAPI frame to local interface does not need to be handled by bridge */
	...
	if (unlikely(br_1905_process(p, skb, dest)))
		return RX_HANDLER_PASS;
