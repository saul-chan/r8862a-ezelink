...
int br_port_get_tagged_pvid(struct net_device *dev)
{
	struct net_bridge_vlan_group *vg;
	struct net_bridge_vlan *v;
	struct net_bridge_port *p;
	...
	/* 1 is reserved for a management VLAN in linux bridge, treat as 0 if bridge port's pvid is 1 */
	if (pvid == 1)
		return 0;

	v = br_vlan_lookup(&vg->vlan_hash, pvid);
	if (!v || (v->flags & BRIDGE_VLAN_INFO_UNTAGGED))
		return 0;

	return pvid;
}
EXPORT_SYMBOL_GPL(br_port_get_tagged_pvid);

int br_port_get_pvid(struct net_device *dev)
{
	struct net_bridge_vlan_group *vg;
	struct net_bridge_port *p;
	u16 pvid;
	...
	/* 1 is reserved for a management VLAN in linux bridge, treat as 0 if bridge port's pvid is 1 */
	return pvid == 1 ? 0 : pvid;
}
EXPORT_SYMBOL_GPL(br_port_get_pvid);

static u16 br_vlan_list[VLAN_N_VID];
static int br_vlan_num = -1;

static void __br_get_vlan_list(void)
{
	struct net_device *dev;
	struct net_bridge_port *p;
	struct net_bridge_vlan *vlan;
	...
	memset(br_vlan_list, 0, sizeof(br_vlan_list));
	br_vlan_num = 0;
	...
	for_each_netdev_rcu(&init_net, dev) {
		...
		vg = nbp_vlan_group_rcu(p);
		list_for_each_entry(vlan, &vg->vlan_list, vlist) {
			/* 1 is reserved for a management VLAN in linux bridge, ignore it */
			if (vlan->vid == 1)
				continue;
			found = 0;
			for (i = 0; i < br_vlan_num; ++i) {
				if (br_vlan_list[i] == vlan->vid) {
					found = 1;
					break;
				}
			}
			if (found)
				continue;
			br_vlan_list[br_vlan_num++] = vlan->vid;
		}
	}
	rcu_read_unlock();
}

static void br_invalidate_vlan_list(void)
{
	br_vlan_num = -1;
}

const u16 *br_get_vlan_list(int *num)
{
	if (br_vlan_num < 0)
		__br_get_vlan_list();
	*num = br_vlan_num;
...
EXPORT_SYMBOL_GPL(br_get_vlan_list);
