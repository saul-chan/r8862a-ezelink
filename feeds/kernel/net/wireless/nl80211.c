...
{
	if (!use_4addr && netdev &&
		!netdev->ieee80211_ptr->use_3addr_br) {
...
static int nl80211_new_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (is_wiphy_repeater(&rdev->wiphy))
		rdev->wiphy.regulatory_flags |= REGULATORY_COUNTRY_IE_IGNORE;
...
static int nl80211_del_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	ret = rdev_del_virtual_intf(rdev, wdev);
	if (ret) {
		if (!is_wiphy_repeater(&rdev->wiphy))
			rdev->wiphy.regulatory_flags &= ~REGULATORY_COUNTRY_IE_IGNORE;
	}
