...
static bool cfg80211_cal_mode = false;
module_param(cfg80211_cal_mode, bool, 0644);
MODULE_PARM_DESC(cfg80211_cal_mode,
		"CLS calibration mode support");

bool is_wiphy_repeater(struct wiphy *wiphy)
{
	struct wireless_dev *wdev;
	int ap_cnt = 0, sta_cnt = 0;

	if (!wiphy) {
		pr_err("Invalid wiphy\n");
		return false;
	}
