...
#define MLME_SUPPORT_MULTI_USER
...
#ifdef MLME_SUPPORT_MULTI_USER
		if (reg->nlportid != snd_portid)
			continue;
#endif
...
#ifndef MLME_SUPPORT_MULTI_USER
		break;
#endif
	}
	...
	for (bandid = 0; bandid < NUM_NL80211_BANDS; bandid++) {
		...
		for (i = 0; i < sband->n_channels; i++) {
			...
			if (c->dfs_state == NL80211_DFS_UNAVAILABLE) {
				...
				if (is_wiphy_repeater(wiphy) || regulatory_pre_cac_allowed(wiphy)
