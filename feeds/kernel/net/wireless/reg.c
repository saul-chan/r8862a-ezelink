...
static bool regdb_loading = false;
				       ...
				       const struct ieee80211_reg_rule *reg_rule)
{
	...
	//FIXME: Need a new smart function to handle dfs state in hostapd/wpa/nl80211
	/* Workaround: Once cls_repeater dfs state is available, it cannot be changed */
	if (!(is_wiphy_repeater(wiphy)
