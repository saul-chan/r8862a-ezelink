...
static irqreturn_t sfp_irq(int irq, void *data)
{
	...
	for (i = 0; i < GPIO_MAX; i++) {
		...
		if (sfp->gpio_irq[i] == irq) {
			v = gpiod_get_value(sfp->gpio[i]);
			irq_set_irq_type(irq, v ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING);
			break;
		}
	}
...
static void sfp_dump_a0h(struct sfp *sfp)
{
	u8 data[256] = {0};
	char buf[128] = {0};
	struct sfp_eeprom_base *base = (struct sfp_eeprom_base *)data;
	int i, ret;

	ret = sfp_read(sfp, false, 0, &base, sizeof(base));
	if (ret < 0) {
		pr_err("Failed to read A0h page: %d\n", ret);
		return;
	}

	pr_err("SFP EEPROM A0 Dump:\n");
	pr_err("Basic Info:\n");
	pr_err("  Identifier: 0x%02x\n", base->phys_id);
	pr_err("  Ext identifier: 0x%02x\n", base->phys_ext_id);
	pr_err("  Connector: 0x%02x\n", base->connector);
	pr_err("  Extended_cc: 0x%02x\n", base->extended_cc);

	pr_err("\nInterface Features:\n");
	pr_err("  Copper: passive=%d, active=%d\n",
			base->if_1x_copper_passive, base->if_1x_copper_active);
	pr_err("  1X: LX=%d, SX=%d\n", base->if_1x_lx, base->if_1x_sx);

	pr_err("\n10G Features:\n");
	pr_err("  10G-BASE-SR: %d\n", base->e10g_base_sr);
	pr_err("  10G-BASE-LR: %d\n", base->e10g_base_lr);
	pr_err("  10G-BASE-LRM: %d\n", base->e10g_base_lrm);
	pr_err("  10G-BASE-ER: %d\n", base->e10g_base_er);

	pr_err("\n1G Compliance:\n");
	pr_err("  1000BASE-SX: %d\n", base->e1000_base_sx);
	pr_err("  1000BASE-LX: %d\n", base->e1000_base_lx);
	pr_err("  1000BASE-CX: %d\n", base->e1000_base_cx);
	pr_err("  1000BASE-T: %d\n", base->e1000_base_t);

	pr_err("\nFibre Channel:\n");
	pr_err("Speed Support:\n");
	pr_err("  100 MBps: %d\n", base->fc_speed_100);
	pr_err("  200 MBps: %d\n", base->fc_speed_200);
	pr_err("  400 MBps: %d\n", base->fc_speed_400);
	pr_err("  800 MBps: %d\n", base->fc_speed_800);
	pr_err("  1600 MBps: %d\n", base->fc_speed_1600);
	pr_err("  3200 MBps: %d\n", base->fc_speed_3200);
	pr_err("  Tech: LC=%d, SA=%d, LL=%d, SL=%d, SN=%d\n",
			base->fc_tech_lc, base->fc_tech_sa,
			base->fc_tech_ll, base->fc_tech_sl, base->fc_tech_sn);

	pr_err("Link Length:\n");
	pr_err("  Very long: %d\n", base->fc_ll_v);
	pr_err("  Short: %d\n", base->fc_ll_s);
	pr_err("  Intermediate: %d\n", base->fc_ll_i);
	pr_err("  Long: %d\n", base->fc_ll_l);
	pr_err("  Medium: %d\n", base->fc_ll_m);

	pr_err("\nSONET Features:\n");
	pr_err("  OC3: short=%d, intermediate=%d, long=%d\n",
			base->sonet_oc3_short_reach,
			base->sonet_oc3_smf_intermediate_reach,
			base->sonet_oc3_smf_long_reach);
	pr_err("  OC12: short=%d, intermediate=%d, long=%d\n",
			base->sonet_oc12_short_reach,
			base->sonet_oc12_smf_intermediate_reach,
			base->sonet_oc12_smf_long_reach);
	pr_err("  OC48: short=%d, intermediate=%d, long=%d\n",
			base->sonet_oc48_short_reach,
			base->sonet_oc48_intermediate_reach,
			base->sonet_oc48_long_reach);
	pr_err("  OC192: short=%d\n", base->sonet_oc192_short_reach);
	pr_err("  Reach bits: bit1=%d, bit2=%d\n",
			base->sonet_reach_bit1, base->sonet_reach_bit2);

	pr_err("\nESCON Features:\n");
	pr_err("  SMF 1310 laser=%d, MMF 1310 LED=%d\n",
			base->escon_smf_1310_laser, base->escon_mmf_1310_led);

	pr_err("\nEthernet Features:\n");
	pr_err("  1000BASE: SX=%d, LX=%d, CX=%d, T=%d\n",
			base->e1000_base_sx, base->e1000_base_lx,
			base->e1000_base_cx, base->e1000_base_t);
	pr_err("  100BASE: LX=%d, FX=%d\n",
			base->e100_base_lx, base->e100_base_fx);
	pr_err("  BASE: BX10=%d, PX=%d\n",
			base->e_base_bx10, base->e_base_px);

	pr_err("\nKey Parameters:\n");
	pr_err("  Encoding: 0x%02x\n", base->encoding);
	pr_err("  BR Nominal: 0x%02x\n", base->br_nominal);
	pr_err("  Rate ID: 0x%02x\n", base->rate_id);

	pr_err("  Link length supported:\n");
	pr_err("    SMF(km): %d\n", base->link_len[0]);
	pr_err("    SMF: %d\n", base->link_len[1]);
	pr_err("    50um: %d\n", base->link_len[2]);
	pr_err("    62.5um: %d\n", base->link_len[3]);
	...
	pr_err("\nVendor Info:\n");
	pr_err("  Name: %.16s\n", base->vendor_name);
	pr_err("  OUI: %02x:%02x:%02x\n",
			base->vendor_oui[0], base->vendor_oui[1], base->vendor_oui[2]);
	pr_err("  PN: %.16s\n", base->vendor_pn);
	pr_err("  Rev: %.4s\n", base->vendor_rev);

	pr_err("\nWavelength/Cable Properties:\n");
	pr_err("  Optical wavelength: %d\n", be16_to_cpu(base->optical_wavelength));
	pr_err("  Cable compliance: 0x%04x\n", be16_to_cpu(base->cable_compliance));

	pr_err("Base CC: 0x%02x\n", base->cc_base);

	pr_err("Raw A0h page dump:\n");
	for (i = 0; i < 256; i += 16) {
		int j;

		sprintf(buf, "%02x: ", i);
		for (j = 0; j < 16; j++)
			sprintf(buf + strlen(buf), "%02x ", data[i + j]);
		pr_err("%s\n", buf);
	}
}

static void sfp_dump_a2h(struct sfp *sfp)
{
	u8 data[256] = {0};
	char buf[128] = {0};
	int i, ret;
	...
	/* Read A2h */
	ret = sfp_read(sfp, true, 0, data, sizeof(data));
	if (ret < 0) {
		pr_err("Failed to read A2h page: %d\n", ret);
		return;
	}

	pr_err("SFP EEPROM A2 Dump:\n");
	pr_err("A2h Alarm/Warning Thresholds:\n");
	pr_err("  Temperature Thresholds:\n");
	pr_err("    High Alarm: %d.%dC\n",
			(s16)((data[0] << 8) | data[1]) >> 8,
			((s16)((data[0] << 8) | data[1]) & 0xff) * 100 / 256);
	pr_err("    Low Alarm:  %d.%dC\n",
			(s16)((data[2] << 8) | data[3]) >> 8,
			((s16)((data[2] << 8) | data[3]) & 0xff) * 100 / 256);
	pr_err("    High Warning: %d.%dC\n",
			(s16)((data[4] << 8) | data[5]) >> 8,
			((s16)((data[4] << 8) | data[5]) & 0xff) * 100 / 256);
	pr_err("    Low Warning:  %d.%dC\n",
			(s16)((data[6] << 8) | data[7]) >> 8,
			((s16)((data[6] << 8) | data[7]) & 0xff) * 100 / 256);

	pr_err("  Voltage Thresholds:\n");
	pr_err("    High Alarm: %d.%03d V\n",
			(data[8] << 8 | data[9]) >> 8,
			((data[8] << 8 | data[9]) & 0xff) * 100 / 256);
	pr_err("    Low Alarm:  %d.%03d V\n",
			(data[10] << 8 | data[11]) >> 8,
			((data[10] << 8 | data[11]) & 0xff) * 100 / 256);
	pr_err("    High Warning: %d.%03d V\n",
			(data[12] << 8 | data[13]) >> 8,
			((data[12] << 8 | data[13]) & 0xff) * 100 / 256);
	pr_err("    Low Warning:  %d.%03d V\n",
			(data[14] << 8 | data[15]) >> 8,
			((data[14] << 8 | data[15]) & 0xff) * 100 / 256);

	pr_err("  TX Bias Thresholds:\n");
	pr_err("    High Alarm: %d.%02d mA\n",
			(data[16] << 8 | data[17]) >> 8,
			((data[16] << 8 | data[17]) & 0xff) * 100 / 256);
	pr_err("    Low Alarm:  %d.%02d mA\n",
			(data[18] << 8 | data[19]) >> 8,
			((data[18] << 8 | data[19]) & 0xff) * 100 / 256);
	pr_err("    High Warning: %d.%02d mA\n",
			(data[20] << 8 | data[21]) >> 8,
			((data[20] << 8 | data[21]) & 0xff) * 100 / 256);
	pr_err("    Low Warning:  %d.%02d mA\n",
			(data[22] << 8 | data[23]) >> 8,
			((data[22] << 8 | data[23]) & 0xff) * 100 / 256);

	pr_err("  TX Power Thresholds:\n");
	pr_err("    High Alarm: %d.%d uW\n",
			(data[24] << 8 | data[25]) >> 8,
			((data[24] << 8 | data[25]) & 0xff) * 100 / 256);
	pr_err("    Low Alarm:  %d.%d uW\n",
			(data[26] << 8 | data[27]) >> 8,
			((data[26] << 8 | data[27]) & 0xff) * 100 / 256);
	pr_err("    High Warning: %d.%d uW\n",
			(data[28] << 8 | data[29]) >> 8,
			((data[28] << 8 | data[29]) & 0xff) * 100 / 256);
	pr_err("    Low Warning:  %d.%d uW\n",
			(data[30] << 8 | data[31]) >> 8,
			((data[30] << 8 | data[31]) & 0xff) * 100 / 256);

	pr_err("  RX Power Thresholds:\n");
	pr_err("    High Alarm: %d.%d uW\n",
			(data[32] << 8 | data[33]) >> 8,
			((data[32] << 8 | data[33]) & 0xff) * 100 / 256);
	pr_err("    Low Alarm:  %d.%d uW\n",
			(data[34] << 8 | data[35]) >> 8,
			((data[34] << 8 | data[35]) & 0xff) * 100 / 256);
	pr_err("    High Warning: %d.%d uW\n",
			(data[36] << 8 | data[37]) >> 8,
			((data[36] << 8 | data[37]) & 0xff) * 100 / 256);
	pr_err("    Low Warning:  %d.%d uW\n",
			(data[38] << 8 | data[39]) >> 8,
			((data[38] << 8 | data[39]) & 0xff) * 100 / 256);

	pr_err("Calibration Constants:\n");
	pr_err("  RX Power Calibration:\n");
	for (i = 0; i < 5; i++) {
		pr_err("    rx_pwr%d: 0x%02x%02x%02x%02x\n",
				4-i, data[56 + i*4], data[57 + i*4],
				data[58 + i*4], data[59 + i*4]);
	}

	pr_err("Real-time Diagnostics:\n");
	pr_err("  Temperature: %d.%dC\n",
			(s16)((data[96] << 8) | data[97]) >> 8,
			((s16)((data[96] << 8) | data[97]) & 0xff) * 100 / 256);
	pr_err("  Vcc: %d.%03d V\n",
			(data[98] << 8 | data[99]) >> 8,
			((data[98] << 8 | data[99]) & 0xff) * 100 / 256);
	pr_err("  TX Bias: %d.%02d mA\n",
			(data[100] << 8 | data[101]) >> 8,
			((data[100] << 8 | data[101]) & 0xff) * 100 / 256);
	pr_err("  TX Power: %d.%d uW\n",
			(data[102] << 8 | data[103]) >> 8,
			((data[102] << 8 | data[103]) & 0xff) * 100 / 256);
	pr_err("  RX Power: %d.%d uW\n",
			(data[104] << 8 | data[105]) >> 8,
			((data[104] << 8 | data[105]) & 0xff) * 100 / 256);

	pr_err("Alarm Flags:\n");
	pr_err("  Alarm0[0x%02x]: %s%s%s%s\n",
			data[112],
			data[112] & BIT(7) ? "Temp High Alarm " : "",
			data[112] & BIT(6) ? "Temp Low Alarm " : "",
			data[112] & BIT(5) ? "Vcc High Alarm " : "",
			data[112] & BIT(4) ? "Vcc Low Alarm " : "");
	pr_err("  Alarm1[0x%02x]: %s%s%s%s\n",
			data[113],
			data[113] & BIT(7) ? "TX Bias High Alarm " : "",
			data[113] & BIT(6) ? "TX Bias Low Alarm " : "",
			data[113] & BIT(5) ? "TX Power High Alarm " : "",
			data[113] & BIT(4) ? "TX Power Low Alarm " : "");

	pr_err("Status and Control:\n");
	pr_err("  Status[0x%02x]: %s%s%s%s\n",
			data[110],
			data[110] & BIT(7) ? "TX Disable " : "",
			data[110] & BIT(2) ? "TX Fault " : "",
			data[110] & BIT(1) ? "RX LOS " : "",
			data[110] & BIT(0) ? "Data Ready" : "");

	pr_err("Raw A2h page dump:\n");
	for (i = 0; i < 256; i += 16) {
		int j;
...
static int sfp_debug_show(struct seq_file *m, void *v)
{
	struct sfp *sfp = m->private;
...
static int sfp_debug_open(struct inode *inode, struct file *file)
{
	return single_open(file, sfp_debug_show, inode->i_private);
}

static const struct file_operations sfp_debug_ops = {
	.open = sfp_debug_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int sfp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < GPIO_MAX; i++) {
		...
		err = devm_request_threaded_irq(sfp->dev, sfp->gpio_irq[i],
						NULL, sfp_irq,
						IRQF_ONESHOT
						| (v ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING)
						,
						sfp_irq_name, sfp);
		if (err) {
			pr_err("%s: %d irq[%d]_failed\n", __func__, __LINE__, i);
			sfp->gpio_irq[i] = 0;
			sfp->need_poll = true;
		}
	}
	...
	if (!sfp->sfp_bus)
		return -ENOMEM;
	sfp->debug_root = debugfs_create_dir("sfp", NULL);
	if (sfp->debug_root) {
		debugfs_create_file("eeprom", 0444, sfp->debug_root,
				sfp, &sfp_debug_ops);
	}
