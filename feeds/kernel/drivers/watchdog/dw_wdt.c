...
struct dw_wdt {
	...
	struct clk		*clk;
	struct clk		*pclk;
#if defined(CONFIG_ARCH_CLOURNEY)
	void __iomem		*gsys_reg;
	u32			wdt_no;
#endif
	unsigned long		rate;
...
static int dw_wdt_drv_probe(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_ARCH_CLOURNEY)
	dw_wdt->gsys_reg = devm_platform_ioremap_resource(pdev, 1);
	if (IS_ERR(dw_wdt->gsys_reg))
		return PTR_ERR(dw_wdt->gsys_reg);

	if (dev && dev->of_node &&
	    of_property_read_u32(dev->of_node, "wdt-number", &(dw_wdt->wdt_no)) == 0) {
		if ( dw_wdt->wdt_no > 3) {
			dev_err(dev, "wdt-number is larger than 3!\n");
			return -EINVAL;
		}
	}

	/* Enable GSYS WDT_RST_EN bit[3:0] wdt3-0 */
	writel((readl(dw_wdt->gsys_reg) | (1 << dw_wdt->wdt_no)), dw_wdt->gsys_reg);

        dw_wdt->clk = devm_clk_get(dev, "osc_ref");
        ...
        if (dw_wdt->rate == 0) {
		dev_err(dev, "dw_wdt->rate get fail!\n");
                return -EINVAL;
        }
	//dev_info(dev, "dw_wdt->rate %d \n", dw_wdt->rate);
	...
	#if defined(CONFIG_ARCH_CLOURNEY)
	dev_info(dev, "DesignWare wdt%d @%ldHz work in %s mode, timeout-sec %ds\n", (int)dw_wdt->wdt_no, dw_wdt->rate, ((dw_wdt->rmod == DW_WDT_RMOD_IRQ) ? "irq" : "reset") , wdd->timeout);
	#endif
	return 0;
