...
{
	u64 ret = 0;
	ret = ioread64(chip->regs + reg);
	pr_info("dma read64 reg:%lx = %lx\n", chip->regs + reg, ret);
	return ret;
}
...
axi_chan_iowrite32(struct axi_dma_chan *chan, u32 reg, u32 val)
{
	pr_info("dma chan write reg:%x = %x\n", chan->chan_regs + reg, val);
	iowrite32(val, chan->chan_regs + reg);
}
...
{
	int ret = 0;
	ret = ioread32(chan->chan_regs + reg);
	pr_info("dma chan read reg:%x = %x\n", chan->chan_regs, ret);
	return ret;
}
...
axi_chan_iowrite64(struct axi_dma_chan *chan, u32 reg, u64 val)
{
	 ...
	 * support 64 bit access.
	 */
	pr_info("dma chan write reg:%lx = %lx\n", chan->chan_regs + reg, val);
	iowrite32(lower_32_bits(val), chan->chan_regs + reg);
	iowrite32(upper_32_bits(val), chan->chan_regs + reg + 4);
}
#endif
...
#if 0
		pr_info("%d xfer_width:%x\n", i, xfer_width);
		pr_info("%d xfer_len:%x\n", i, xfer_len);
		pr_info("%d max_block_ts:%x\n", i, max_block_ts);
		pr_info("%d src_adr:%x\n", i, src_adr);
		pr_info("%d dst_adr:%x\n", i, dst_adr);
#endif
...
static int dw_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < hdata->nr_channels; i++) {
		...
		if (ret) {
			pr_err("axi dma request irq failed\n");
			return ret;
		}
	}
