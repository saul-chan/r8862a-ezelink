...
static void sdhci_init(struct sdhci_host *host, int soft)
{
	...
	/* read vendor base register address */
	vender_base = sdhci_readw(host, SDHCI_VENDOR_BASE_REG) & 0xFFF;
	/* Improve emmc bus efficiency */
	reg_value = sdhci_readb(host, vender_base + SDHCI_MBIU_CTRL_REG);
	reg_value &= ~SDHCI_BURST_INCR_MASK;
	reg_value |= (SDHCI_BURST_INCR16_EN | SDHCI_BURST_INCR8_EN | SDHCI_BURST_INCR4_EN);
	sdhci_writeb(host, reg_value, vender_base + SDHCI_MBIU_CTRL_REG);
	/* Enable Read Wait Control */
	sdhci_writeb(host, RD_WAIT_CTRL_ENABLE, SDHCI_BLOCK_GAP_CONTROL);
}
