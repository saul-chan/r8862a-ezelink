...
{
#ifdef CLS_BMU_MAP
	dma_addr_t bmu_dma = 0;
#if 0
	if (cls_bmu_dma_addr) {
		bmu_dma = cls_bmu_dma_addr(ptr);
		pr_err("bmu_dma:%px\n", bmu_dma);
	}
#endif

	if (cls_bmu_dma_addr && (bmu_dma = cls_bmu_dma_addr(ptr))) {
		if (dir == DMA_TO_DEVICE) {
			__flush_dcache_area(ptr, size);
		} else
			pr_err("dma_map_single_attrs not support :%x cache operation\n", dir);
