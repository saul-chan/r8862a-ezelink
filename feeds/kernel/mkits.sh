...
while getopts ":A:a:c:C:D:d:e:f:i:k:l:n:o:O:v:Z:r:t:H:" OPTION
do
	case $OPTION in
		A ) ARCH=$OPTARG;;
		a ) LOAD_ADDR=$OPTARG;;
...
if [ -n "${DTB}" ]; then
	FDT_NODE="
		fdt${REFERENCE_CHAR}$FDTNUM {
			description = \"${ARCH_UPPER} ClourneySemi ${DEVICE} device tree blob\";
			${COMPATIBLE_PROP}
			data = /incbin/(\"${DTB}\");
			type = \"flat_dt\";
			${DTADDR:+load = <${DTADDR}>;}
...
if [ -n "${INITRD}" ]; then
	INITRD_NODE="
		initrd${REFERENCE_CHAR}$INITRDNUM {
			description = \"${ARCH_UPPER} ClourneySemi ${DEVICE} initrd\";
			${COMPATIBLE_PROP}
			data = /incbin/(\"${INITRD}\");
			type = \"ramdisk\";
			compression = \"${INITRD_ZIP}\";
			arch = \"${ARCH}\";
			os = \"linux\";
			hash@1 {
				algo = \"crc32\";
...
if [ -n "${ROOTFS}" ]; then
	...
	ROOTFS_NODE="
		rootfs${REFERENCE_CHAR}$ROOTFSNUM {
			description = \"${ARCH_UPPER} ClourneySemi ${DEVICE} rootfs\";
			${COMPATIBLE_PROP}
...
[ "$DTOVERLAY" ] && for overlay in $DTOVERLAY ; do
	...
	FDTOVERLAY_NODE="$FDTOVERLAY_NODE
		...
		$ovnode {
			description = \"${ARCH_UPPER} ClourneySemi ${DEVICE} device tree overlay $ovname\";
			${COMPATIBLE_PROP}
...
"
	OVCONFIGS="$OVCONFIGS
		...
		config-$ovname {
			description = \"ClourneySemi ${DEVICE} with $ovname\";
...
/ {
	description = \"${ARCH_UPPER} ClourneySemi FIT (Flattened Image Tree)\";
	#address-cells = <1>;

	images {
		kernel${REFERENCE_CHAR}1 {
			description = \"${ARCH_UPPER} ClourneySemi Linux-${VERSION}\";
			data = /incbin/(\"${KERNEL}\");
			type = \"kernel\";
			arch = \"${ARCH}\";
			os = \"linux\";
...
${ROOTFS_NODE}
	...
	configurations {
		default = \"${CONFIG}\";
		${CONFIG} {
			description = \"ClourneySemi ${DEVICE}\";
			kernel = \"kernel${REFERENCE_CHAR}1\";
			${FDT_PROP}
			${LOADABLES:+loadables = ${LOADABLES};}
			${COMPATIBLE_PROP}
