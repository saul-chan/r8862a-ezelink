		      ...
		      struct snd_pcm_substream *substream)
{
	...
	i2s_write_reg(dev->i2s_base, IER, readl(dev->i2s_base + IER) | 1);
...
static void dw_i2s_config(struct dw_i2s_dev *dev, int stream)
{
	...
	for (ch_reg = 0; ch_reg < (config->chan_nr / 2); ch_reg++) {
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			if(dev->mode == I2S_MODE)
				i2s_write_reg(dev->i2s_base, TER(ch_reg), 0x101);
			else
				i2s_write_reg(dev->i2s_base, TER(ch_reg), 0x301);
			#else
			i2s_write_reg(dev->i2s_base, TER(ch_reg), 0x1);
			#endif
		} else {
			...
			#ifdef CONFIG_I2S_DMA_REPLACE_FIFO
			if(dev->mode == I2S_MODE)
				i2s_write_reg(dev->i2s_base, RER(ch_reg), 0x101);
			else
				i2s_write_reg(dev->i2s_base, RER(ch_reg), 0x301);
			#else
			i2s_write_reg(dev->i2s_base, RER(ch_reg), 0x1);
			#endif
		}
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (dev->capability & DW_I2S_MASTER) {
		if (dev->i2s_clk_cfg) {
			...
			writel(DIV_ROUND_CLOSEST(top_sysclk_frequence, bitclk), dev->i2s_clk_base + TOP_SYSCLK_DIV_OFFSET);
...
static int dw_i2s_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_DSP_B:
		dev->mode = PCM_MODE;
		i2s_write_reg(dev->i2s_base, IER, PCM_MODE_AND_SLOT_VAL);
		break;
	default:
		return -EINVAL;
	}
