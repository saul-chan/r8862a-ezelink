...
return baseclass.extend({
	...
	addLanDeviceOption: function(s, tab, name, label, description, ld_list) {
		var o = s.taboption(tab, this.CBIDynamicMultiValueList, name, label, description);

		o.modalonly = true;
		o.placeholder = _('-- add Lan Device --');

		L.sortedKeys(ld_list).forEach(function(index) {
			if (ld_list[index].dev.hasOwnProperty('bridge'))
				o.value(ld_list[index].device, E([], [ ld_list[index].device]));
		});
	...
	CBIProtocolSelect: form.MultiValue.extend({
		...
		load: function(section_id) {
			...
			['all', 'tcp', 'udp'].concat(cfgvalue).forEach(L.bind(function(value) {
				switch (value) {
				case 'all':
				case 'any':
				case '*':
