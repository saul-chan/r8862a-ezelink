...
#define RX_BUFFER_SIZE 64*1024 /*MTC0330-52*/
...
CTTYComm2::CTTYComm2()
{
    ...
    m_RxBuffer = malloc(RX_BUFFER_SIZE); /*MTC0330-52*/
}

CTTYComm2::~CTTYComm2()
{
    Close();
    if(m_RxBuffer) { /*MTC0330-52*/
        free(m_RxBuffer);
        m_RxBuffer = NULL;
    }
}
...
{
	uint32_t nRealSize = RX_BUFFER_SIZE;//0; /*MTC0330-52*/
	uint8_t *pBuf = (uint8_t *) m_RxBuffer; //m_MemMgr.GetMemory(1024,&nRealSize);
...
*/
	...
	{
		m_pObserver->OnChannelData(pBuf,uRead,(void*)m_dwPort);
	}
	
	//m_MemMgr.FreeMemory(pBuf);/*MTC0330-52*/
	return;
}
...
void* CTTYComm2::RecvFunc()
{
    ...
    {
        tv_timeout.tv_sec = 0; /*MTC0330-52*/
        tv_timeout.tv_usec = 500*1000; //500ms
