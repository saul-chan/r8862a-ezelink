...
/* log threashold */
long long log_file_threashold = 0;
long long log_dir_threashold  = 0;
...
BOOL CChannelDataHandler::LoadConfig()
{
	...
	if (config)
	{
        ...
        log_file_threashold *= SIZE_GB;

        log_dir_threashold  = ini_config_get_int(config, "Setting", "log_dir_threashold", 10); // default 10 GB
        log_dir_threashold *= SIZE_GB;
	}
	else
	{		
		return FALSE;
...
BOOL CChannelDataHandler::StartProc()
{
    ...
    do {
        ...
        #endif
        printf("CChannelDataHandler::StartProc:%s %s %llu %llu", szDevPath_diag, szDevPath_log, log_file_threashold, log_dir_threashold);
...
void* CChannelDataHandler::handlerServiceFunc(void* lpParam)
{
    ...
    if (NULL != p)
    {
        ...
        while (1)
        {
            ...
            nFd_u2s  = open(szDevPath_u2s,  O_RDWR|O_NOCTTY,0666);
            close(nFd_u2s);
...
int CChannelDataHandler::OnChannelData(void* lpData, uint32_t ulDataSize,void* reserved /*=0*/ )
{
    LockDataProc( true );
