...
extern int dlt_vlog(int prio, const char *format, ...);
...
/*this is implemented by porting part, it is needed to check if it is quiting*/
int is_Quiting() {
    int result = 0;
...
//#define dbg(fmt, arg... ) do { unsigned msec = slog_msecs();  dlt_vlog(LOG_INFO,"[%03d.%03d]" fmt,  msec/1000, msec%1000, ## arg);} while (0)
...
#define dbg(fmt, arg...)                                            \
    ...
    {                                                               \
        ...
        dlt_vlog(LOG_INFO,"[%03d.%03d]" fmt, msec / 1000, msec % 1000, ##arg); \
    } while (0)
...
static int tftp_send_data_pkt(int sock, struct tftp_data_packet *tx_pkt, uint16_t block, int size, struct sockaddr_in *to, int is_sync) {
    struct tftp_packet rx_pkt;
    int wait_ack = 1;  /*willa.liu 20240529 JIRA:MBB0120-492 */
    int ret;
    struct sockaddr_in sender;
    ...
    while (wait_ack) {  /*willa.liu 20240529 JIRA:MBB0120-492 */
        ...
        if(ret >= 4 && rx_pkt.cmd == htons(TFTP_ACK) && rx_pkt.block == htons(block)) {
            wait_ack = 0;  /*willa.liu 20240529 JIRA:MBB0120-492 */
            return size;
        }
...
void s_ftp_quit(void)
{
    ...
    for(i=0;i<3;i++)
    {
        ...
        if (s_ftp_tag_comb.q_ftp_t[i].server_fd != -1) {
            ...
            strcpy(s_ftp_tag_comb.q_ftp_t[i].login, "no");
        }

        if (s_ftp_tag_comb.q_ftp_t[i].server_fd != -1) {
            close(s_ftp_tag_comb.q_ftp_t[i].server_fd);
            s_ftp_tag_comb.q_ftp_t[i].server_fd = -1;
        }
    }
}
