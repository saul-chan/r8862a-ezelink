...
extern int dlt_receive_update_latest_read_time(void);
...
DltReturnValue dlt_filter_load(DltFilter *filter, const char *filename, int verbose)
{
    ...
    while (!feof(handle)) {
        ...
        if (log_level < 0) {
            log_level = 0;
        } else if(log_level >= DLT_LOG_MAX){
            log_level = DLT_LOG_MAX - 1;
        }

        if (filter->counter < DLT_FILTER_MAX)
            dlt_filter_add(filter, apid, ctid, log_level, 0, INT32_MAX, verbose);// old: log_level is 0
        else
            dlt_vlog(LOG_WARNING,
...
int dlt_receiver_receive(DltReceiver *receiver)
{
    ...
    if (receiver->type == DLT_RECEIVE_SOCKET)
        ...
        receiver->bytesRcvd = read(receiver->fd,
                                   receiver->buf + receiver->lastBytesRcvd,
                                   receiver->buffersize - (uint32_t) receiver->lastBytesRcvd);
        // record latest time after getting valid data
        if (receiver->bytesRcvd > 0) {
            dlt_receive_update_latest_read_time();
        }
    ...
    if (receiver->bytesRcvd <= 0) {
        //dlt_vlog(LOG_WARNING,"read failed, errno=%d,%s,receiver->bytesRcvd=%d\n",errno,strerror(errno),receiver->bytesRcvd);
        receiver->bytesRcvd = 0;
        //dlt_vlog(LOG_WARNING,"read failed, errno=%d,%s\n",errno,strerror(errno));
