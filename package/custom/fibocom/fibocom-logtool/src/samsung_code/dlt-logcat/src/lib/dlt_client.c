...
DltReturnValue dlt_client_main_loop(DltClient *client, void *data, int verbose)
{
    ...
    while ((!g_quit_flag) && fetch_next_message) {
        /* wait for data from socket or serial connection */
        ret = dlt_receiver_receive(&(client->receiver));
        //dlt_vlog(LOG_WARNING,"after dlt_receiver_receive ret=%d,bytesRcvd=%d,totalBytesRcvd=%d\n",ret,client->receiver.bytesRcvd,client->receiver.totalBytesRcvd);
        if (ret <= 0) {
            //for debug
            //dlt_vlog(LOG_WARNING,"dlt_client_main_loop, errno=%d,%s,ret=%d\n",errno,strerror(errno),ret);
            if (errno == 0) {
                //dlt_vlog(LOG_WARNING,"dlt_client_main_loop, errno=%d,%s,ret=%d, line=%d, go on!!!\n",errno,strerror(errno),ret, __LINE__);
                //{
                //    int * hel = 0x0;
                //    *hel = 99;
                //}
                if (!g_quit_flag && (client->serial_exists != 0)) {
                    continue;
                }
            }
            ...
            //dlt_vlog(LOG_WARNING,"dlt_client_main_loop, errno=%d,%s,ret=%d\n",errno,strerror(errno),ret);
			//change it to be wrong after receiver got problem
        ...
        while ((read_result = dlt_message_read(&msg, (unsigned char *)(client->receiver.buf),
                                (unsigned int) client->receiver.bytesRcvd, 1,  //old is 0, which is no resync, 
        ...
        /*new added: special case for DLT_MESSAGE_ERROR_CONTENT*/
        /*give up current wrong message, another chance to get next message*/
        if (read_result == DLT_MESSAGE_ERROR_CONTENT) {
            int resync_offset = 0;
            if (msg.resync_offset > 0 && (msg.resync_offset < client->receiver.bytesRcvd)) {
                resync_offset = msg.resync_offset;
            }
            dlt_vlog(LOG_WARNING,"receiver correct wrong log content, size=%d, resync_offset=%d\n",
			                      (int) (msg.headersize + msg.datasize - sizeof(DltStorageHeader)), resync_offset);
            if (msg.found_serialheader) {
                //dlt_vlog(LOG_WARNING,"serialheader detected ,size=%d\n",(int) (msg.headersize + msg.datasize - sizeof(DltStorageHeader) + sizeof(dltSerialHeader)));
                if (dlt_receiver_remove(&(client->receiver),
                                        (int) (msg.headersize + msg.datasize - sizeof(DltStorageHeader) +
                                        sizeof(dltSerialHeader) + resync_offset)) ==
                    DLT_RETURN_ERROR) {
                    /* Return value ignored */
                    dlt_message_free(&msg, verbose);
                    dlt_vlog(LOG_WARNING,"receiver remove failed 00\n");
                    return DLT_RETURN_ERROR;
                }
            }
            else if (dlt_receiver_remove(&(client->receiver),
                ...
                /* Return value ignored */
                dlt_message_free(&msg, verbose);
                dlt_vlog(LOG_WARNING,"receiver remove failed 11\n");
                return DLT_RETURN_ERROR;
            }
...
DltReturnValue dlt_client_send_ctrl_msg(DltClient *client, char *apid, char *ctid, uint8_t *payload, uint32_t size)
{
    ...
    if ((client->mode == DLT_CLIENT_MODE_TCP) || (client->mode == DLT_CLIENT_MODE_SERIAL)) {
        /*send serial Header always*/
        if (client->mode == DLT_CLIENT_MODE_SERIAL) {
            ret =
                (int) write(client->sock, (void *)dltSerialHeader, sizeof(dltSerialHeader));

            if (0 > ret) {
                dlt_vlog(LOG_ERR, "%s: Sending serial header failed\n", __func__);
                dlt_message_free(&msg, 0);
                return DLT_RETURN_ERROR;
            }
        }
...
/*new added for informing stop output log via serial*/
DltReturnValue dlt_client_message_stop_output(DltClient *client)
{
    DltServiceMessageStopOutput *req;
    int ret = DLT_RETURN_ERROR;

    if (client == NULL)
        return ret;

    req = (DltServiceMessageStopOutput *)malloc(sizeof(DltServiceMessageStopOutput));

    req->service_id = DLT_SERVICE_ID_MESSAGE_STOP_OUTPUT;
    ...
    ret = dlt_client_send_ctrl_msg(client,
                                   ...
                                   "",
                                   (uint8_t *)req,
                                   sizeof(DltServiceMessageStopOutput));
