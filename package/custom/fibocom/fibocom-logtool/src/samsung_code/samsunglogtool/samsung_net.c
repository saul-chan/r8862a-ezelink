...
static int ss_tcp_create_socket(char *ifname, char *server_ip, int server_port)
{
    int sock = -1;
    struct ifreq ifr;
    ...
    if (connect(sock, (struct sockaddr *)&ser, sizeof(ser)) < 0) {
        LogInfo("TCP connection error %d:%s\n", errno, strerror(errno));
        close(sock);
        return -1;
    }
...
int samsung_tcp_write(const void *handle, void *pbuf, int size, int portnum)
{
    int sock = ss_find_sock_by_handle((void *)handle);

    (void)portnum;

    return ss_tcp_write(sock, pbuf, size);
}

int samsung_tcp_read(const void *handle, void *pbuf, int size, int portnum)
{
    int sock = ss_find_sock_by_handle((void *)handle);

    (void)portnum;

    return ss_tcp_read(sock, pbuf, size);
}

int samsung_open_tcp(fibo_usbdev_t *pdev, char *ifname, char *server_ip, int server_port)
{
    int sock = -1;
    ...
    sock = ss_tcp_create_socket(ifname, server_ip, server_port);
    if (sock < 0) {
