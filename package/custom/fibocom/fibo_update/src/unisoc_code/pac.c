...
int pac_open_bin(pac_ctx_t *ctx, struct bin_header_t *bin_hdr)
{
    ...
    if (verbose) dprintf("%s FileID: %s, FileName: %s, Size: %u\n",
...
int xml_parse_Partition(pac_ctx_t *ctx, const char *xml_s, const char *xml_e)
{
    ...
    if (verbose) dprintf("id=\"%s\" size=\"%s\"\n", Partition.id, Partition.size);
...
*/
static int xml_parse_File(pac_ctx_t *ctx, const char *xml_s, const char *xml_e, char backup)
{
    Scheme_t File;
    char *p, *pBlock;
    size_t n, nBlock;
...
static int xml_parser(pac_ctx_t *ctx, char *xml_buf, size_t xml_len)
{
    ...
    if (!pPartitions) goto _skip_partition; // UIC8910_MODEM no partition
...
_skip_partition:
    ...
    if (verbose) dprintf("%-20s%-16s%-12s%-12s%s\n", "FileID", "Base", "Size", "FileSize", "FileName");
...
static int pac_parser_xml(pac_ctx_t *ctx)
{
    ...
    if (!bin_hdr)
    {
        dprintf("bin_hdr is NULL\n");
        goto out;
    }

    ret = pac_open_bin(ctx, bin_hdr);
    if (0 !=ret)
    {
        dprintf("open bin fail,ret %d\n", ret);
        goto out;
    }
...
int pac_parser(pac_ctx_t *ctx, FILE *fw_fp)
{
    ...
    if (strlen((char *)(pachdr->szPrdVersion)))
        dprintf("ProductVersion: %s\n", (char *)(pachdr->szPrdVersion));
    dprintf("szPrdAlias: %s\n", (char *)(pachdr->szPrdAlias));

    if (!strcasecmp((char *)(pachdr->szPrdName), "SL8563") || !strcasecmp((char *)(pachdr->szPrdName), "sl8563-cpe-2h10-vsim")) {
        ctx->sl8563 = 1;
        dprintf("ctx->sl8563: %d\n", ctx->sl8563);
    }
