...
int extra_backup_nv_8850[]={IMEI2_ITEM_ID, STATIC_NV_2_ITEM_ID, RF_NV_ITEM_ID};
int extra_backup_nv_8910[]={IMEI2_ITEM_ID, BT_CONFIG_ITEM_ID,STATIC_NV_ITEM_ID, STATIC_NV_2_ITEM_ID,
    RF_NV_ITEM_ID, AUDIO_CALIB_ITEM_ID, VERSION_ITEM_ID};
...
static const BSL_CMD_NAME_T bsl_cmd_name[] = {
    ...
    bsl_cmd_item(BSL_CMD_ENABLE_WRITE_FLASH), //added by fibocom
...
static int ql_get_kernel_version(void)
{
    ...
    dprintf("utsname.release: %s\n", utsname.release);
    kernel_version = KVERSION(osmaj, osmin, ospatch);
    dprintf("kernel_version: %d\n", kernel_version);
...
static int pdl_send_cmd(pac_ctx_t *ctx, PDL_CMD_T *pdl_cmd)
{
    ...
    ret = ctx->pdev->write(ctx->pdev, &pdl_cmd->ucTag, sizeof(PDL_CMD_T), 0, 3000);
    if (ret == sizeof(PDL_CMD_T)) {
        ret = ctx->pdev->write(ctx->pdev, pdl_cmd->pkt, sizeof(struct pdl_pkt_tag), 0, 3000);
        if (ret == sizeof(struct pdl_pkt_tag)) {
            int size = le32toh(pdl_cmd->nDataSize) - sizeof(struct pdl_pkt_tag);
            if (size > 0)
                ret = ctx->pdev->write(ctx->pdev, pdl_cmd->pkt[0].data, size, 0, 3000);
        }
    }
...
static PDL_CMD_T *pdl_read_cmd(pac_ctx_t *ctx, unsigned timeout)
{
    ...
    ret = ctx->pdev->read(ctx->pdev, pdl_rsp, sizeof(ctx->bsl_rsp_data), 0, timeout? timeout : 15000);
...
int pdl_send_cmd_wait_ack(pac_ctx_t *ctx, PDL_CMD_T *pdl_req, uint16_t pdl_cmd, unsigned timeout)
{
    ...
    if (le16toh(pdl_rsp->pkt[0].dwCmdType) != PDL_RSP_ACK)
        { dprintf("pdl_rsp->pkt[0].dwCmdType: 0x%X\n", be16toh(pdl_rsp->pkt[0].dwCmdType)); return -1; }
...
void bsl_cmd_dump(BSL_CMD_T *bsl_cmd)
{
    ...
    if (!name) {
        dprintf("unknow bsl_cmd: cmd:0x%04x, len:%d\n", cmd, be16toh(bsl_cmd->len));
        return;
    }
...
static int bsl_send_cmd(pac_ctx_t *ctx, BSL_CMD_T *bsl_cmd)
{
    ...
    if (be16toh(bsl_cmd->len) && skip_7d) {
        ...
        return ctx->pdev->write(ctx->pdev, &bsl_cmd->magic, size, 0, 3000);
     }
    ...
    return ctx->pdev->write(ctx->pdev, buf, size, 0, 3000);
}
...
static BSL_CMD_T *bsl_read_cmd(pac_ctx_t *ctx, unsigned timeout)
{
    ...
    ret = ctx->pdev->read(ctx->pdev, bsl_rsp, sizeof(ctx->bsl_rsp_data), 0, timeout? timeout : 15000);
    ...
    if (ret > 0) {
        ...
        if (be16toh(bsl_rsp->cmd) == BSL_REP_READ_FLASH) {
            ...
            if ((ctx->platform == UIC8910 || ctx->platform == UIC8850 || ctx->platform == UIC8310) && (skip_7d == 0)) {
                ...
                while ((read_len < (need_len + cnt_7d)) && (ret == 128 || ret == 129)) {
                    ret = ctx->pdev->read(ctx->pdev, data + read_len, sizeof(ctx->bsl_rsp_data), 0, 3000);
                    if (ret > 0) {
                        for (i = 0; i < (uint32_t)ret; i++) {
...
static int bsl_check_baud(pac_ctx_t *ctx) {
    ...
    dprintf("\t%c %s\n", '>', "BSL_CMD_CHECK_BAUD");
    for (i = 0; i < 5; i++) {
        ret = ctx->pdev->write(ctx->pdev, &hello, 1, 0, 3000);
        if (ret != 1) return -1;

        bsl_rsp = bsl_read_cmd(ctx, 1000);
        if (bsl_rsp && be16toh(bsl_rsp->cmd) == BSL_REP_VER) {
            dprintf("BSL_CMD_CHECK_BAUD OK\n");
            return 0;
        }
    }
...
static int FileisNV(pac_ctx_t *ctx, Scheme_t *File) {
    if (ctx->platform == UIC8910 || ctx->platform == UIC8850 || ctx->platform == UIC8310)
        return (!strcasecmp(File->FileID, "NV"));

    return (!strcasecmp(File->FileID, "NV_NR") || !strcasecmp(File->FileID, "NV_LTE"));
}
...
static int bsl_flash_id(pac_ctx_t *ctx, Scheme_t *File)
{
    ...
    if (ctx->platform == UIC8850 && !strcasecmp(File->FileID, "NV"))
        frame_sz = FRAMESZ_PDL;

    if (ctx->platform == UIC8310 && !strcasecmp(File->FileID, "NV"))
        frame_sz = FRAMESZ_PDL;
    ...
    if (nv_crc && File->nv_buf) {
        ...
        if (ctx->platform == UDX710 && ctx->sl8563 == 0 && File->CheckFlag[0] == '0')
        {
            //do nothing
        }else
        {
    ...
    while (curSize < fileSize)
    {
        ...
        curSize += ret;
        if (bsl_send_cmd_wait_ack(ctx, bsl_req, 0, 0)) {
            dprintf("curSize: %zd, fileSize:%zd\n", curSize, fileSize);
            bsl_exec(-1);
        }
        // bsl_exec(bsl_send_cmd_wait_ack(ctx, bsl_req, 0, 0));
    }
...
static int bsl_backup_id(pac_ctx_t *ctx, Scheme_t *File)
{
    ...
    if (access(backup_dir, W_OK) && errno == ENOENT)
        backup_dir = "/cache";
    snprintf(backup_file, sizeof(backup_file), "%s/unisoc_bak_%s", backup_dir, File->FileID);
    dprintf("%-8s %s -> '%s'\n", "Backup", File->FileID, backup_file);

    fileSize= strtoul(File->Size, NULL, !strncmp(File->Size, "0x", 2) ? 16 : 10);

    if(0 == fileSize && !strcasecmp(File->FileID, "NV_LTE"))
    if (File->partition) {
        fileSize = File->partition->PartitionSize;
    }

    if (ctx->platform == UDX710) {
    ...
    if (nv_crc) {
        ...
        if(ctx->platform == UIC8850)
        {
            for(i=0; i<(sizeof(extra_backup_nv_8850)/sizeof(extra_backup_nv_8850[0])); i++)
            ctx->NVBackupList[ctx->NVBackupNum++] = extra_backup_nv_8850[i];
        }
        else if(ctx->platform == UIC8910)
        {
            for(i=0; i<(sizeof(extra_backup_nv_8910)/sizeof(extra_backup_nv_8910[0])); i++)
            ctx->NVBackupList[ctx->NVBackupNum++] = extra_backup_nv_8910[i];
        }

        for(i=0; i<ctx->NVBackupNum; i++)
        {
            dprintf("NVBackupList %d, nvid is %x\r\n", i, ctx->NVBackupList[i]);
        }
...
static int dloader_bsl(pac_ctx_t *ctx )
{
    int i, j;
    int spl_index = -1;
    Scheme_t *spl_x;
    ...
    for (i = 0; i < ctx->SchemeNum; i++)
    {
        ...
        Scheme_t *bx = NULL;
        dprintf("FileID: %s,platform %d,q_erase_all %d, erase_all_once %d\r\n", x->FileID, ctx->platform, q_erase_all, erase_all_once);
        if (q_erase_all && ctx->platform == UIC8910 && erase_all_once) //erass all
        {
            Scheme_t x_tmp;
        ...
        if (!strcasecmp(x->FileID, "HOST_FDL")) {
            frame_sz = FRAMESZ_PDL;
            ctx->pdev->usb_need_zero_package = 0;
            ...
            ctx->pdev->usb_need_zero_package = 1;
        }
            ...
            for (j = i; j < ctx->SchemeNum; j++) {
                ...
                if (ctx->platform == UIC8910 || ctx->platform == UIC8310) {
            ...
            //start: add for sl8563
            if (ctx->sl8563){
                bsl_exec(bsl_send_cmd_wait_ack(ctx, NULL, BSL_CMD_ENABLE_WRITE_FLASH, 0));
            }
            //end: add for sl8563
        ...
        else if (x->backup[0] == '1' && ctx->platform == UDX710) {
        }
        else if (x->CheckFlag[0] == '0' && ctx->platform == UDX710) {
            dprintf("skip download FileID: %s\n", x->FileID);
        }
#if FIBO_UIC8310
        else if (ctx->platform == UIC8310 && strncmp(x->FileID,"Erase BOOT0",11) == 0) {
            dprintf("into FileID: %s\n", x->FileID);
            bsl_exec(bsl_erase_id(ctx, x));
        }
#endif
        else if (ctx->platform == UIC8910 && strcmp(x->FileID,"ERASE_BOOT") == 0) {
            bsl_exec(bsl_erase_id(ctx, x));
        }

        else if (ctx->platform == UIC8850 && strcmp(x->FileID, "SPL") == 0)
        {
            frame_sz = FRAMESZ_PDL;
            bsl_exec(bsl_check_baud(ctx));
            bsl_exec(bsl_send_cmd_wait_ack(ctx, NULL, BSL_CMD_CONNECT, 0));
            //first erase spl,and wait for other partitions to complete the upgrade
            bsl_exec(bsl_erase_id(ctx, x));
            spl_index = i;
            //bsl_exec(bsl_flash_id(ctx, x));
        ...
        else if (!strcasecmp(x->FileID, "PhaseCheck")) {
            dprintf("skip FileID: %s\n", x->FileID);
        }
#if FIBO_UIC8310
        else if (ctx->platform == UIC8310 && (!strcasecmp(x->FileID, "BOOT") || !strcasecmp(x->FileID, "EraseUserNV") || !strcasecmp(x->FileID, "EraseRunNV"))) {
            bsl_exec(bsl_erase_id(ctx, x));
        }
        else if (ctx->platform == UIC8310 && (!strcasecmp(x->FileID, "XFILE") || !strcasecmp(x->FileID, "AUTOSMS")
            ...
            || !strncmp(x->FileID, "Erase FS", 8) || !strcasecmp(x->FileID, "PreloadUdisk"))) {
            dprintf("skip FileID: %s\n", x->FileID);
        }
        else if (ctx->platform == UIC8310 && (!strcasecmp(x->FileID, "PhaseCheck") || !strcasecmp(x->FileID, "FotaUpdate"))) {
            bsl_exec(bsl_flash_id(ctx, x));
        }
#endif
        ...
        else if ((ctx->platform == UIC8910 || ctx->platform == UIC8850) && !strcasecmp(x->FileID, "FMT_FSSYS")) {
            //dprintf("Unknow FileID: %s\n", x->FileID);
        }
        else if (ctx->platform == UIC8850 && (!strcasecmp(x->FileID, "FMT_FSMOD") || !strcasecmp(x->FileID, "FLASH"))) {
            bsl_exec(bsl_erase_id(ctx, x));
        }
        else if (ctx->platform == UIC8910 && !strcasecmp(x->FileID, "FMT_FSEXT")) {
            //dprintf("Unknow FileID: %s\n", x->FileID);
        }
        else if (!strcasecmp(x->Type, "EraseFlash2")|| !strcasecmp(x->FileID, "Misc"))
        {
            bsl_exec(bsl_erase_id(ctx, x));
        }
        else {
    ...
    if(spl_index != -1)
    {
        //Finally upgrade the SPL partition,The reason for doing this is to implement power failure
        //protection that the download mode can still be entered next time the upgrade is interrupted and the power is cut off
        spl_x = &ctx->SchemeList[spl_index];
        frame_sz = FRAMESZ_PDL;
        bsl_exec(bsl_flash_id(ctx, spl_x));
    }
...
int unisoc_dloader_main(fibo_usbdev_t *pdev, char *package_name)
{
    ...
    else if (strstr(pdev->ModuleName, "UIC8910")) {
        ctx->platform = UIC8910;
    }
    else if (strstr(pdev->ModuleName, "UIC8850")) {
        ctx->platform = UIC8850;
    }
    else {
    ...
    fw_fp = fopen(package_name, "rb");
    if (!fw_fp) {
        LogInfo("fopen (%s) failed, errno: %d (%s)\n", package_name, errno, strerror(errno));
        goto out;
    }

    LogInfo("fopen package success\n");
    q_erase_all = pdev->erase_all_before_download;
    if (pac_parser(ctx, fw_fp))
        {
            LogInfo("pac_parser error\n");
            goto out;
        }

    LogInfo("parser package success\n");
    ret = dloader_bsl(ctx);
out:
    if (ctx->xml_buf) {
        free(ctx->xml_buf);
