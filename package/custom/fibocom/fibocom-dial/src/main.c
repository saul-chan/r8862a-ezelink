...
#define MAX_PATH 256
#define SUBDIR_SIZE 1024
#define SUBDIR2_SIZE 2048
//2020-12-23 willa.liu@fibocom.com changed begin for support mantis 0065286
struct sockaddr_un {
...
int g_project_mode = PROJECT_UNKONW;
...
//2024-03-19 Nick@fibocom.com changed for support X35 apn set
static int g_set_apn_by_m2m_qmi = 0;
//2020-03-19 Nick@fibocom.com changed for support X35 apn set end
...
static int usage(const char *progname)
{
    ...
    dbg_time("-d qmiChannelName                      Specify qmichannel name");
...
//2020-12-29 willa.liu@fibocom.com changed end for support mantis 0066901
    ...
    dbg_time(
    "-m PDP ID                              Specify PDP and APN when "
    "multi-pdn data connection(This ID corresponds to the query result for cgdcont).");
    dbg_time(
    "-n virtual network adapter ID        Specify virtual network adapter ID when "
...
//2020-12-23 willa.liu@fibocom.com changed end for support mantis 0065286
    ...
    dbg_time("Example 3: %s -s 3gnet carl 1234 0 -p 1234 -f gobinet_log.txt",
         progname);
    dbg_time("Example 4: %s -d /dev/mhi_QMI0 -s 3gnet carl 1234 0 -p 1234 -f gobinet_log.txt",
         progname);
    return 0;
}
...
static int qmipciedevice_detect(char **pp_qmichannel, char **pp_usbnet_adapter)
{
    ...
    while ((ent = readdir(pDir)) != NULL) {
        ...
        DIR *psubDir;
        char idVendor[4 + 3] = {0}; //Modify by gabriel.tian@fibocom.com to avoid of overflow.Reason :device id in pcie mode strlen(0x1508\0)=7.
        char idDevice[4 + 3] = {0}; //Modify by gabriel.tian@fibocom.com to avoid of overflow.Reason :strlen(0x1508\0)=7.
        ...
        if (!strncasecmp(idVendor, "0x17cb", 6) && (!strncasecmp(idDevice, "0x0306", 6) || !strncasecmp(idDevice, "0x0308", 6)))
        ;
        else
        continue;
...
//2021-02-08 zhangkaibo@fibocom.com changed start for mantis 0070613
        ...
        else if(!strncasecmp(idProduct, "0112", 4)
            || !strncasecmp(idProduct, "0116", 4)
            || !strncasecmp(idProduct, "0117", 4))
        {
            g_set_apn_by_m2m_qmi = 1;  //support apn by m2m qmi
            indexid = 0;
...
//2021-03-24 willa.liu@fibocom.com changed end for support mantis 0071817
    ...
    dbg_time("Fibocom-dial_Linux_Tool_V1.%s", VERSION_STRING());
...
//end modified by zhangkaibo limit input parameters. mantis 0048965  20200605
    ...
    case 'm':
        if (has_more_argv())
        {
                ...
                if(strlen(argvtemp) > 1 ||  !IsInt(argvtemp))
                {
                    dbg_time("Invalid parament for \"-m\"");
                    return -1;
                }
...
//2020-12-23 willa.liu@fibocom.com changed end for support mantis 0065286
    ...
    dbg_time("%s adapter[%d] profile[%d] = %s/%s/%s/%d, pincode = %s", argv[0], profile.pdp, profile.pdpindex,
...
//2021-02-08 zhangkaibo@fibocom.com changed end for mantis 0070613
    ...
    fibo_get_netcard_driver_info(profile.usbnet_adapter);
...
//end modified by zhangkaibo limit input parameters. mantis 0048965  20200605
    ...
    product = requestBaseBandVersion(NULL);
#endif
    requestSetEthMode(&profile);

    if(g_set_apn_by_m2m_qmi)
    {
        if(product == 1)
        {
            g_project_mode = PROJECT_FG131;
        }
        else if(product == 0)
        {
            g_project_mode = PROJECT_FG132;
            dbg_time("Project: FG132");
        }
    }

#ifdef CONFIG_SIM
    //2024-06-28 Modify Start, qiujianhua, MTC0732-2017, Send AT will delay 3s, FG132 no need rebind NAS/DMS.
    if(g_project_mode == PROJECT_FG132)
    {
        sim_number = 0;
    }
    else
    ...
    //2024-06-28 Modify End, qiujianhua, MTC0732-2017
...
//2021-03-24 willa.liu@fibocom.com changed begin for support mantis 0071817
    ...
    if (profile.apn || profile.user || profile.password)
    {
        if(g_set_apn_by_m2m_qmi)
        {
            requestSetProfile_by_m2m(&profile , product);
        }
        else
        {
...
//2021-02-05 willa.liu@fibocom.com changed begin for support mantis 0070460
            ...
            case SIGUSR2:
            {
                ...
                if((profile.ipv4_flag && profile.ipv6_flag ) && ((IPv4ConnectionStatus == QWDS_PKT_DATA_CONNECTED && IPV6ConnectionStatus == QWDS_PKT_DATA_CONNECTED))    )
                    {
                ...
                else if(profile.ipv4_flag == 1 && profile.ipv6_flag != 1 )
                {
                    if(profile.ipv4_flag && IPv4ConnectionStatus == QWDS_PKT_DATA_DISCONNECTED)
                    {
                        send_signo_to_main(SIGUSR1);
                ...
                {
                    if(profile.ipv6_flag && IPV6ConnectionStatus == QWDS_PKT_DATA_DISCONNECTED)
                    {
                        send_signo_to_main(SIGUSR1);
                    }
                }
