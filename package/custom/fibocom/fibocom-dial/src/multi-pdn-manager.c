...
static bool is_gobinet_dial()
{
    int i = 0;
    char qmi[20];
    while (i != MAX_DEVICE){
        snprintf(qmi,sizeof(qmi),"/dev/qcqmi%d",i);
        if (access(qmi, F_OK) == 0)
            return true;
        i++;
   }
...
static bool is_qmiwwan_dial()
{
     int i = 0;
     char qmi[20];
     while (i != MAX_DEVICE){
         snprintf(qmi,sizeof(qmi),"/dev/cdc-wdm%d",i);
         if (access(qmi, F_OK) == 0)
            return true;
         i++;
    }
...
char fileContents[FILE_LINE_MAX][LINE_CONTENT_MAX_LEN];
int fileIndex = 0;
...
//end modified by zhangkaibo add sim pin operate. mantis 0049142  20200612
    ...
    // read all
    while (feof(fp) == 0 && fileIndex < FILE_LINE_MAX) {
        memset(lineContent, 0, LINE_CONTENT_MAX_LEN);
        fgets(lineContent, LINE_CONTENT_MAX_LEN, fp);
        if (lineContent[0] == '\r' || lineContent[0] == '\0' ||
            lineContent[0] == '\n' || strlen(lineContent) < 1) {
            fileIndex++;
            continue;
        }
    ...
    if (fileIndex >= FILE_LINE_MAX)
        printf("file line reach end of arrary, please check!!, line content is %s\n", lineContent);
