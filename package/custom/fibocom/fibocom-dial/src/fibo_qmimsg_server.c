...
static void fibo_get_driver_rmnet_info(char *ifname, RMNET_INFO *rmnet_info) {
    int ifc_ctl_sock;
    struct ifreq ifr;
    int rc;
    int request = 0x89F3;
...
static void fibo_set_driver_qmap_setting(char *ifname, QMAP_SETTING *qmap_settings) {
    int ifc_ctl_sock;
    struct ifreq ifr;
    int rc;
    int request = 0x89F2;
...
static int qmi_proxy_init(char *qmi_device, char* getidproduct) {
    ...
    else if (!access("/sys/class/net/usb0", F_OK))
        strcpy(ifname, "usb0");
    else if (!access("/sys/class/net/wwan0", F_OK))
        strcpy(ifname, "wwan0");
    ...
    if(!strncasecmp(getidproduct, "0104", 4)|| !strncasecmp(getidproduct, "1001", 4) || !strncasecmp(getidproduct, "9025", 4))
        ...
        iface_id = 0x02;
    else if(!strncasecmp(getidproduct, "0112", 4) || !strncasecmp(getidproduct, "0116", 4) || !strncasecmp(getidproduct, "0117", 4))
        iface_id = 0x00;
    ...
    fibo_get_driver_rmnet_info(ifname, &rmnet_info);
    if (rmnet_info.size) {
...
//Uplink (UL) data aggregation protocol to be used for uplink data transfer.
    ...
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationProtocolTlv.Value = cpu_to_le32(qmap_version); //UL QMAP is enabled
//Downlink (DL) data aggregation protocol to be used for downlink data transfer
    ...
    pQMI->MUXMsg.SetDataFormatReq.DownlinkDataAggregationProtocolTlv.Value = cpu_to_le32(qmap_version); //UL QMAP is enabled
...
//Maximum number of datagrams in a single aggregated packet on uplink
    ...
    pQMI->MUXMsg.SetDataFormatReq.DlMinimumPassingTlv.TLVLength = cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.DlMinimumPassingTlv.Value = cpu_to_le32(qmap_settings.dl_minimum_padding);
        
//Maximum number of datagrams in a single aggregated packet on uplink
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.TLVType = 0x1B; 
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.TLVLength = cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.Value = cpu_to_le32(qmap_settings.ul_data_aggregation_max_datagrams);
        
//Maximum size in bytes of a single aggregated packet allowed on downlink
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.TLVType = 0x1C; 
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.TLVLength = cpu_to_le16(4);
    pQMI->MUXMsg.SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.Value = cpu_to_le32(qmap_settings.ul_data_aggregation_max_size);
    ...
    if (pRsp) {
        ...
        if (qmap_settings.ul_data_aggregation_max_datagrams > 1) {
            fibo_set_driver_qmap_setting(ifname, &qmap_settings);
        }
...
static void qmidevice_detect(char **device_name, char **idproduct) {
    ...
    if (pDir)  {
        ...
        else if((!strncasecmp(idVendor, "2cb7", 4) && !strncasecmp(idProduct, "0109", 4)) || (!strncasecmp(idVendor, "1508", 4) && !strncasecmp(idProduct, "1000", 4))|| ((!strncasecmp(idVendor, "05c6", 4)) && (!strncasecmp(idProduct, "90B8", 4))))
        {
        ...
        else if((!strncasecmp(idVendor, "2cb7", 4) && !strncasecmp(idProduct, "0112", 4))
             || (!strncasecmp(idVendor, "2cb7", 4) && !strncasecmp(idProduct, "0116", 4))
             || (!strncasecmp(idVendor, "2cb7", 4) && !strncasecmp(idProduct, "0117", 4)))
        {
        ...
        strncpy(idproduct,idProduct, sizeof(idProduct)-1);

        idproduct = strdup(idProduct);
