...
re_write:
    if (pollfds[0].revents & POLLOUT)
    {
        ...
        if (ret == nwrites)
        {
            ret = 0;
            write_count = 0;
            dbg_time("%s write successfully ret=%d, nwrites:%d", __func__, ret,nwrites);
        }
        else
        {
            write_count ++;
            dbg_time("%s write=%d, errno: %d (%s),write_count:%d", __func__, ret, errno,
                strerror(errno),write_count);
            if(write_count < 5)
            {
                sleep(1);
                goto re_write;
...
static int QmiWwanGetClientID(UCHAR QMIType)
{
	...
	if (!QMUXResult && !QMUXError &&
		...
		break;
	    case QMUX_TYPE_M2M:
		dbg_time("Get clientM2M = %d", ClientId);
		break;
...
static int QmiWwanInit(PROFILE_T *profile)
{
    ...
    qmiclientId[QMUX_TYPE_M2M] = QmiWwanGetClientID(QMUX_TYPE_M2M);
    if (profile->qmap_mode == 0 || profile->qmap_mode == 1)
    ...
    while (1)
    {
        ...
        for (ne = 0; ne < nevents; ne++)
        {
            ...
            if (revents & (POLLERR | POLLHUP | POLLNVAL))
            {
                ...
                if (revents &(POLLHUP | POLLNVAL))  // bug, Can get POLLERR
            ...
            re_read:
                ...
                {
                    read_count ++;
                    dbg_time("%s read=%d errno: %d (%s),read_count:%d", __func__, (int)nreads,
                    errno, strerror(errno),read_count);
                    if(read_count < 5)
                    {
                        sleep(1);
                        goto re_read;
