...
void fibo_set_driver_qmap_setting(PROFILE_T *profile, QMAP_SETTING *qmap_settings) {
    int ifc_ctl_sock;
    struct ifreq ifr;
    int rc;
    int request = 0x89F2;
...
static USHORT WdaSetDataFormat(PQMUX_MSG pMUXMsg, void *arg)
{
	...
	pMUXMsg->SetDataFormatReq.UplinkDataAggregationProtocolTlv.Value =
	    cpu_to_le32(qmap_version);	// UL QMAP is enabled
...
static USHORT fibo_m2mSetProfileReq(PQMUX_MSG pMUXMsg, void *arg)
{
    USHORT TLVLength = 0;
    UCHAR *pTLV;
    PROFILE_T *profile = (PROFILE_T *)arg;
    PFIBO_PDPTYPE pPdpType;
    

    pMUXMsg->setProfileReq.Length =
    cpu_to_le16(sizeof(FIBO_SET_PROFILE_REQ_MSG) - 4);
    pMUXMsg->setProfileReq.TLVType = 0x01;
    pMUXMsg->setProfileReq.TLVLength = cpu_to_le16(0x01);
    pMUXMsg->setProfileReq.ProfileIndex = (UCHAR)profile->pdpindex;

    pTLV = (UCHAR *)(&pMUXMsg->setProfileReq + 1);

    pPdpType = (PFIBO_PDPTYPE)(pTLV + TLVLength);
    pPdpType->TLVType = 0x10;
    pPdpType->TLVLength = cpu_to_le16(0x04);
    // 0 ?C PDP-IP (IPv4)
    // 1 ?C PDP-PPP
    // 2 ?C PDP-IPv6
    // 3 ?C PDP-IPv4v6
    ...
    return sizeof(FIBO_SET_PROFILE_REQ_MSG) + TLVLength;
}
...
re_send_qmi:
    ...
    if (ret == 0)
    {
        ...
        {
            send_count ++;
            dbg_time("%s pthread_cond_timeout_np=%d, errno: %d (%s), send_count:%d.", __func__,
                ret, errno, strerror(errno),send_count);
            if(send_count < 3)
            {
                sleep(1);
...
{
    return QmiThreadSendQMITimeout(pRequest, ppResponse, 10 * 1000);
}
...
//2021-02-08 zhangkaibo@fibocom.com changed end for mantis 0070613
        ...
        if(!qmidev_is_qmiwwan(profile->qmichannel)){
            ...
            qmap_settings.ul_data_aggregation_max_datagrams = 0;
        }
        ...
        if (qmidev_is_gobinet(profile->qmichannel)) {    // GobiNet set data format
            // in GobiNet driver
            goto skip_WdaSetDataFormat;
         } else if (profile->qmap_mode > 1) {  // QMAP MUX enabled, set data format in fibo-qmi-proxy
    ...
    if(!qmidev_is_qmiwwan(profile->qmichannel)){
        if (qmap_settings.ul_data_aggregation_max_datagrams)
        {
            ...
            if (qmap_settings.ul_data_aggregation_max_datagrams > 1) {
                fibo_set_driver_qmap_setting(profile, &qmap_settings);
            }
        }
    }
...
int requestSetProfile_by_m2m(PROFILE_T *profile,int product)
{
    PQCQMIMSG pRequest;
    PQCQMIMSG pResponse;
    PQMUX_MSG pMUXMsg;
    ...
    UCHAR QMIType = QMUX_TYPE_M2M;

    if(product == 1){
        dbg_time("FG131\n");
        pRequest = ComposeQMUXMsg(QMIType, QMI_FIBO_WDS_X35_SET_PROFILE_INFO_RESP_V01,
                   fibo_m2mSetProfileReq, profile);
    }
    else{
        dbg_time("FG132\n");
        pRequest = ComposeQMUXMsg(QMIType, FIBO_MODIFY_PROFILE_SETTINGS_REQ,
                   fibo_m2mSetProfileReq, profile);
    }
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check();
...
int requestGetSignalInfo(void)
{
    ...
    // 5G
    {
        PQMINAS_SIG_INFO_5G_TLV_MSG ptlv = (PQMINAS_SIG_INFO_5G_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x17);
        if (ptlv && ptlv->TLVLength)
        {
            dbg_time("%s 5G: RSCP %d dBm", __func__, ptlv->rsrp);
        }
    }
...
int requestBaseBandVersion(const char **pp_reversion)
{
	...
	*pp_reversion = NULL;
    ...
    if (revId && le16_to_cpu(revId->TLVLength)) {
        ...
        if(strstr(DeviceRevisionID,"89170")){
            return 1;
        }
    }
