...
char packet_buffer[TCP_BUFFER_SIZE] = {0};
...
int connect_to_server() {
    ...
    char *SERVER_IP = order("uci -q get nport.@ttyS2[0].serverip");
    int SERVER_PORT = function("uci -q get nport.@ttyS2[0].serverport");
...
void add_to_packet(const char *line) {
    ...
    pthread_mutex_lock(&data_mutex);
    size_t free_space = TCP_BUFFER_SIZE - strlen(packet_buffer) - 1;
...
void send_packet() {
    ...
    if (sockfd == -1) {
        if (!reconnect_server()) {
            printf("服务器连接失败，丢弃当前数据包\n");
            memset(packet_buffer, 0, TCP_BUFFER_SIZE);
    ...
    ssize_t send_len = send(sockfd, packet_buffer, strlen(packet_buffer), 0);
    if (send_len < 0) {
        perror("发送数据包失败");
        close(sockfd);
        sockfd = -1;
        ...
        printf("[%s] 第%d次发送TCP数据包，类型: %s，大小: %zu字节，设备ID: %s，包含 %zu 条NMEA语句\n",
               time_str, packet_count, current_packet_type, strlen(packet_buffer), device_id,
                ...
                if (buffer[i] == '\n' || line_len >= SERIAL_BUFFER_SIZE - 1) {
                    ...
                    if (strlen(line_buffer) > 10) {
                        ...
                        if (is_nmea_start(line_buffer)) {
                            ...
                            // 开始新数据包
                            snprintf(packet_buffer, TCP_BUFFER_SIZE, "$ID:%s\r\n", device_id);
                            strncat(packet_buffer, line_buffer, 
                                    TCP_BUFFER_SIZE - strlen(packet_buffer) - 1);
...
void *receive_thread(void *arg) {
    ...
    while (running) {
        ...
        bytes_read = recv(sockfd, buffer, TCP_BUFFER_SIZE - 1, 0);
        if (bytes_read > 0) {
            buffer[bytes_read] = '\0';
            printf("收到服务器响应: %s\n", buffer);
        } 
...
{
	SERIAL_PORT=argv[1];
    pthread_t serial_thread, upload_thread_id, hb_thread, recv_thread;
    ...
    printf("GNSS带ID数据包分割上传程序启动（TCP版本）- RMC优先级高于GGA\n");
    printf("配置: 串口=%s, 超时上传间隔=%d秒, 发送间隔=%d秒, 缓冲区大小=%d字节\n",
           SERIAL_PORT, UPLOAD_INTERVAL, SEND_INTERVAL, TCP_BUFFER_SIZE);
    ...
    if (pthread_create(&hb_thread, NULL, heartbeat_thread, NULL) != 0) {
        perror("创建心跳线程失败");
        pthread_cancel(serial_thread);
        pthread_cancel(upload_thread_id);
        return 1;
    }
