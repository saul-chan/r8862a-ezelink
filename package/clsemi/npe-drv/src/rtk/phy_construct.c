...
rtl8214fc_basicPhy_config(uint32 unit, uint8 phy_base_macId)
{
	...
	for (port_id = 0; port_id < PORT_NUM_IN_8214FC; port_id++)
	{
		...
		RTK_MII_WRITE(unit, port, HAL_MIIM_FIX_PAGE, 0x4, 0xde1);
		RTK_MII_WRITE(unit, port, HAL_MIIM_FIX_PAGE, 0x9, 0xe00);
		RTK_MII_WRITE(unit, port, HAL_MIIM_FIX_PAGE, 0x0, 0x9140);
	}
#else 
...
_rtl8218d_phy_config(uint32 unit, uint8 phy_base_macId)
{
   ...
   // printf("chip_id = %d, chip_rev_id=%d\n", chip_id, chip_rev_id);
