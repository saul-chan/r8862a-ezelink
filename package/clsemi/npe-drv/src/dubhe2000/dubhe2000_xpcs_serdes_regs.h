#ifndef __DUBHE1000_XPCS_SERDES_REGS_H__
#define __DUBHE1000_XPCS_SERDES_REGS_H__
...
#define SR_XS_PCS_STS2	  ((0x8 * 4) + SR_XS_PCS_BASE)
...
#define SR_PMA_CTRL1		(0x10000 * 0x4)
...
#define SUPPRESS_LOS_DET_BEGIN		       4
#define SUPPRESS_LOS_DET_END		       4
...
#define CREATE_SGMII_FFE_SERDES_CONFIG_(name) {                                                                        \
	...
	XPCS_FIELD(VR_XS_PCS_DEBUG_CTRL, SUPPRESS_LOS_DET, 0),                                                         \
...
#define DO_SERDES_CONFIG_(xpcs_priv, id, name, mode, ...) do {                                                         \
	int i = 0;                                                                                                     \
	u32 tmp;                                                                                                       \
	struct serdes_reg_ext_st serdes_config[] = CREATE_SERDES_CONFIG(id, mode, ##__VA_ARGS__);                      \
	struct serdes_reg_ext_st serdes_ffe[] = CREATE_SERDES_CONFIG(id, mode##_FFE);                                  \
	for (i = 0; i < sizeof(serdes_config) / sizeof(*serdes_config); i++) {                                         \
		xpcs_write_ext(xpcs_priv, serdes_config[i].reg, serdes_config[i].field_end,                            \
			       serdes_config[i].field_begin, serdes_config[i].value);                                  \
	}                                                                                                              \
	xpcs_write_ext(xpcs_priv, VR_XS_PCS_DIG_CTRL1, VR_RST_END, VR_RST_BEGIN, 1);                                   \
	dubhe1000_serdes_load_and_modify(xpcs_priv);                                                                   \
	if (readl_poll_timeout(xpcs_priv->ioaddr + VR_XS_PCS_DIG_CTRL1, tmp, !(tmp & BIT(15)), 10, 50000)) {           \
		pr_info("Line %d: wait status[%d] timeout", __LINE__, tmp);                                            \
	}                                                                                                              \
	for (i = 0; i < sizeof(serdes_ffe) / sizeof(*serdes_ffe); i++) {                                               \
		xpcs_write_ext(xpcs_priv, serdes_ffe[i].reg, serdes_ffe[i].field_end,                                  \
			       serdes_ffe[i].field_begin, serdes_ffe[i].value);                                        \
	}                                                                                                              \
} while (0)
