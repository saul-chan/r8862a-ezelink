...
struct cls_npu_config cls_dut_npu_cfg;
...
int main(int argc, char **argv)
{
	...
	memset(&cls_dut_npu_cfg, 0, sizeof(cls_dut_npu_cfg));
	...
	while ((opt = getopt(argc, argv, "hi:p:l:b:m:n:")) != -1) {
		...
		case 'b':
			if (strlen(optarg) < sizeof(cls_dut_npu_cfg.br_ipaddr))
				strcpy((char *)cls_dut_npu_cfg.br_ipaddr, optarg);
			else {
		...
		case 'm':
			{
				...
				if (!ether_aton_r((char *)al_macstr,
					(struct ether_addr *)cls_dut_npu_cfg.al_macaddr)) {
		...
		case 'n':
			sscanf(optarg, "%d", &cls_dut_npu_cfg.npu_topology);
			if (cls_dut_npu_cfg.npu_topology) {
				...
				strcpy(cls_dut_npu_cfg.ssh_cli, "ssh -i /.ssh/id_rsa root@");
			}
			break;
		}
	}
	...
	int error;
	//int error = clsapi_init();
	//if (error < 0) {
	//	fprintf(stderr, "can't init CLSAPI, error %d", error);
	//	return error;
	//}
