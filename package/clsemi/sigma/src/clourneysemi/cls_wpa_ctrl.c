			...
			size_t buf_size)
{
	...
	if (!mon) {
		cls_error("Invalid WPA ctrl monitor");
		return -1;
	}

	fd = wpa_ctrl_get_fd(mon);
	if (fd < 0) {
		cls_error("get_wpa_cli: invalid fd monitor");
		return -1;
	}
	...
	while (1) {
		...
		if (ret == 0) {
			cls_error("Timeout on waiting for events");
			return -1;
		}
		if (ret < 0) {
			cls_error("select: %s", strerror(errno));
			return -1;
		}
		len = buf_size;
		if (wpa_ctrl_recv(mon, buf, &len) < 0) {
			cls_error("Failure while waiting for events");
			return -1;
		}
		if (len == buf_size)
			len--;
		...
		time(&now);
		if ((unsigned int) (now - start) > timeout) {
			cls_error("Timeout on waiting for event");
			return -1;
		}
	}
}
...
static int cls_parse_hex(char c)
{
	if (c >= '0' && c <= '9')
		return c - '0';
...
static int cls_hex_byte(const char *str)
{
	int res1, res2;

	res1 = cls_parse_hex(str[0]);
	if (res1 < 0)
		return -1;
	res2 = cls_parse_hex(str[1]);
	if (res2 < 0)
		return -1;
	return (res1 << 4) | res2;
}
...
int cls_hexstr_to_ascii(const char *hex, char *buf, size_t buflen)
{
	size_t i;
	const char *pos = hex;
...
void cls_ascii_to_hexstr(const char *str, char *hex)
{
	int i, length;
