...
int wfaTrafficAgentSendResp(BYTE *cmdBuf)
{
    ...
    else 
    {
   	...
   	strncat(gRespStr, ",txFrames,", sizeof(gRespStr)-1);
   	for(i=0; i<numStreams; i++) 
        {
      	...
      	strncat(gRespStr, ",rxFrames,", sizeof(gRespStr)-1);
       	for(i=0; i<numStreams; i++) 
        {
       	...
       	strncat(gRespStr, ",txPayloadBytes,", sizeof(gRespStr)-1);
       	for(i=0; i<numStreams; i++) 
        {
        ...
        strncat(gRespStr, ",rxPayloadBytes,", sizeof(gRespStr)-1);
        for(i=0; i<numStreams; i++) 
        {
       	    sprintf(copyBuf, " %llu ", statResp[i].cmdru.stats.rxPayloadBytes);
            strncat(gRespStr, copyBuf, sizeof(gRespStr)-1);
        }
        strncat(gRespStr, ",outOfSequenceFrames,", sizeof(gRespStr)-1);
        for(i=0; i<numStreams; i++) 
        {
...
int wfaDeviceListIFResp(BYTE *cmdBuf)
{
    ...
    switch(devListIfResp->status)
    {
       ...
       for(i=0; i<1; i++)
       {
         ...
         {
            strncat(gRespStr,",", 4);
            strncat(gRespStr, ifResp->ifs[i], sizeof(gRespStr) - 1);
            strncat(gRespStr, "\r\n", 4);
         }
       }
