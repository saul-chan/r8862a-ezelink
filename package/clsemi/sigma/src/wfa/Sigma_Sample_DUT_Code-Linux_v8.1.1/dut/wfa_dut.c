...
void cls_handle_dut_req(int xcCmdTag, int len, unsigned char *params, int *respLen, BYTE *respBuf);
...
wfa_dut_main(int argc, char **argv)
{
    ...
    while (isExit) 
    {
        ...
        if(gxcSockfd >= 0 && FD_ISSET(gxcSockfd, &sockSet)) 
        {
	       ...
	       {
	       		cls_handle_dut_req(xcCmdTag, cmdLen, parmsVal, &respLen, (BYTE *)respBuf);
	       }
