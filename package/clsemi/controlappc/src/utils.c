...
extern char bssid_24g[32];
extern char bssid_5g[32];
...
static void loopback_server_receive_message(int sock, void *eloop_ctx, void *sock_ctx) {
    ...
    indigo_logger(LOG_LEVEL_INFO, "Loopback server received length = %d, ip=%s", len, inet_ntop(AF_INET, &(((struct sockaddr_in *) &from)->sin_addr), s, 64));
...
static void cls_prepare_vif_for_owetrans(int band, char *owe_trans_bssid)
{
    char *base_bssid = NULL;
    char *cursor = NULL;

    if (band == BAND_5GHZ)
        base_bssid  = bssid_5g;
    else
        base_bssid = bssid_24g;

    cursor = strchr(base_bssid, ':');
    if (!cursor) {
        indigo_logger(LOG_LEVEL_WARNING, "wrong format for bssid\n");
        return;
    }

    sprintf(owe_trans_bssid, "%s%s", "d2", cursor);
    return;
}

struct interface_info* cls_add_wif_for_owetrans(struct bss_identifier_info *bss) {
    int idx = interface_count;
    char tmp_name[16] = {0};
    int i;

    indigo_logger(LOG_LEVEL_INFO, "add new vap(identifier=%d) to comfort owetrans\n", bss->identifier);

    if (idx) {
        cls_prepare_vif_for_owetrans(bss->band, interfaces[idx].bssid);
        sprintf(tmp_name, "wlan%d-1", bss->band);
        strcpy(interfaces[idx].ifname, tmp_name);
        interfaces[idx].band = bss->band;
        interfaces[idx].identifier = bss->identifier;
        interfaces[idx].mbssid_enable = bss->mbssid_enable;
        interfaces[idx].transmitter = bss->transmitter;
        interfaces[idx].hapd_bss_id = band_mbssid_cnt[bss->band];
        memset(interfaces[idx].hapd_conf_file, 0, sizeof(interfaces[idx].hapd_conf_file));
        for(i = 0; i < configured_interface_count; i++) { /* multi VAPs on same band share one hostapd conf */
            if (interfaces[i].identifier != UNUSED_IDENTIFIER && interfaces[i].band == bss->band) {
                snprintf(interfaces[idx].hapd_conf_file, sizeof(interfaces[idx].hapd_conf_file),
                         "%s", interfaces[i].hapd_conf_file);
                break;
            }
        }
        band_mbssid_cnt[bss->band]++;
...
int add_wireless_interface_info(int band, int bssid, char *name) {
    ...
    memset(interfaces[interface_count].bssid, 0, 32);
...
void cls_interfaces_init(void)
{
    int i;

    for (i = 0; i < interface_count; i++) {
        if (interfaces[i].band == BAND_5GHZ)
            strncpy(interfaces[i].bssid, bssid_5g, 32);
        if (interfaces[i].band == BAND_24GHZ)
            strncpy(interfaces[i].bssid, bssid_24g, 32);
    }
}
