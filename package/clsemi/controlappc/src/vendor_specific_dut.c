...
char bssid_5g[32] = {0};
char bssid_24g[32] = {0};
void cls_get_local_bssid(void)
{
    FILE *wlan0_file, *wlan1_file;
    char *pos = NULL;

    wlan0_file = popen("cat /sys/devices/platform/soc/800000.wpu_b/ieee80211/phy0/macaddress", "r");
    wlan1_file = popen("cat /sys/devices/platform/soc/800000.wpu_b/ieee80211/phy1/macaddress", "r");

    if (!wlan0_file || !wlan1_file) {
        indigo_logger(LOG_LEVEL_WARNING, "can NOT open mac file");
        goto out;
    }
    if (!fgets(bssid_24g, sizeof(bssid_24g), wlan0_file)) {
        indigo_logger(LOG_LEVEL_WARNING, "can NOT get mac of 2.4G");
        goto out;
    }
    if (!fgets(bssid_5g, sizeof(bssid_5g), wlan1_file)) {
        indigo_logger(LOG_LEVEL_WARNING, "can NOT get mac of 5G");
        goto out;
    }
    ...
    indigo_logger(LOG_LEVEL_INFO, "get D2K==BSSID[2g]=%s, BSSID[5g]=%s\n ", bssid_24g, bssid_5g);
...
void cls_wifi_interface_cleanup(void)
{
    system("ubus call hostapd config_remove '{\"iface\":\"wlan0\"}' >/dev/null 2>/dev/null");
    system("ip link set dev wlan0 down >/dev/null 2>/dev/null");
    system("iw dev wlan0 del >/dev/null 2>/dev/null");
    sleep(3);
    system("ubus call hostapd config_remove '{\"iface\":\"wlan1\"}' >/dev/null 2>/dev/null");
    system("ip link set dev wlan1 down >/dev/null 2>/dev/null");
    system("iw dev wlan1 del >/dev/null 2>/dev/null");
    sleep(3);
    system("ubus call hostapd config_remove '{\"iface\":\"wlan1-1\"}' >/dev/null 2>/dev/null");
    system("ip link set dev wlan1-1 down >/dev/null 2>/dev/null");
    system("iw dev wlan1-1 del >/dev/null 2>/dev/null");
    sleep(3);
}

void cls_wifi_interface_creation(void)
{
    system("iw phy phy0 interface add wlan0 type __ap");
    system("ip link set dev wlan0 up");
    sleep(2);
    system("iw phy phy1 interface add wlan1 type __ap");
    system("ip link set dev wlan1 up");
    sleep(2);
    system("iw phy phy%d interface add wlan1-1 type __ap");
    system("ip link set dev wlan1-1 up");
    sleep(2);
}

void cls_hostapd_file_cleanup(void)
{
    system("rm -f /tmp/run/hostapd-phy*.*");
}
