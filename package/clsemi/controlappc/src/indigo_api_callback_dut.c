...
static int generate_hostapd_config(char *output, int output_size, struct packet_wrapper *wrapper, struct interface_info* wlanp) {
    ...
    for (i = 0; i < wrapper->tlv_num; i++) {
        ...
        if (tlv->id == TLV_OWE_TRANSITION_BSS_IDENTIFIER) {
            ...
            if (!wlan)
                wlan = cls_add_wif_for_owetrans(&bss_info);
    ...
    if (has_sae && has_sae_groups == 0) {
        strcat(output, "sae_groups=19 20 21\n");
    }
    ...
    //add anqp neighbor report
    strcat(output, "anqp_elem=272:3410FFFFFFFFFFFF1806B3BA747324\n");

    // add bssid of management vap of 2.4/5G
    sprintf(cfg_item, "bssid=%s\n", wlanp->bssid);
    strcat(output, cfg_item);

    strcat(output, "utf8_ssid=1\n");
...
static int start_ap_handler(struct packet_wrapper *req, struct packet_wrapper *resp) {
    ...
    cls_wifi_interface_creation();

    cls_hostapd_file_cleanup();
...
static int get_mac_addr_handler(struct packet_wrapper *req, struct packet_wrapper *resp) {
    ...
    if (-1 == bss_info.identifier) {
        if (bss_info.band == BAND_24GHZ) {
            sprintf(mac_addr, "%s", bssid_24g);
        } else
            sprintf(mac_addr, "%s", bssid_5g);
    } else {
        wlan = get_wireless_interface_info(bss_info.band, bss_info.identifier);
        if (wlan) {
            sprintf(mac_addr, "%s", wlan->bssid);
        } else {
            indigo_logger(LOG_LEVEL_ERROR, "can NOT get interface_info for bss_identity=%d\n", bss_identifier);
...
static void timer2disable(void *eloop_ctx, void *timeout_ctx) {
    struct wpa_ctrl *w = NULL;
    char *request = "DISABLE";
    char response[4096];
    size_t resp_len;

    indigo_logger(LOG_LEVEL_INFO, "Time to disable bss");
    w = wpa_ctrl_open(get_hapd_ctrl_path());
    if (!w) {
        indigo_logger(LOG_LEVEL_ERROR, "Failed to connect to hostapd");
        return;
...
static int send_ap_btm_handler(struct packet_wrapper *req, struct packet_wrapper *resp) {
    ...
    message = TLV_VALUE_OK;
    if (strlen(bss_term_bit) && strlen(bss_term_tsf) && strlen(bss_term_duration) ) {
        indigo_logger(LOG_LEVEL_INFO, "Scheduled timer to disable bss");
        eloop_register_timeout(1, 0, timer2disable, NULL, NULL);
    }
done:
    fill_wrapper_message_hdr(resp, API_CMD_RESPONSE, req->hdr.seq);
    fill_wrapper_tlv_byte(resp, TLV_STATUS, status);
