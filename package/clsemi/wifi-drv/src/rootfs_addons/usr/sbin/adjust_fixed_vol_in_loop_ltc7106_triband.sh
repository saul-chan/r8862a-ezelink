...
usage() {
	...
	printf "\n\t-S ==> Select Voltage BUS with Mask 'V1V2_DCDC V0V8_DCDC V1V8_DCDC' 0x7"
	printf "\n\r"
	exit 1
}
...
V1V2_DCDC_Current_no=0

DEBUG_INFO_PARA=""

V0V8_DCDC_Fixed_VolArray="0.7876  0.8290  0.8705"
V1V2_DCDC_Fixed_VolArray="1.1400  1.2000  1.2600"
V1V8_DCDC_Fixed_VolArray="1.7385  1.8300  1.9215"

V0V8_DCDC_SELECT_MASK=0x01
V1V2_DCDC_SELECT_MASK=0x02
V1V8_DCDC_SELECT_MASK=0x04
VOLTAGE_BUS_SELECT_MASK=0x7

V0V8_DCDC_Current_Value="0.8290"
V1V2_DCDC_Current_Value="1.2000"
V1V8_DCDC_Current_Value="1.8300"

V0V8_DCDC_CPLD_Value="0.8290"
V1V2_DCDC_CPLD_Value="1.2000"
V1V8_DCDC_CPLD_Value="1.8300"
...
Get_Bus_Current_Voltag_Value() {
	VoltageBus=$1
        case $VoltageBus in
                "V0V8_DCDC" )
			echo $(awk 'BEGIN{printf "%.4f\n", ('$(ina226_triband.sh -U 0 -R 5 |  awk '{print $2}')') /  '1000'}')
                        ;;
                "V1V2_DCDC" )
			echo $(awk 'BEGIN{printf "%.4f\n", ('$(ina226_triband.sh -U 2 -R 5 |  awk '{print $2}')') /  '1000'}')
                        ;;
                "V1V8_DCDC" )
			echo $(awk 'BEGIN{printf "%.4f\n", ('$(ina226_triband.sh -U 1 -R 5 |  awk '{print $2}')') /  '1000'}')
                        ;;
...
Get_Bus_CPLD_Set_Voltag_Value() {
        ...
        case $VoltageBus in
                "V0V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_ltc7106_triband.sh  -N V0V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V1V2_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_ltc7106_triband.sh  -N V1V2_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V1V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_ltc7106_triband.sh  -N V1V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                * ) echo "Invalid option passed to '$0' $VoltageBus"
                        usage;;
        esac
...
Get_All_Bus_Compensation_Value() {
        ...
        if [ $((V1V2_DCDC_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((V1V2_DCDC_SELECT_MASK)) ]; then
                # V1V2_DCDC 2 R_Shunt=5
                V1V2_DCDC_Current_Value=$(Get_Bus_Current_Voltag_Value V1V2_DCDC)
                V1V2_DCDC_CPLD_Value=$(Get_Bus_CPLD_Set_Voltag_Value V1V2_DCDC)
                V1V2_DCDC_Compensation_Value=$(awk 'BEGIN{printf "%.4f\n", '$V1V2_DCDC_CPLD_Value' - '$V1V2_DCDC_Current_Value'}')
                #echo "V1V2_DCDC_Compensation_Value $V1V2_DCDC_Compensation_Value"
        fi

        if [ $((V0V8_DCDC_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((V0V8_DCDC_SELECT_MASK)) ]; then
                # V0V8_DCDC 0 R_Shunt=5
...
Auto_Compensation_Bus_Vol_By_Step() {
	...
	if [ `echo "$Bus_Voltage_Set_Value > $Bus_Voltage_Current_Value" |bc` -eq 1 ] ; then
		...
		do
			...
			adjust_vol_ltc7106_triband.sh -N $VoltageBus -V $Set_Compensation_Vol_Value
			usleep 800000
...
Set_All_Bus_High_Vol() {
	...
	if [ $((V1V2_DCDC_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((V1V2_DCDC_SELECT_MASK)) ]; then
		Set_High_Vol_Value=$(echo $V1V2_DCDC_Fixed_VolArray | awk '{print $2}')
		Set_High_Vol_Value=$(awk 'BEGIN{printf "%.4f\n", '$Set_High_Vol_Value' * ('1' + '$Voltage_Adjust_Percentage')}')
                Get_Current_Value=$(Get_Bus_Current_Voltag_Value V1V2_DCDC)
                Auto_Compensation_Bus_Vol_By_Step V1V2_DCDC $Set_High_Vol_Value $Get_Current_Value $LTC7106_1V2_Unit $V1V2_DCDC_Compensation_Value
	fi
}
...
Set_All_Bus_Low_Vol() {
	...
	if [ $((V1V2_DCDC_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((V1V2_DCDC_SELECT_MASK)) ]; then
		Set_Low_Vol_Value=$(echo $V1V2_DCDC_Fixed_VolArray | awk '{print $2}')
		Set_Low_Vol_Value=$(awk 'BEGIN{printf "%.4f\n", '$Set_Low_Vol_Value' * ('1' - '$Voltage_Adjust_Percentage')}')
                Get_Current_Value=$(Get_Bus_Current_Voltag_Value V1V2_DCDC)
                Auto_Compensation_Bus_Vol_By_Step V1V2_DCDC $Set_Low_Vol_Value $Get_Current_Value $LTC7106_1V2_Unit $V1V2_DCDC_Compensation_Value
	fi
}
...
Set_VoltageBus_Random_Vol() {
	...
	case $VoltageBus in
			...
			VoltageCompensationValue=$V0V8_DCDC_Compensation_Value
                        ;;
                "V1V2_DCDC" )
			setVoltage=$(echo $V1V2_DCDC_Fixed_VolArray | awk '{print $'$((delta + 1))' }')
			V1V2_DCDC_Current_no=$delta
			VoltageUnitVal=$LTC7106_1V2_Unit
			VoltageCompensationValue=$V1V2_DCDC_Compensation_Value
                        ;;
                "V1V8_DCDC" )
...
Set_All_Bus_Random_Vol() {
	...
	if [ $((V1V2_DCDC_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((V1V2_DCDC_SELECT_MASK)) ]; then
		Rnd=$(Rand 0 2)
		Set_VoltageBus_Random_Vol $V1V2_DCDC_Current_no $Rnd  "V1V2_DCDC"
	fi
}
