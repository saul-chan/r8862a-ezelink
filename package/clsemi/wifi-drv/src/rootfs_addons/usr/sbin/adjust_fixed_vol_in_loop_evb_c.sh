...
Get_Bus_Current_Voltag_Value() {
	VoltageBus=$1
        case $VoltageBus in
                "V0V8_DCDC" )
			echo $(awk 'BEGIN{printf "%.4f\n", ('$(ina226.sh -U 1 -R 5 |  awk '{print $2}')') /  '1000'}')
                        ;;
                "DDR_VDDQ" )
			echo $(awk 'BEGIN{printf "%.4f\n", ('$(ina226.sh -U 0 -R 10 |  awk '{print $2}')') /  '1000'}')
                        ;;
                "V1V8_DCDC" )
...
Get_Bus_CPLD_Set_Voltag_Value() {
        ...
        case $VoltageBus in
                "V0V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_evb_c.sh  -N V0V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "DDR_VDDQ" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_evb_c.sh  -N DDR_VDDQ -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V1V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol_evb_c.sh  -N V1V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                * ) echo "Invalid option passed to '$0' $VoltageBus"
                        usage;;
        esac
...
Auto_Compensation_Bus_Vol_By_Step() {
	...
	if [ `echo "$Bus_Voltage_Set_Value > $Bus_Voltage_Current_Value" |bc` -eq 1 ] ; then
		...
		do
			...
			adjust_vol_evb_c.sh -N $VoltageBus -V $Set_Compensation_Vol_Value
			usleep 500000
