...
Get_Bus_CPLD_Set_Voltag_Value() {
        ...
        case $VoltageBus in
                "AVDD18_VCO" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N AVDD18_VCO -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "AVDD18_CKI" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N AVDD18_CKI -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "AVDD12_ABB" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N AVDD12_ABB -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "AVDD18_RF" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N AVDD18_RF -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V0V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N V0V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "DDR_VDDQ" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N DDR_VDDQ -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V3V3_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N V3V3_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                "V1V8_DCDC" )
                        echo $(awk 'BEGIN{printf "%.4f\n", ('$(adjust_vol.sh  -N V1V8_DCDC -R yes | awk '{print $3}')') /  '1000'}')
                        ;;
                * ) echo "Invalid option passed to '$0' $VoltageBus"
                        usage;;
        esac
...
Auto_Compensation_Bus_Vol_By_Step() {
	...
	if [ `echo "$Bus_Voltage_Set_Value > $Bus_Voltage_Current_Value" |bc` -eq 1 ] ; then
		...
		do
			...
			adjust_vol.sh -N $VoltageBus -V $Set_Compensation_Vol_Value
			usleep 400000
...
Set_All_Bus_High_Vol() {
	...
	if [ $((DDR_VDDQ_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((DDR_VDDQ_SELECT_MASK)) ]; then
		...
		Set_High_Vol_Value=$(awk 'BEGIN{printf "%.4f\n", '$Set_High_Vol_Value' * ('1' + '$Voltage_Adjust_Percentage')}')
                ...
                Auto_Compensation_Bus_Vol_By_Step DDR_VDDQ $Set_High_Vol_Value $Get_Current_Value $ISP1220_1V2_Unit $DDR_VDDQ_Compensation_Value
	fi
...
Set_All_Bus_Low_Vol() {
	...
	if [ $((DDR_VDDQ_SELECT_MASK & VOLTAGE_BUS_SELECT_MASK)) -eq $((DDR_VDDQ_SELECT_MASK)) ]; then
		...
		Set_Low_Vol_Value=$(awk 'BEGIN{printf "%.4f\n", '$Set_Low_Vol_Value' * ('1' - '$Voltage_Adjust_Percentage')}')
                Get_Current_Value=$(Get_Bus_Current_Voltag_Value DDR_VDDQ)
                Auto_Compensation_Bus_Vol_By_Step DDR_VDDQ $Set_Low_Vol_Value $Get_Current_Value $ISP1220_1V2_Unit $DDR_VDDQ_Compensation_Value
	fi
