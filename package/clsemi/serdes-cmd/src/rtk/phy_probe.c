...
phy_driver_hook(uint32 unit, rtk_port_t port, rt_phyctrl_t **ppPhyctrl)
{
    ...
    *ppPhyctrl = NULL;
	...
	//printf("!!! PHY drv index =%d %d, \n",pPhyctrl->pPhydrv->phydrv_index, RT_PHYDRV_RTL8214FC_MP);
#if 1
	//printf("=====>!!!fPhypatch_db_init[%llx:%llx] pPhy_patchDb[%#x]\n",pPhyctrl->pPhydrv->fPhypatch_db_init, phy_common_unavail,pPhyctrl->pPhy_patchDb);
    if(MIIM_PHY_API_AVAILABLE(pPhyctrl->pPhydrv->fPhypatch_db_init)
			&& pPhyctrl->pPhydrv->fPhypatch_db_init(unit, port, &pPhyctrl->pPhy_patchDb) == RT_ERR_FAILED)
    {
        return RT_ERR_FAILED;
    }
#endif
