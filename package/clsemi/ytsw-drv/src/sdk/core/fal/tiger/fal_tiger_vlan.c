...
yt_ret_t  fal_tiger_vlan_protocol_based_vlan_group_set(yt_unit_t unit, uint8_t group_id, yt_vlan_protocol_key_t key)
{
    protocol_based_vlann_t  entry;
    cmm_err_t ret = CMM_ERR_OK;
    uint8_t l2_type_tmp = 0;

    if (CAL_SWCHIP_MODEL(unit) != YT_SW_MODEL_9218)
    {
        return CMM_ERR_NOT_SUPPORT;
    }

    if (group_id >= VLAN_PROTOCOl_MAX_NUM)
    {
        return CMM_ERR_INPUT;
    }

    if (key.l2_type == L2_ETHV2)
    {
        l2_type_tmp = L2_TYPE_BITMAP_ETHV2;
    }
    else if (key.l2_type == L2_ETHSAP)
    {
        l2_type_tmp = L2_TYPE_BITMAP_ETHSAP;
    }
    else if (key.l2_type == L2_ETHSNAP)
    {
        l2_type_tmp = L2_TYPE_BITMAP_ETHSNAP;
    }
    else if (key.l2_type >= L2_ETHMAX)
    {
        return CMM_ERR_EXCEED_RANGE;
    }

    CMM_ERR_CHK(HAL_TBL_REG_READ(unit, PROTOCOL_BASED_VLANNm, group_id, sizeof(protocol_based_vlann_t), &entry), ret);
   
    HAL_FIELD_SET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_L2_TYPE_BITMAPf, &entry, l2_type_tmp);
    HAL_FIELD_SET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_ETH_TYPEf, &entry, key.eth_type);
    if (key.key_valid == TRUE)
    {
        HAL_FIELD_SET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_VALIDf, &entry, 1);
    }
    else
    {
        HAL_FIELD_SET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_VALIDf, &entry, 0);
    }
    
    CMM_ERR_CHK(HAL_TBL_REG_WRITE(unit, PROTOCOL_BASED_VLANNm, group_id, sizeof(port_vlan_ctrln_t), &entry), ret);

    return CMM_ERR_OK;
}

yt_ret_t  fal_tiger_vlan_protocol_based_vlan_group_get(yt_unit_t unit, uint8_t group_id, yt_vlan_protocol_key_t *pKey)
{
    protocol_based_vlann_t  entry;
    cmm_err_t ret = CMM_ERR_OK;
    uint8_t key_valid_tmp = 0;
    uint8_t l2_type_tmp = 0;
    ...
    HAL_FIELD_GET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_L2_TYPE_BITMAPf, &entry, &l2_type_tmp);
    HAL_FIELD_GET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_ETH_TYPEf, &entry, &(pKey->eth_type));
    HAL_FIELD_GET(PROTOCOL_BASED_VLANNm, PROTOCOL_BASED_VLANN_VALIDf, &entry, &key_valid_tmp);

    if (l2_type_tmp == L2_TYPE_BITMAP_ETHV2)
    {
        pKey->l2_type = L2_ETHV2;
    }
    else if (l2_type_tmp == L2_TYPE_BITMAP_ETHSAP)
    {
        pKey->l2_type = L2_ETHSAP;
    }
    else if (l2_type_tmp == L2_TYPE_BITMAP_ETHSNAP)
    {
        pKey->l2_type = L2_ETHSNAP;
    }
    ...
    {
        pKey->key_valid = FALSE;
    }

    return CMM_ERR_OK;
}

yt_ret_t  fal_tiger_vlan_protocol_based_vlan_table_set(yt_unit_t unit, yt_port_t port, uint8_t group_id, yt_vlan_protocol_action_t action)
{
    ...
    if (action.cvid > YT_VLAN_ID_MAX || action.svid > YT_VLAN_ID_MAX)
    {
        return CMM_ERR_EXCEED_RANGE;
    }

    if (action.cvlan_action > VLAN_TRANS_ACTION_REPLACE || action.cvlan_action > VLAN_TRANS_ACTION_REPLACE)
    {
        return CMM_ERR_EXCEED_RANGE;
    }
	
    proto_idx = macid*VLAN_PROTOCOl_MAX_NUM+group_id;
    ...
    HAL_FIELD_SET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_SVIDf, &entry, action.svid); 
    HAL_FIELD_SET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_SVID_CMDf, &entry, action.cvlan_action); 
    HAL_FIELD_SET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_CVIDf, &entry, action.cvid); 
    HAL_FIELD_SET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_CVID_CMDf, &entry, action.cvlan_action); 
...
yt_ret_t  fal_tiger_vlan_protocol_based_vlan_table_get(yt_unit_t unit, yt_port_t port, uint8_t  group_id, yt_vlan_protocol_action_t *pAction)
{
    port_vlan_ctrln_t entry;
    cmm_err_t ret = CMM_ERR_OK;
    yt_macid_t macid = CAL_YTP_TO_MAC(unit, port);
    ...
    proto_idx = macid*VLAN_PROTOCOl_MAX_NUM+group_id;
    CMM_ERR_CHK(HAL_TBL_REG_READ(unit, PROTOCOL_BASED_VLAN_DATA_TBLm, proto_idx, sizeof(protocol_based_vlan_data_tbl_t), &entry), ret);
    HAL_FIELD_GET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_SVIDf, &entry, &(pAction->svid));
    HAL_FIELD_GET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_SVID_CMDf, &entry, &(pAction->cvlan_action));
    HAL_FIELD_GET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_CVIDf, &entry, &(pAction->cvid));
    HAL_FIELD_GET(PROTOCOL_BASED_VLAN_DATA_TBLm, PROTOCOL_BASED_VLAN_DATA_TBL_CVID_CMDf, &entry, &(pAction->cvlan_action));

    return CMM_ERR_OK;
}

yt_ret_t  fal_tiger_vlan_protocol_based_vlan_table_del(yt_unit_t unit, yt_port_t port, uint8_t group_id)
{
    protocol_based_vlan_data_tbl_t entry;
    cmm_err_t ret = CMM_ERR_OK;
    yt_macid_t macid = CAL_YTP_TO_MAC(unit, port);
    ...
    osal_memset(&action, 0, sizeof(yt_vlan_protocol_action_t));
