...
yt_ret_t fal_tiger_port_extif_mode_get(yt_unit_t unit, yt_port_t port, yt_extif_mode_t *pMode)
{
    ...
    else
    {
        ...
        if(*pMode > YT_EXTIF_MODE_SGFIB_RSVD - YT_EXTIF_MODE_SG_MAC)
        {
            *pMode = YT_EXTIF_MODE_SGFIB_RSVD;
            return CMM_ERR_FAIL;
        }
...
yt_ret_t fal_tiger_port_phyCrossover_mode_set(yt_unit_t unit, yt_port_t port, yt_utp_crossover_mode_t mode)
{
    ...
    if(phy_addr != INVALID_ID)
    {
        if(HALPHYDRV(unit, mac_id) != NULL)
        {
            CMM_ERR_CHK(HALPHYDRV_FUNC(unit, mac_id)->phy_crossover_mode_set(unit, phy_addr, mode), ret);
            return CMM_ERR_OK;
        }
    }
...
yt_ret_t fal_tiger_port_phyCrossover_mode_get(yt_unit_t unit, yt_port_t port, yt_utp_crossover_mode_t *pMode)
{
    ...
    if(phy_addr != INVALID_ID)
    {
        if(HALPHYDRV(unit, mac_id) != NULL)
        {
            CMM_ERR_CHK(HALPHYDRV_FUNC(unit, mac_id)->phy_crossover_mode_get(unit, phy_addr, pMode), ret);
            return CMM_ERR_OK;
        }
    }
...
yt_ret_t fal_tiger_port_phyCrossover_status_get(yt_unit_t unit, yt_port_t port, yt_utp_crossover_status_t *pStatus)
{
    ...
    if(phy_addr != INVALID_ID)
    {
        if(HALPHYDRV(unit, mac_id) != NULL)
        {
            CMM_ERR_CHK(HALPHYDRV_FUNC(unit, mac_id)->phy_crossover_status_get(unit, phy_addr, pStatus), ret);
            return CMM_ERR_OK;
        }
    }
