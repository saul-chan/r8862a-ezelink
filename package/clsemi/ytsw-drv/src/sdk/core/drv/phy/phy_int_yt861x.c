...
yt_ret_t int_yt861x_crossover_mode_set(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_mode_t mode)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;

    CMM_ERR_CHK(HALSWDRV_FUNC(unit)->switch_intif_read(unit, phy_addr, PHY_FUNCTION_CTRL_REG_16, &phydata), ret);
    ...
    HALSWDRV_FUNC(unit)->switch_intif_write(unit, phy_addr, PHY_FUNCTION_CTRL_REG_16, phydata);
    int_yt861x_restart(unit, phy_addr);

    return CMM_ERR_OK;
}

yt_ret_t int_yt861x_crossover_mode_get(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_mode_t *pMode)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;
...
yt_ret_t int_yt861x_crossover_status_get(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_status_t *pStatus)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;
...
yt_ret_t int_yt861x_loopback_set(yt_unit_t unit, uint8_t phy_addr, yt_phy_loopback_mode_t mode)
{
    yt_ret_t ret;
    uint16_t data;
    ...
    switch(mode)
    {
        ...
        case YT_PHY_LOOPBACK_MODE_EXTERNAL:
            /* disable sleep */
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x27, &data), ret);
            CLEAR_BIT(data, 15);
            CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x27, data), ret);

            /* enable external loopback */
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0xa, &data), ret);
            SET_BIT(data, 4);
            CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0xa, data), ret);

            /* reset */
            CMM_ERR_CHK(int_yt861x_restart(unit, phy_addr), ret);
            break;
