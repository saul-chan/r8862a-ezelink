...
typedef struct yt_rx_pkt_info_s
{
    uint8_t cpu_code;
    uint8_t src_port;
    uint16_t len;
...
typedef struct yt_rx_pkt_s
{
    uint8_t raw_data[PKT_BUF_LEN];
    yt_rx_pkt_info_t rx_pkt_info;
} yt_rx_pkt_t;

typedef uint32_t (*yt_nic_rx_cb_f)(yt_rx_pkt_t *p_pkt);
