...
static int matchhere(RE *regexp, char *text)
{
	...
	if (regexp->type == END && regexp->next == NULL)
		return *text == '\0';
	if (regexp->next && matchfun[regexp->next->type][0])
		return matchfun[regexp->next->type][0](regexp, regexp->next->next, text);
