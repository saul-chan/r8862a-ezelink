...
_wireless_set_brsnoop_isolation() {
	...
	[ ${proxy_arp:-0} -gt 0 ] && json_add_boolean isolate 1
}
...
for_each_interface() {
	...
	for _w_iface in $_w_ifaces; do
		...
		if [ $_w_iface_idx -ge $max_interfaces ]; then
			json_get_var section section
			logger "$phy only support $max_interfaces interfaces, '$@' on interface '$_w_iface($section)' is ignored"
			json_select ..
			continue
		fi
		_w_iface_idx="$(($_w_iface_idx + 1))"
		"$@" "$_w_iface"
		json_select ..
	done
	json_select ..
...
_wdev_common_iface_config() {
	config_add_string ssid encryption 'key:wpakey'
}
