...
static struct uci_package *uci_opmode;
...
static bool
config_bridge_vlan_disabled(struct uci_section *s)
{
	const char *vlan_disabled;

	vlan_disabled = uci_lookup_option_string(uci_ctx, s, "vlan_disabled");
	if (vlan_disabled && !strcmp(vlan_disabled, "1"))
		return true;
...
config_fixup_bridge_vlan_filtering(struct uci_section *s, const char *name)
{
	...
	bool vlan_disabled = config_bridge_vlan_disabled(s);
...
config_parse_vlan(struct device *dev, struct uci_section *s)
{
	...
	static const struct blobmsg_policy vlan_attrs[__BRVLAN_ATTR_MAX] = {
		...
		[BRVLAN_ATTR_PRIMARY] = { "primary", BLOBMSG_TYPE_BOOL },
	...
	if (tb[BRVLAN_ATTR_PRIMARY])
		vlan->primary = blobmsg_get_bool(tb[BRVLAN_ATTR_PRIMARY]);
...
bool
config_is_router_mode(void)
{
	struct uci_section *globals;
	const char *mode;

	if (!uci_opmode)
		return true;

	globals = uci_lookup_section(uci_ctx, uci_opmode, "globals");
	if (!globals)
		return true;

	mode = uci_lookup_option_string(uci_ctx, globals, "mode");
	if (!mode || !strcmp(mode, "router"))
		return true;
...
static void
config_parse_brvlan(struct uci_section *s, const char *s_name)
{
	struct uci_option *o;
	struct uci_element *e;
	struct wireless_brvlan *brvlan;
	struct wireless_brvlan_vlan *vlan;
	char *sep;
	int n_vlans = 0;
	int name_len = strlen(s_name) + 1;

	o = uci_lookup_option(uci_ctx, s, "brvlan");
	...
	brvlan = calloc(1, sizeof(*brvlan) + n_vlans * sizeof(*vlan) + name_len);
	if (!brvlan)
		return;

	brvlan->n_vlans = n_vlans;
	brvlan->vlans = vlan = (struct wireless_brvlan_vlan *)(&brvlan[1]);
	brvlan->section = (char *)&vlan[n_vlans];
	strcpy(brvlan->section, s_name);

	uci_foreach_element(&o->v.list, e) {
		vlan->vid = strtoul(e->name, &sep, 10);
		vlan->flags = BRVLAN_F_UNTAGGED;
		for (; *sep; sep++) {
			switch (*sep) {
			case '*':
				vlan->flags |= BRVLAN_F_PVID;
				break;
			case 't':
				vlan->flags &= ~BRVLAN_F_UNTAGGED;
				break;
	...
	vlist_add(&wireless_brvlans, &brvlan->node, brvlan->section);
}
...
config_parse_wireless_interface(struct wireless_device *wdev, struct uci_section *s)
{
	...
	name = alloca(strlen(s->type) + 16);
	sprintf(name, "@%s[%d]", s->type, config_section_idx(s));
	config_parse_brvlan(s, s->anonymous ? name : s->e.name);
...
config_init_all(void)
{
	...
	uci_opmode = config_init_package("cls-opmode");
	if (!uci_opmode && uci_ctx->err != UCI_ERR_NOTFOUND) {
		uci_get_errorstr(uci_ctx, &err, NULL);
		netifd_log_message(L_CRIT, "Failed to load cls-opmode config (%s)\n", err);
		free(err);
		ret = -1;
	}
	...
	vlist_update(&wireless_brvlans);
	wireless_brvlan_conf_update = false;
