...
system_bridge_set_wireless(struct device *bridge, struct device *dev)
{
	...
	system_set_dev_sysfs("af_pkt_sup_vlan", dev->ifname, dev->af_pkt_sup_vlan ? "1" : "0");
}
...
int system_bridge_addif(struct device *bridge, struct device *dev)
{
	...
	for (tries = 0; tries < 3; tries++) {
		...
		netifd_log_message(L_WARNING, "Failed to add %sdevice '%s' to bridge '%s' (tries=%d): %s\n",
				   dev->wireless ? "wireless " : "", dev->ifname, bridge->ifname,
...
static void set_npe_port_pvid(const char *iface, uint16_t vid)
{
	char path[128];
	char val[8];

	snprintf(path, sizeof(path), "/sys/kernel/debug/cls_npe/%s/default_vid", iface);
	snprintf(val, sizeof(val), "%u", vid);

	write_file(path, val);
}

bool system_if_is_npe_port(const char *ifname)
{
	char buf[64];
	int if_port;

	if (system_get_dev_sysfs("if_port", ifname, buf, sizeof(buf)) < 0)
		return false;

	if_port = strtoul(buf, NULL, 0);
	/* see kernel net/bridge/br_fwt.h */
	if (if_port >= 1 && if_port <= 5)
		return true;
	return false;
}

void system_set_sw_fwd_vid(uint16_t vid)
{
	char val[8];

	snprintf(val, sizeof(val), "%u", vid);

	write_file("/sys/class/net/sw_fwd_vid_list", val);
}
...
int system_bridge_vlan(const char *iface, uint16_t vid, int16_t vid_end, bool add, unsigned int vflags)
{
	...
	if (vflags & BRVLAN_F_PVID) {
		...
		set_npe_port_pvid(iface, add ? vid : 0);
	}
