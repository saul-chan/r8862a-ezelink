...
static const struct blobmsg_policy bridge_attrs[__BRIDGE_ATTR_MAX] = {
	...
	[BRIDGE_ATTR_PRIMARY_PORT] = { "primary_port", BLOBMSG_TYPE_STRING },
	...
	[BRIDGE_ATTR_VLAN_DISABLED] = { "vlan_disabled", BLOBMSG_TYPE_BOOL },
...
bridge_reset_primary(struct bridge_state *bst)
{
	...
	struct bridge_member *primary_bm = NULL;
	...
	if (bst->config.primary_port) {
		vlist_for_each_element(&bst->members, bm, node) {
			if (!strcmp(bst->config.primary_port, bm->name)) {
				primary_bm = bm;
				break;
			}
		}
...
{
	uint16_t flags = BRVLAN_F_SELF;

	if (!vlan->local && add)
		return;

	if (vlan->primary)
		flags |= BRVLAN_F_UNTAGGED | BRVLAN_F_PVID;

	system_bridge_vlan(bst->dev.ifname, vlan->vid, -1, add, flags);
}
...
bridge_enable_member(struct bridge_member *bm)
{
	...
	if (bst->has_vlans) {
		if (bm->dft_vid) {
			system_bridge_vlan(bm->dev.dev->ifname, bm->dft_vid, -1, false, 0);
			bm->dft_vid = 0;
		}
		...
		vlist_for_each_element(&bst->dev.vlans, vlan, node)
			vlans += bridge_set_member_vlan(bm, vlan, true);

		if (vlans == 0) {
			vlist_for_each_element(&bst->dev.vlans, vlan, node) {
				if (vlan->primary) {
					primary_vid = vlan->vid;
					break;
				}
			}
			if (primary_vid) {
				system_bridge_vlan(bm->dev.dev->ifname, primary_vid, -1, true, BRVLAN_F_UNTAGGED | BRVLAN_F_PVID);
				bm->dft_vid = primary_vid;
			}
		}
	}
...
bridge_dump_vlan(struct blob_buf *b, struct bridge_vlan *vlan)
{
	...
	blobmsg_add_u8(b, "primary", vlan->primary);
...
bridge_config_init(struct device *dev)
{
	...
	if (bst->ports) {
		if (bst->config.primary_port)
			bridge_add_member(bst, bst->config.primary_port);

		blobmsg_for_each_attr(cur, bst->ports, rem) {
			if (bst->config.primary_port && !strcmp(bst->config.primary_port, blobmsg_data(cur)))
				continue;
...
bridge_apply_settings(struct bridge_state *bst, struct blob_attr **tb)
{
	...
	if ((cur = tb[BRIDGE_ATTR_PRIMARY_PORT]))
		cfg->primary_port = blobmsg_get_string(cur);
	...
	if ((cur = tb[BRIDGE_ATTR_VLAN_DISABLED]))
		cfg->vlan_disabled = blobmsg_get_bool(cur);
}
...
bridge_vlan_equal(struct bridge_vlan *v1, struct bridge_vlan *v2)
{
	...
	if (v1->local != v2->local || v1->primary != v2->primary)
		return false;
...
static void bridge_update_sw_fwd_vid_list(struct device *dev)
{
	struct bridge_vlan *vlan;
	struct device *mdev;
	int i;
	bool has_tagged_npe, has_untagged_npe;

	/* clean sw fwd vid list */
	system_set_sw_fwd_vid(0);
	vlist_for_each_element(&dev->vlans, vlan, node) {
		has_untagged_npe = has_tagged_npe = false;
		for (i = 0; i < vlan->n_ports; i++) {
			if (!system_if_is_npe_port(vlan->ports[i].ifname))
				continue;

			mdev = device_find(vlan->ports[i].ifname);
			if (!mdev)
				continue;
			...
			if (vlan->ports[i].flags & BRVLAN_F_UNTAGGED)
				has_untagged_npe = true;
			else
				has_tagged_npe = true;
		}
		if (has_untagged_npe && has_tagged_npe)
			system_set_sw_fwd_vid(vlan->vid);
	}
}

static bool bridge_member_has_vlan(struct device *dev, struct bridge_member *bm)
{
	const char *ifname = bm->dev.dev->ifname;
	struct bridge_vlan *vlan;
	int i;
...
static void bridge_update_primary_vlan_state(struct device *dev)
{
	struct bridge_state *bst = container_of(dev, struct bridge_state, dev);
	struct bridge_member *bm;
	struct bridge_vlan *vlan;
	...
	vlist_for_each_element(&bst->members, bm, node) {
		if (bridge_member_has_vlan(dev, bm)) {
			if (bm->dft_vid) {
				system_bridge_vlan(bm->dev.dev->ifname, bm->dft_vid, -1, false, 0);
				bm->dft_vid = 0;
			}
		} else if (primary_vid != bm->dft_vid) {
			if (bm->dft_vid) {
				system_bridge_vlan(bm->dev.dev->ifname, bm->dft_vid, -1, false, 0);
				bm->dft_vid = 0;
			}
...
bridge_dev_vlan_update(struct device *dev)
{
	...
	bridge_update_sw_fwd_vid_list(dev);
	bridge_update_primary_vlan_state(dev);
	vlist_for_each_element(&dev->vlans, vlan, node) {
		if (!vlan->pending)
			continue;
