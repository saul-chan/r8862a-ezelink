...
bool wireless_brvlan_conf_update;
struct vlist_tree wireless_brvlans;
...
static const struct blobmsg_policy vif_policy[__VIF_ATTR_MAX] = {
	...
	[VIF_ATTR_MULTI_AP] = { .name = "multi_ap", .type = BLOBMSG_TYPE_BOOL },
	[VIF_ATTR_MULTI_AP_PROFILE] = { .name = "multi_ap_profile", .type = BLOBMSG_TYPE_INT32 },
};
...
prepare_config(struct wireless_device *wdev, struct blob_buf *buf, bool up)
{
	...
	vlist_for_each_element(&wdev->interfaces, vif, node) {
		...
		blobmsg_add_string(&b, "section", vif->section);
...
static void wireless_interface_handle_link(struct wireless_interface *vif, const char *ifname, bool up)
{
	...
	if (up) {
		...
		if (dev && !strcmp(ifname, vif->ifname)) {
			...
			if (vif->multi_ap && vif->multi_ap_profile > 1)
				dev->af_pkt_sup_vlan = true;
		}
	}
...
static bool
wireless_brvlan_equal(struct wireless_brvlan *v1, struct wireless_brvlan *v2)
{
	int i;

	if (v1->n_vlans != v2->n_vlans)
		return false;

	for (i = 0; i < v1->n_vlans; i++)
		if (v1->vlans[i].vid != v2->vlans[i].vid ||
		    v1->vlans[i].flags != v2->vlans[i].flags)
			return false;
...
static void
wireless_brvlan_update(struct vlist_tree *tree, struct vlist_node *node_new, struct vlist_node *node_old)
{
	struct wireless_brvlan *brvlan_new = NULL, *brvlan_old = NULL;

	if (node_old)
		brvlan_old = container_of(node_old, struct wireless_brvlan, node);
	if (node_new)
		brvlan_new = container_of(node_new, struct wireless_brvlan, node);

	if (node_new && node_old && wireless_brvlan_equal(brvlan_old, brvlan_new)) {
		free(brvlan_old);
		return;
	}

	wireless_brvlan_conf_update = true;
	free(brvlan_old);
}

void wireless_init(void)
{
	vlist_init(&wireless_brvlans, avl_strcmp, wireless_brvlan_update);
...
wireless_interface_init_config(struct wireless_interface *vif)
{
	...
	cur = tb[VIF_ATTR_MULTI_AP];
	if (cur)
		vif->multi_ap = blobmsg_get_bool(cur);

	cur = tb[VIF_ATTR_MULTI_AP_PROFILE];
	if (cur)
		vif->multi_ap_profile = blobmsg_get_u32(cur);
}
...
struct wireless_interface* wireless_interface_create(struct wireless_device *wdev, struct blob_attr *data, const char *section)
{
	...
	vif->name = strcpy(name_buf, section);
	vif->wdev = wdev;
	vif->config = data;
	vif->section = section;
	vif->isolate = false;
	...
	return vlist_find(&wdev->interfaces, section, vif, node);
}
...
__wireless_start_pending(struct uloop_timeout *t)
{
	...
	vlist_for_each_element(&wireless_devices, wdev, node) {
		...
		__wireless_device_set_up(wdev, 0);
		if (wireless_brvlan_conf_update && wdev->state == IFS_UP) {
			netifd_log_message(L_NOTICE, "apply brvlan for %s\n", wdev->name);
			sprintf(cmd, "/sbin/wifi_device_apply_brvlan %s", wdev->name);
			system(cmd);
		}
	}
}
