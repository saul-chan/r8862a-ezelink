...
#define CLS_WIFI_MAX_RSSI (0)
#define CLS_WIFI_MIN_RSSI (-100)
...
static char * format_signal(int sig)
{
	...
	if (!sig)
		snprintf(buf, sizeof(buf), "unknown");
	else if (sig > CLS_WIFI_MAX_RSSI)
		snprintf(buf, sizeof(buf), "%d dBm", CLS_WIFI_MAX_RSSI);
	else if (sig < CLS_WIFI_MIN_RSSI)
		snprintf(buf, sizeof(buf), "%d dBm", CLS_WIFI_MIN_RSSI);
	else
...
static char *format_ps_mode(int ps_mode)
{
	static char buf[16];
