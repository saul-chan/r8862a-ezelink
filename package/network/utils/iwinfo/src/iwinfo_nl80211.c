...
static int nl80211_get_sta_ps_mode_cb(struct nl_msg *msg, void *arg)
{
	int *ps_mode = arg;
	...
	nla_parse(tb_vendor, CLS_NL80211_ATTR_MAX,
			nla_data(nl_vendor), nla_len(nl_vendor), NULL);

	if (tb_vendor[CLS_NL80211_ATTR_STA_PS_MODE])
		*ps_mode = nla_get_u8(tb_vendor[CLS_NL80211_ATTR_STA_PS_MODE]);
	else
		*ps_mode = -1;
...
static int nl80211_get_sta_ps_mode(const char *ifname, uint8_t *macaddr, int *buf)
{
	int ps_mode = -1;
	...
	nla_put_u32(cv->msg, NL80211_ATTR_VENDOR_SUBCMD, CLS_NL80211_CMD_GET_STA_PS_MODE);

	params = nla_nest_start(cv->msg, NL80211_ATTR_VENDOR_DATA);
	if (params)
		nla_put(cv->msg, CLS_NL80211_ATTR_MAC_ADDR, 6, macaddr);

	nla_nest_end(cv->msg, params);

	if (nl80211_send(cv, nl80211_get_sta_ps_mode_cb, &ps_mode))
...
static int nl80211_get_assoclist(const char *ifname, char *buf, int *len)
{
	...
	if ((d = opendir("/sys/class/net")) != NULL)
	{
		...
		for (i = 0, e = arr.buf; i < arr.count; i++, e++)
			nl80211_get_sta_ps_mode(ifname, e->mac, &(e->ps_mode));
                                    ...
                                    struct iwinfo_scanlist_entry *e)
{
	...
	while (ielen >= 2 && ielen >= ie[1])
	{
		...
		case 192: /* VHT operation */
			...
			break;
		case 255: /* Extension Element ID */
			if (ie[1] >= 3 && ie[2] == 36) /* HE Opertaion */
				e->he_present = 1;
			break;
		}
...
struct freqlist_ctx {
	struct channels_ctx channels;
	struct nl80211_array_buf arr_buf;
};
...
static int nl80211_get_freqlist_cb(struct nl_msg *msg, void *arg)
{
	...
	struct freqlist_ctx *freqlist = (struct freqlist_ctx *)arg;
	struct channels_ctx *channels = &freqlist->channels;
	struct nl80211_array_buf *arr = &freqlist->arr_buf;
	...
	if (attr[NL80211_ATTR_WIPHY_BANDS]) {
		nla_for_each_nested(band, attr[NL80211_ATTR_WIPHY_BANDS], bands_remain)
		{
			if (channels->last_band != band->nla_type) {
				channels->width_40 = false;
				channels->width_80 = false;
				channels->width_160 = false;
				channels->last_band = band->nla_type;
			}
			...
			if (bands[NL80211_BAND_ATTR_HT_CAPA]) {
				__u16 cap = nla_get_u16(bands[NL80211_BAND_ATTR_HT_CAPA]);

				if (cap & BIT(1))
					freqlist->channels.width_40 = true;
			}

			if (bands[NL80211_BAND_ATTR_VHT_CAPA]) {
				__u32 capa;

				freqlist->channels.width_80 = true;

				capa = nla_get_u32(bands[NL80211_BAND_ATTR_VHT_CAPA]);
				switch ((capa >> 2) & 3) {
				case 2:
					/* width_80p80 = true; */
					/* fall through */
				case 1:
					freqlist->channels.width_160 = true;
					break;
				}
			}
...
{
	struct nl80211_msg_conveyor *cv;
	struct channels_ctx channels = { .last_band = -1 };
	struct nl80211_array_buf arr = { .buf = buf, .count = 0 };
	uint32_t features = nl80211_get_protocol_features(ifname);
	int flags;
	struct freqlist_ctx freqlist;

	freqlist.channels = channels;
	freqlist.arr_buf = arr;
	...
	if (nl80211_send(cv, nl80211_get_freqlist_cb, &freqlist))
		goto out;

	*len = freqlist.arr_buf.count * sizeof(struct iwinfo_freqlist_entry);
	return 0;
...
static int nl80211_eval_modelist(struct nl80211_modes *m)
{
	...
	if (m->bands & IWINFO_BAND_5)
	{
		...
		/* should supports 80211A in 5G band */
		m->hw |= IWINFO_80211_A;
	}
...
static int iwinfo_oper_mode_is_he(const char *ifname)
{
	bool he = false;
	char *res, b[2] = { 0 };
...
static int iwinfo_oper_mode_is_vht(const char *ifname)
{
	bool vht = false;
	char *res, b[2] = { 0 };

	res = nl80211_phy2ifname(ifname);
	if (nl80211_hostapd_query(res ? res : ifname, "ieee80211ac", b, sizeof(b)))
		vht = b[0] == '1';
	else if (nl80211_wpactl_query(res ? res : ifname, "wifi_generation", b, sizeof(b)))
		vht = b[0] == '5';
...
static int iwinfo_oper_mode_is_ht(const char *ifname)
{
	bool ht = false;
	char *res, b[2] = { 0 };

	res = nl80211_phy2ifname(ifname);
	if (nl80211_hostapd_query(res ? res : ifname, "ieee80211n", b, sizeof(b)))
		ht = b[0] == '1';
	else if (nl80211_wpactl_query(res ? res : ifname, "wifi_generation", b, sizeof(b)))
		ht = b[0] == '4';
...
static int iwinfo_oper_mode_is_a(const char *ifname)
{
	bool a = false;
	char *res, b[2] = { 0 };

	res = nl80211_phy2ifname(ifname);
	if (nl80211_hostapd_query(res ? res : ifname, "hw_mode", b, sizeof(b)))
		a = b[0] == 'a';

	return a;
}

static int iwinfo_oper_mode_is_g(const char *ifname)
{
	bool g = false;
	char *res, b[2] = { 0 };
...
static int nl80211_get_htmode(const char *ifname, int *buf)
{
	...
	*buf = 0;
	...
	case NL80211_CHAN_WIDTH_20:
		if (iwinfo_oper_mode_is_he(ifname))
			*buf = IWINFO_HTMODE_HE20;
		else if (iwinfo_oper_mode_is_vht(ifname))
			*buf = IWINFO_HTMODE_VHT20;
		else if (iwinfo_oper_mode_is_ht(ifname))
	...
	case NL80211_CHAN_WIDTH_20_NOHT:
		if (iwinfo_oper_mode_is_g(ifname))
			*buf = IWINFO_HTMODE_NOHT_G;
		else if (iwinfo_oper_mode_is_a(ifname))
			*buf = IWINFO_HTMODE_NOHT_A;
		else
