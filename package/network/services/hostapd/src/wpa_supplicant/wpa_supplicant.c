			      ...
			      enum wpa_states state)
{
	...
	/*Send ubus event to applicantion*/
	wpas_ubus_notify_wpa_state(wpa_s, state);
#endif
...
#ifdef CLS_WIFI_CONFIG
	if (wpa_s->conf->ecr_rssi != 0 && bss->level < wpa_s->conf->ecr_rssi) {
		wpa_printf(MSG_DEBUG,
			"selected SSID(%s) RSSI is %d less than Enhanced Connection Restrict RSSI(%d)",
			wpa_ssid_txt(ssid->ssid, ssid->ssid_len),
			bss->level, wpa_s->conf->ecr_rssi);
		return;
	}
#endif
