				   ...
				   const struct wps_credential *cred)
{
	...
	case WPS_AUTH_WPA2PSK:
		...
		if (add_sae && cred->key_len != 2 * PMK_LEN) {
			...
			/* wpa_supplicant should support PSK_SHA256 if it already supported SAE */
	...
	/*
	 * In mesh, the backhaul may use a hidden SSID.
	 * In this case, wpa_supplicant needs to explicitly specify this SSID for scanning
	 * to successfully connect to the AP.
	 */
	ssid->scan_ssid = 1;
#endif
				     ...
				     union wps_event_data *data)
{
	...
	wpas_ubus_notify_wps_state(wpa_s, event);
#endif
}
...
#ifdef CLS_WIFI_CONFIG
	/* catch wps credentials in /sbin/wps */
	if (!access("/sbin/wps", X_OK))
		system("/sbin/wps");
#endif
