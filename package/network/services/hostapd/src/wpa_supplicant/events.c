...
#define CLS_STA_AUTO_4ADDR

#ifdef CLS_STA_AUTO_4ADDR
#include <net/if.h>
#include <sys/ioctl.h>
#include <unistd.h>
#include <string.h>
#include <cls/cls_nl80211_vendor.h>
#define CLS_OUI_SUBTYPE 0x0
#define OUI_CLS_TYPE  (CLSEMI_OUI << 8 | CLS_OUI_SUBTYPE)

#define ntoh24(p) (((uint32_t)(p)[0] << 16) | \
		((uint32_t)(p)[1] << 8)  | \
		((uint32_t)(p)[2]))
bool cls_vendor_ie_check(const uint8_t *ie, uint8_t *hop_count, uint8_t *hop_max)
{
	uint8_t len;
	...
	if (len < 4)
		return false; // OUI(3) + subtype(1) must exist

	uint32_t oui = (ie[2] << 16) | (ie[3] << 8) | ie[4];

	if (oui != CLSEMI_OUI)
		return false;

	if (ie[5] != 0x00)
		return false; // subtype == 0

	const uint8_t *pos = ie + 6;
	size_t rem = len - 4; // remove OUI + subtype
	...
	while (rem >= 2) {
		...
		if (l + 2 > rem)
			break; // oversize protect

		if (t == 1 && l > 0) {
			uint32_t val = 0;

			if (l == 1) {
				val = pos[2]; // 1 type no need ntoh
			} else if (l == 2) {
				uint16_t tmp;
				...
				// not 1/2/4 byte get first 4 to parse
				size_t copy_len = (l > 4) ? 4 : l;
				uint8_t buf[4] = {0};
				uint32_t tmp;

				memcpy(buf + (4 - copy_len), pos + 2, copy_len); // right align
				memcpy(&tmp, buf, 4);
				val = ntohl(tmp);
			}

			*hop_count = (val >> 8)  & 0x3F; // bit8-13 (6 bits)
			*hop_max  = (val >> 14) & 0x3F; // bit14-19 (6 bits)
...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->multi_ap_vlanid_set &&
	    wpa_drv_set_multi_ap_vlan(wpa_s, wpa_s->multi_ap_vlanid, 1) == 0) {
		wpa_s->multi_ap_vlanid = 0;
		wpa_s->multi_ap_vlanid_set = 0;
	}
					...
					const u8 *ies, size_t ies_len)
{
	...
	wpa_s->multi_ap_vlanid = multi_ap.vlanid;
	wpa_s->multi_ap_ie = 1;
}
...
static void multi_ap_set_vlan(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s->current_ssid ||
	    !wpa_s->current_ssid->multi_ap_backhaul_sta)
		return;
	...
	if (!wpa_s->multi_ap_vlanid)
		return;

	if (wpa_drv_set_multi_ap_vlan(wpa_s, wpa_s->multi_ap_vlanid, 0) < 0) {
		wpa_printf(MSG_ERROR, "Failed to set vlan");
		goto fail;
	}
	wpa_s->multi_ap_vlanid_set = 1;
	return;
				       ...
				       union wpa_event_data *data)
{
	...
	int new_bss = 0;
#if defined(CONFIG_FILS) || defined(CONFIG_MBO) || defined(CLS_STA_AUTO_4ADDR)
	struct wpa_bss *bss;
	const u8 *cls_vendor_ie;
...
#if defined(CLS_STA_AUTO_4ADDR)
	int cls_ap_4addr = 0;
	uint8_t hop_count, hop_max;

	wpa_hexdump(MSG_MSGDUMP, "IEs:", bss->ies, bss->ie_len);
	cls_vendor_ie = wpa_bss_get_vendor_ie(bss, OUI_CLS_TYPE);
	wpa_msg(wpa_s, MSG_INFO, "CLSOUI:%08x\n", OUI_CLS_TYPE);
	if (cls_vendor_ie) {
		cls_ap_4addr = cls_vendor_ie_check(cls_vendor_ie, &hop_count, &hop_max);
		wpa_msg(wpa_s, MSG_INFO, "found CLS IE " MACSTR, MAC2STR(bssid));
		wpa_msg(wpa_s, MSG_INFO, "s4:%d, hop_count:%d, hop_max:%d ", cls_ap_4addr, hop_count, hop_max);
		/*TODO save hop_count and hop_max to uci for hostapd use*/
	} else
		wpa_msg(wpa_s, MSG_INFO, "no CLS IE " MACSTR, MAC2STR(bssid));

	if (wpa_s->current_ssid) {
		if ((cls_ap_4addr || wpa_s->current_ssid->enable_4addr_mode) &&
				!wpa_s->current_ssid->force_3addr_mode) {
			wpa_msg(wpa_s, MSG_INFO, "force 4addr mode for cls OUI with " MACSTR, MAC2STR(bssid));
			wpa_supplicant_set_4addr_mode(wpa_s);
		}
	}
#else
