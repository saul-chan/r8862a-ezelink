...
static inline int wpa_drv_set_multi_ap_vlan(struct wpa_supplicant *wpa_s, u16 val, u8 del)
{
	if (!wpa_s->driver->set_multi_ap_vlan)
		return -1;
	return wpa_s->driver->set_multi_ap_vlan(wpa_s->drv_priv,
					     wpa_s->ifname, val, del);
}
