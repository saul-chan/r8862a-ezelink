...
void wpas_ubus_notify(struct wpa_supplicant *wpa_s, const struct wps_credential *cred)
{
	...
	if (add_sae && auth_type == WPS_AUTH_WPA2PSK &&
	    cred->key_len != 2 * PMK_LEN)
		auth_type = WPS_AUTH_WPA2WPA3;
...
void wpas_ubus_notify_wpa_state(struct wpa_supplicant *wpa_s, enum wpa_states state)
{
	if (!ctx)
		return;

	blob_buf_init(&b, 0);
	blobmsg_add_u16(&b, "wpa_state", state);

	ubus_notify(ctx, &wpa_s->ubus.obj, "wpa_supplicant", b.head, -1);
}

static void wps_pbc_timeout_cb(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;

	wpas_ubus_notify_wps_state(wpa_s, WPS_EV_PBC_TIMEOUT);
}

void wpas_ubus_notify_wps_state(struct wpa_supplicant *wpa_s, enum wps_event event)
{
	if (!ctx)
		return;

	eloop_cancel_timeout(wps_pbc_timeout_cb, wpa_s, NULL);

	if (event == WPS_EV_PBC_ACTIVE)
		eloop_register_timeout(120, 0, wps_pbc_timeout_cb, wpa_s, NULL);

	blob_buf_init(&b, 0);
	blobmsg_add_u16(&b, "wps_event", event);

	ubus_send_event(ctx, "wpa_supplicant.wps", b.head);
}
#endif
