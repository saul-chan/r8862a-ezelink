...
/* In WPS AP IN mode, STA is WPS Registrar
 * It will send WSC_NACK msg instead of M8 after it processing M7 that from AP.
 * It caused AP can NOT recevie M8 msg and AP thinks WPS Session is Failed.
 * but STA can connect to AP successfully since it already got AP's credential from M7.
 * */
int wps_ap_pin_failed_in_nack = false;
#endif
...
#ifdef CLS_WIFI_CONFIG
	/* recover the variable to false when launching a new WPS Session */
	wps_ap_pin_failed_in_nack = false;
#endif
