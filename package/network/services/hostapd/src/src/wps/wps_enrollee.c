						 ...
						 const struct wpabuf *msg)
{
	...
	case RECV_M8:
		...
		/* In WPS registrar(STA Side),
		 * it sends WSC_NAK msg instead of M8 if it found new_ap_settings is NULL
		 * in wps_process_ap_settings_r() @ wps_registrar.c
		 *
		 * The variable 'new_ap_settings' will be NULL by using AP PIN mode.
		 *
		 * in contruct defintion ' wps_config' in wps.h
		 ...
		 *
		 * In this case, if config_error is NO error and AP just sent M7,
		 * set wps_ap_pin_failed_in_nack = true;
		 * */
		if (config_error == WPS_CFG_NO_ERROR)
			wps_ap_pin_failed_in_nack = true;
#endif
