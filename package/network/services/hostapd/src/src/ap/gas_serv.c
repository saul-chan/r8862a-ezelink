...
{
	struct anqp_element *nei_elem;

	/* For WFA-MBO testcase 4.2.4.1, one neighbor is enough, testbed will NOT check the content of neighbor IE neither */
	/* So we just use the bss anqp elem to fill GAS_INIT_RSP */
	if (!(nei_elem = get_anqp_elem(hapd, ANQP_NEIGHBOR_REPORT)))
		return;

	if (nei_elem->payload) {
		if (is_broadcast_ether_addr(nei_elem->payload->buf)) /* means the bssid in neighbor report IE is NOT set */
			os_memcpy(nei_elem->payload->buf, hapd->own_addr, ETH_ALEN);
		wpabuf_put_le16(buf, ANQP_NEIGHBOR_REPORT);
		wpabuf_put_le16(buf, (u16)wpabuf_len(nei_elem->payload));
		wpabuf_put_buf(buf, nei_elem->payload);
	}
}
#endif
...
#ifdef CLS_WIFI_CONFIG
		if (extra_req[i] == ANQP_NEIGHBOR_REPORT) {
