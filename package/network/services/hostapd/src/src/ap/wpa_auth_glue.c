				...
				int encrypt)
{
	...
	if (capwap_enable && capwap_auth_enable) {
		int res = 0;
		char wtp_data[WTP_RESPONSE_MAX_LEN + 1] = {0};
		size_t wtp_data_len;

		if (sta->wpa_sm->eapol_socket >= 0) {
			wtp_data_len = hostapd_receive_eapol_from_wtp(sta->wpa_sm, wtp_data);
			if (wtp_data_len > 0) {
				res = hostapd_drv_hapd_send_eapol(hapd, addr, (u8 *)wtp_data, wtp_data_len,
