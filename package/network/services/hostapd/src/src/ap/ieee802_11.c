...
#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable) {
		wpa_printf(MSG_DEBUG, "Send Authenticaion request to WTP: interface=%s, len=%ld",
			hapd->conf->iface, len);
		if (hostapd_send_frame_to_wtp(hapd->conf->iface, mgmt, len) < 0)
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
#endif
...
u16 check_wmm(struct hostapd_data *hapd, struct sta_info *sta,
...
#ifdef CONFIG_WAPI
static u16 check_wapi_ie(struct hostapd_data *hapd, struct sta_info *sta,
			const u8 *wapi, size_t wapi_len)
{
	int offset = 0;
	const u8 *pos = wapi;
	u16 n_akm = 0;
	...
	if (!hapd->wapi_auth)
		return WLAN_STATUS_WAPI_INVALID_IE_CAPA;
	if (!wapi || !wapi_len)
		return WLAN_STATUS_WAPI_INVALID_IE;
	if (wapi) {
		if (wapi_len < MIN_WAPI_IE_LEN)
			return WLAN_STATUS_WAPI_INVALID_IE;

		//version
		if (*(pos + offset) != 0x01 || *(pos + offset + 1) != 0x00)
			return WLAN_STATUS_WAPI_INVALID_IE_VERSION;

		//akm suit number
		offset += 2;
		n_akm = WPA_GET_LE16(pos + offset);
		if (!n_akm)
			return WLAN_STATUS_WAPI_INVALID_AKMP;

		offset += 2;
		for (i = 0; i < n_akm; i++) {
			oui = WPA_GET_BE24(pos + offset + 4 * i);
			if (oui != OUI_WAPI)
				return WLAN_STATUS_WAPI_INVALID_AKMP;
		}
		...
		j = n_akm;
		for (i = 0; i < n_akm; i++) {
			if (hapd->wapi_auth->wapi_config.wapi_key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
				if (*(pos + offset + 4 * i + 3) == WAPI_PSK) {
					j = i;
					break;
				}
			}
			if (hapd->wapi_auth->wapi_config.wapi_key_mgmt & WPA_KEY_MGMT_WAPI_CERT) {
				if (*(pos + offset + 4 * i + 3) == WAPI_CERT) {
					j = i;
					break;
				}
			}
		...
		if (j == n_akm)
			return WLAN_STATUS_WAPI_REJECT_KEY_SUIT;

		// unicast key suit
		offset += 4 * n_akm;
		n_uk = WPA_GET_LE16(pos + offset);
		if (!n_uk)
			return WLAN_STATUS_WAPI_INVALID_UNI_KEY_SUIT;
		...
		for (i = 0; i < n_uk; i++) {
			if (*(pos + offset + 4 * i + 3) != WPI_SMS4)
				return WLAN_STATUS_WAPI_INVALID_UNI_KEY_SUIT;
		}

		//multicast key suit
		offset += 4 * n_uk;
		oui = WPA_GET_BE24(pos + offset);
		if (oui != OUI_WAPI)
			return WLAN_STATUS_WAPI_INVALID_MULI_KEY_SUIT;
		offset += 3;
		if (*(pos + offset) != WPI_SMS4)
			return WLAN_STATUS_WAPI_INVALID_MULI_KEY_SUIT;
		offset += 1;
		capab = WPA_GET_LE16(pos + offset);
		if (capab != 0x0000)
			return WLAN_STATUS_WAPI_INVALID_IE_CAPA;
	}
...
#ifdef CONFIG_WAPI
	if (hapd->conf->wapi) {
		resp = check_wapi_ie(hapd, sta, elems->wapi_parms, elems->wapi_parms_len);
		if (resp != WLAN_STATUS_SUCCESS)
			return resp;
		sta->wapi_ie_len = elems->wapi_parms_len;
		sta->wapi_ie = os_malloc(sta->wapi_ie_len);
		memcpy(sta->wapi_ie, elems->wapi_parms, elems->wapi_parms_len);
	}
#endif
...
#ifdef CONFIG_WAPI
	if (hapd->conf->wapi && (sta->wapi_sm == NULL)) {
		sta->wapi_sm = wapi_auth_sta_init(hapd->wapi_auth, sta->addr, NULL);
		if (sta->wapi_sm == NULL) {
			wpa_printf(MSG_ERROR, "Failed to initialize WAPI "
					"state machine");
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
	}
...
#ifdef CONFIG_CAPWAP_IPC
	else if (capwap_enable) {
		wpa_printf(MSG_DEBUG, "Send Association response to WTP: interface=%s, len=%d",
			hapd->conf->iface, send_len);
		if (hostapd_send_frame_to_wtp(hapd->conf->iface, reply, send_len) < 0)
			res = WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
#endif
			 ...
			 int reassoc, int rssi)
{
	...
	if (resp != WLAN_STATUS_SUCCESS)
		goto fail;
#if defined (CLS_WIFI_CONFIG) && defined (CONFIG_MBO)
	if (sta->non_pref_chan) {
		hostapd_ubus_notify_sta_non_pref(hapd, sta);
		wpa_printf(MSG_WARNING, "GO TO REPORT STA-NON-PREF from assoc-req!\n");
	}
#endif
	...
	if (vbss_check_enabled(hapd) == 1) {
		if (vbss_get_aid(hapd, sta) < 0) {
			hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
				       HOSTAPD_LEVEL_INFO, "Invalid vbss aid");
			resp = WLAN_STATUS_AP_UNABLE_TO_HANDLE_NEW_STA;
			goto fail;
		}
	} else {
...
#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable) {
		wpa_printf(MSG_DEBUG, "Send Association reqesut to WTP: interface=%s, len=%ld", hapd->conf->iface, len);
		if (hostapd_send_frame_to_wtp(hapd->conf->iface, mgmt, len) < 0)
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
#endif
...
#ifdef CONFIG_WAPI
	hapd_send_wapi_envent_message(hapd, sta, WAPI_EVENT_DIASSOC);
#endif
	ap_sta_set_authorized(hapd, sta, 0);
	sta->last_seq_ctrl = WLAN_INVALID_MGMT_SEQ;
...
#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable) {
		wpa_printf(MSG_DEBUG, "Send disassociation reqesut to WTP: interface=%s, len=%ld",
			hapd->conf->iface, len);
		hostapd_send_frame_to_wtp(hapd->conf->iface, mgmt, len);
	}
#endif
...
#ifdef CONFIG_WAPI
	hapd_send_wapi_envent_message(hapd, sta, WAPI_EVENT_DEAUTH);
#endif
...
#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable) {
		wpa_printf(MSG_DEBUG, "Send deauthentication to WTP: interface=%s, len=%ld", hapd->conf->iface, len);
		hostapd_send_frame_to_wtp(hapd->conf->iface, mgmt, len);
	}
#endif
}
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	accounting_sta_stop(hapd, sta);
#ifdef CONFIG_WAPI
	hostapd_get_bctx_pn(hapd, &(sta->bctx_pn));
	hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
		       HOSTAPD_LEVEL_INFO,
		       "associated (aid %d), bctx_pn(0x%016llx)",
		       sta->aid, (unsigned long long)sta->bctx_pn);
	hapd_send_wapi_envent_message(hapd, sta, WAPI_EVENT_ASSOC);
#else
	hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
		       HOSTAPD_LEVEL_INFO,
		       "associated (aid %d)",
...
#ifdef CONFIG_WAPI
	if (sta->wapi_sm)
		wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_ASSOC, NULL);
#endif
