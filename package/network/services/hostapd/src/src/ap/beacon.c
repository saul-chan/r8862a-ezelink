...
#ifdef CONFIG_WAPI
const u8 *hostapd_wapi_ie(struct hostapd_data *hapd, u8 eid)
{
	const u8 *ies;
	size_t ies_len;

	ies = wapi_auth_get_wapi_ie(hapd->wapi_auth, &ies_len);
	if (!ies)
		return NULL;
...
static u8 *hostapd_get_wapi(struct hostapd_data *hapd, u8 *pos, size_t len)
{
	const u8 *ie;

	ie = hostapd_wapi_ie(hapd, WLAN_EID_BSS_AC_ACCESS_DELAY);
	if (!ie || 2U + ie[1] > len)
		return pos;
...
#ifdef CONFIG_WAPI
	pos = hostapd_get_wapi(hapd, pos, epos - pos);
#endif
		      ...
		      int ssi_signal)
{
	 ...
	 */
	 /* WFA-MBO-TC4.2.3_24G need probe-rsp for Intel station as beacon report measurement */
	if (!(hapd->conf->radio_measurements[0] & WLAN_RRM_CAPS_BEACON_REPORT_ACTIVE) &&
		elems.ds_params &&
	...
	if (vbss_check_enabled(hapd) == 1) {
		if (hapd->vbss_hostapd_info.role == VBSS_VAP_ROAMING) {
			wpa_printf(MSG_INFO, "%s(roam vap): don't response the probe request" MACSTR,
				hapd->conf->iface, MAC2STR(mgmt->sa));
			return;
		}
	} else {
...
#ifdef CONFIG_WAPI
	tailpos = hostapd_get_wapi(hapd, tailpos, tailend - tailpos);
#endif
...
#ifdef CONFIG_WAPI
	if (hapd->conf->wapi) {
		if (hapd->conf->wapi_pairwise == 0)
			params->pairwise_ciphers = WPA_CIPHER_SMS4;
		else
			params->pairwise_ciphers = hapd->conf->wapi_pairwise;
		if (hapd->conf->wapi_key_mgmt == 0)
			params->key_mgmt_suites = WPA_KEY_MGMT_WAPI_PSK | WPA_KEY_MGMT_WAPI_CERT;
		else
			params->key_mgmt_suites = hapd->conf->wapi_key_mgmt;
		params->wpa_version = 1 << 2;
		params->privacy = 1 << 2;
		params->group_cipher = params->pairwise_ciphers;
	}
#endif
#ifdef CONFIG_WEP
