...
#ifdef CONFIG_WAPI
	wapi_auth_sta_deinit(sta->wapi_sm);
	sta->wapi_sm = NULL;
#endif
...
#ifdef CONFIG_CAPWAP_IPC
		if (capwap_enable)
			hostapd_wtp_event_send(HOSTAPD_WTP_STA_DISCONNECTED, sta->addr);
#endif
...
skip_poll:
	...
	case STA_REMOVE:
		...
		mlme_deauthenticate_indication(
			hapd, sta,
			WLAN_REASON_PREV_AUTH_NOT_VALID);
		if (sta->deauth_reason == WLAN_REASON_DISASSOC_DUE_TO_INACTIVITY)
			   ...
			   int authorized)
{
	...
	vbss_set_hidden_ssid(hapd, authorized);
}
...
#ifdef CONFIG_WAPI
	wapi_auth_sm_event(sta->wapi_sm, WAPI_DEAUTH, NULL);
#endif
	ieee802_1x_notify_port_enabled(sta->eapol_sm, 0);
...
#ifdef CONFIG_WAPI
void send_msg_to_wapid(int sock, void *eloop_ctx, void *sock_ctx)
{
	int ret = 0;
	struct hostapd_data *hapd = eloop_ctx;
	struct sockaddr_un client_addr;

	if (!hapd || !hapd->wapid_buff) {
		eloop_unregister_sock(hapd->ctrl_sock, EVENT_TYPE_WRITE);
		eloop_register_read_sock(hapd->ctrl_sock,
				     hostapd_ctrl_iface_receive, hapd, NULL);
		return;
	}

	wpa_printf(MSG_INFO, "-----enter %s,send message type[%d]\n", __func__, *(hapd->wapid_buff));

	ret = sendto(sock, hapd->wapid_buff, hapd->wapid_buff_len, 0,
		(struct sockaddr *)&hapd->wapi_auth->wapid_addr, hapd->wapi_auth->socklen);
	if (ret == -1)
		wpa_printf(MSG_INFO, "CTRL: sendto failed: %s", strerror(errno));
	else
		wpa_printf(MSG_INFO, "send msg to wapid %d bytes\n", ret);
...
void hapd_register_write_function(struct hostapd_data *hapd)
{
	eloop_unregister_sock(hapd->ctrl_sock, EVENT_TYPE_READ);
	eloop_register_sock(hapd->ctrl_sock, EVENT_TYPE_WRITE,
					send_msg_to_wapid, hapd, NULL);
}

void hapd_send_wapi_envent_message(struct hostapd_data *hapd, struct sta_info *sta,
					enum wapi_event event_type)
{
	struct sta_state sta_state;
	struct wapi_sta_info wapi_sta_info;
	uint16_t data_len = 0;
	...
	case WAPI_EVENT_DEAUTH:
		if (hapd->conf->wapi && sta->wapi_sm
			&& sta->wapi_sm->wapi_auth_state != WAPI_DEAUTH) {
			os_memset(&sta_state, 0, sizeof(sta_state));
			os_memcpy(sta_state.ap_mac, hapd->own_addr, ETH_ALEN);
			os_memcpy(sta_state.sta_mac, sta->addr, ETH_ALEN);
			sta_state.status = 0x01;
			if (!hapd->wapid_buff) {
				data_len = sizeof(sta_state) & 0xffff;
				hapd->wapid_buff_len = 512;
				hapd->wapid_buff = os_zalloc(sizeof(u8) * hapd->wapid_buff_len);
				if (hapd->wapid_buff) {
					*(hapd->wapid_buff) = EVENT_STA_HAPD_DEAUTH;
					*(hapd->wapid_buff + 1) = MESSAGE_REQUEST;
					os_memcpy(hapd->wapid_buff + 2, &data_len, 2);
					os_memcpy(hapd->wapid_buff + 4, &sta_state, sizeof(sta_state));
					hapd_register_write_function(hapd);
					wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_DEAUTH, NULL);
				}
			}
		}
	break;
	case WAPI_EVENT_ASSOC:
		if (hapd->conf->wapi && sta->wapi_sm
			&& (sta->wapi_sm->wapi_auth_state == WAPI_AUTH_CERT
			|| sta->wapi_sm->wapi_auth_state == WAPI_AUTH_PTK)) {
			os_memset(&wapi_sta_info, 0, sizeof(wapi_sta_info));
			wapi_sta_info.type = EVENT_STA_HAPD_ASSOC_SUCCESS;
			wapi_sta_info.data_len = 32 + 2 + sta->wapi_ie_len;
			os_memcpy(wapi_sta_info.ap_mac, hapd->own_addr, ETH_ALEN);
			os_memcpy(wapi_sta_info.sta_mac, sta->addr, ETH_ALEN);
			wapi_sta_info.wie[0] = 0x44;
			wapi_sta_info.wie[1] = sta->wapi_ie_len;
			os_memcpy(wapi_sta_info.wie + 2, sta->wapi_ie, sta->wapi_ie_len);
			// to add pn
			os_memcpy(wapi_sta_info.data_packet_num, (uint8_t *)(&sta->bctx_pn),
					sizeof(sta->bctx_pn));

			if (!hapd->wapid_buff) {
				data_len = sizeof(wapi_sta_info) & 0xffff;
				hapd->wapid_buff_len = 512;
				hapd->wapid_buff = os_zalloc(sizeof(u8) * hapd->wapid_buff_len);
				if (hapd->wapid_buff) {
					*(hapd->wapid_buff) = EVENT_STA_HAPD_ASSOC_SUCCESS;
					*(hapd->wapid_buff + 1) = MESSAGE_REQUEST;
					os_memcpy(hapd->wapid_buff + 2, &data_len, 2);
					os_memcpy(hapd->wapid_buff + 4, &wapi_sta_info, sizeof(wapi_sta_info));
					hapd_register_write_function(hapd);
				}
			}
		}
	...
	case WAPI_EVENT_DIASSOC:
		if (hapd->conf->wapi && sta->wapi_sm
			...
			if (!hapd->wapid_buff) {
				...
				hapd->wapid_buff = os_zalloc(sizeof(u8) * hapd->wapid_buff_len);
				if (hapd->wapid_buff) {
					*(hapd->wapid_buff) = EVENT_STA_HAPD_ASSOC_FAIL;
					*(hapd->wapid_buff + 1) = MESSAGE_REQUEST;
					os_memcpy(hapd->wapid_buff + 2, &data_len, 2);
					os_memcpy(hapd->wapid_buff + 4, &wapi_sta_info, sizeof(wapi_sta_info));
					hapd_register_write_function(hapd);
					wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_DIASSOC, NULL);
				}
			}
		}
	break;
