...
#ifdef CONFIG_WAPI
int is_wapi_enabled(struct hostapd_data *hapd)
{
	//bit 0: WPA_KEY_MGMT_WAPI_PSK, bit 1: WPA_KEY_MGMT_WAPI_CERT,
	//bit 2: WAPI pre authentication
	int enabled = 0;

	if (!hapd->conf->wapi)
		return enabled;
	if (hapd->conf->wapi_key_mgmt & WPA_KEY_MGMT_WAPI_CERT) {
		enabled |= 1 << 0;
	} else {
		if (hapd->conf->wapi_key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
			if (hapd->conf->ssid.wapi_passphrase[0] != 0)
				enabled |= 1 << 1;
		}
	}
	if (hapd->conf->wapi_pre_auth)
		enabled |= 1 << 2;
	return enabled;
}
