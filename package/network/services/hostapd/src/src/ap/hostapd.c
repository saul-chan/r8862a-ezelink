...
#define WPS_AP_PIN_LEN 32
#define WPS_AP_PIN_DEFAULT_TIMEOUT 30
...
#ifdef CONFIG_WAPI
	if (hapd->conf->wapi && hapd->wapi_auth == NULL)
		hostapd_setup_wapi(hapd);
#endif
				      ...
				      struct hostapd_config *oldconf)
{
	...
	if (newconf->radio_config_id != oldconf->radio_config_id) {
		if (!newconf->radio_config_id ||
		    !oldconf->radio_config_id ||
		    os_strcmp(newconf->radio_config_id, oldconf->radio_config_id))
			return 1;
	}
...
static int reconf_ctrl_sock = -1;
static struct sockaddr_storage *reconf_from;
static socklen_t reconf_fromlen;

void set_reconf_ctrl_sock(int ctrl_sock, struct sockaddr_storage *from, socklen_t fromlen)
{
	reconf_ctrl_sock = ctrl_sock;
	reconf_from = from;
	reconf_fromlen = fromlen;
}

static void send_reconf_reply_in_advance(char *reply, int reply_len)
{
	if (reconf_ctrl_sock < 0)
		return;

	sendto(reconf_ctrl_sock, reply, reply_len, 0, (struct sockaddr *)reconf_from, reconf_fromlen);
}
...
int hostapd_reload_config(struct hostapd_iface *iface)
{
	...
	if (!strncmp(iface->config_fname, "data:", 5))
		config_fname = hapd->iconf->radio_config_file;
	else
		config_fname = iface->config_fname;

	newconf = iface->interfaces->config_read_cb(config_fname);
	if (newconf == NULL)
		return -1;
	...
	if (hostapd_iface_conf_changed(newconf, oldconf)) {
		...
		wpa_printf(MSG_INFO, "reload config %s", config_fname);
		fname = os_strdup(config_fname);
		if (!fname) {
			hostapd_config_free(newconf);
			return -1;
		}
		send_reconf_reply_in_advance("OK\n", 3);
...
static int hostapd_broadcast_key_clear(struct hostapd_data *hapd)
{
...
#ifdef CONFIG_WAPI
	if (conf->wapi && hostapd_setup_wapi(hapd)) {
		wpa_printf(MSG_INFO, "%s :set up wapi authenticator failed.", __func__);
		return -1;
	}
#endif
...
dfs_offload:
	...
	/* update channel and chwidth of smart_dfs_cac_data after completing setup interface */
...
#ifdef CLS_WIFI_CONFIG
	/* It only updates time when deiniting interface */
	hostapd_update_smart_dfs_cac(iface, 1);
#endif
...
#ifdef CLS_WIFI_CONFIG
	/* init smart dfs cac's data */
	hostapd_init_smart_dfs_cac(hapd_iface);
#endif
...
void hostapd_set_state(struct hostapd_iface *iface, enum hostapd_iface_state s)
{
	...
	if (capwap_enable) {
		...
		if (iface->conf) {
			if (s == HAPD_IFACE_ENABLED) {
				if (hostapd_wtp_event_send(HOSTAPD_WTP_INTERFACE_INIT, iface->conf->bss[0]->iface) < 0)
					wpa_printf(MSG_ERROR, "Send HOSTAPD_WTP_INTERFACE_INIT event for %s failed.\n",
						iface->conf->bss[0]->iface);
				for (i = 0; i < iface->conf->num_bss; i++)
					if (!hostapd_wtp_conn_add(iface->conf->bss[i]->iface))
						wpa_printf(MSG_ERROR, "Add hostapd data connection for %s failed.\n",
							iface->conf->bss[i]->iface);
			} else if (s == HAPD_IFACE_DISABLED) {
				hostapd_wtp_conn_del(iface->conf->bss[0]->iface);
			}
		}
	}
#endif
...
int hostapd_find_bss(struct hostapd_iface *iface, const char *ifname)
{
	int i;
...
int hostapd_add_bss(struct hostapd_iface *iface, const char *bss_name)
{
	...
	if (!strncmp(iface->config_fname, "data:", 5))
		config_fname = iface->bss[0]->iconf->radio_config_file;
	else
		config_fname = iface->config_fname;
	...
	if ((cfg_idx >= 0) && (bss_idx < 0)) {
		...
		iface->bss[iface->num_bss++] = hapd;
		hapd->msg_ctx = hapd;
		if (hostapd_setup_bss(hapd, 0, true) < 0) {
			bss_idx = hostapd_find_bss(iface, bss_name);
			if (bss_idx > 0)
...
#ifdef CONFIG_WEP
		hostapd_broadcast_key_clear(hapd);
#endif
