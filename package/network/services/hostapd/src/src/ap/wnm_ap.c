...
static u8 *ieee802_11_fill_own_candidatelist(struct hostapd_data *hapd, u8 *frm)
{
	u8 *pos = frm;
	...
	*pos++ = 13;
	...
	pos+=ETH_ALEN;
	WPA_PUT_LE32(pos, bssid_info);
	pos+=4;
	*pos++ = op_class;
	*pos++ = channel;
	*pos++ = ieee80211_get_phy_type(hapd->iface->freq, ht, vht);
	return pos;
}
#endif
						  ...
						  u8 dialog_token)
{
...
#ifdef CLS_WIFI_CONFIG
	/*
	 * including one own neighbor report
	 */
	mgmt = os_zalloc(sizeof(*mgmt)+15);
#else
	...
	pos = ieee802_11_fill_own_candidatelist(hapd, pos);
#endif
