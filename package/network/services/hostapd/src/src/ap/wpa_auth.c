...
void wpa_auth_set_eapol(struct wpa_authenticator *wpa_auth,
...
#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable && capwap_auth_enable) {
		if (sm->eapol_socket >= 0)
			hostapd_send_eapol_to_wtp(sm, data, data_len);
	}
#endif
...
static int wpa_sm_step(struct wpa_state_machine *sm)
{
	...
	if (capwap_enable && capwap_auth_enable) {
		if (sm->eapol_socket < 0)
			hostapd_wtp_eapol_socket_init(sm);
	}
#endif
	sm->in_step_loop = 1;
	do {
...
int vbss_get_sta_sec_info(struct wpa_authenticator *wpa_auth,
		struct wpa_state_machine *wpa_sm, struct vbss_key_info *vbss_key)

{
	struct wpa_group *group;
	struct wpa_ptk *ptk;

	if (wpa_auth == NULL || wpa_sm == NULL) {
		wpa_printf(MSG_INFO, "%s failed", __func__);

		return -1;
	}

	vbss_key->wpa_alg = wpa_cipher_to_alg(wpa_sm->pairwise);
	os_memcpy(vbss_key->tk, wpa_sm->PTK.tk, WPA_TK_MAX_LEN);
	wpa_hexdump(MSG_DEBUG, "tk", vbss_key->tk, 16);
	group = wpa_auth->group;
	if (group) {
		os_memcpy(vbss_key->GMK, group->GMK, sizeof(group->GMK));
		os_memcpy(vbss_key->GTK, group->GTK, sizeof(group->GTK));
		vbss_key->GTK_len = group->GTK_len;
		vbss_key->GN = group->GN;
		vbss_key->GM = group->GM;

		os_memcpy(vbss_key->IGTK, group->IGTK, sizeof(group->IGTK));
		os_memcpy(vbss_key->BIGTK, group->BIGTK, sizeof(group->BIGTK));
		vbss_key->GN_igtk = group->GN_igtk;
		vbss_key->GM_igtk = group->GM_igtk;

		vbss_key->GN_bigtk = group->GN_bigtk;
		vbss_key->GM_bigtk = group->GM_bigtk;
	}
	return 0;
}
#else
...
int vbss_disable_gmk_rekey(struct wpa_authenticator *wpa_auth)
{
	...
	if (conf->wpa_gmk_rekey)
		eloop_cancel_timeout(wpa_rekey_gmk, wpa_auth, NULL);
	return 0;
}

int vbss_disable_gtk_rekey(struct wpa_authenticator *wpa_auth)
{
	struct wpa_auth_config *conf = NULL;
	...
	if (!wpa_auth)
		return -1;
	conf = &wpa_auth->conf;
	if (conf->wpa_group_rekey)
		eloop_cancel_timeout(wpa_rekey_gtk, wpa_auth, NULL);
	return 0;
}

int vbss_disable_ptk_rekey(struct wpa_state_machine *wpa_sm)
{
	struct wpa_auth_config *conf = NULL;

	if (!wpa_sm || !wpa_sm->wpa_auth)
		return -1;
	conf = &wpa_sm->wpa_auth->conf;
	if (conf->wpa_ptk_rekey)
		eloop_cancel_timeout(wpa_rekey_ptk, wpa_sm->wpa_auth, wpa_sm);
	return 0;
}
#endif
