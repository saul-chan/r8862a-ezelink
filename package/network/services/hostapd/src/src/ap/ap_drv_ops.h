...
#ifdef CONFIG_WAPI
void hostapd_get_bctx_pn(struct hostapd_data *hapd, uint64_t *pn);
#endif
...
static inline int
hostapd_set_smart_dfs_cac(struct hostapd_iface *iface, int skip_dfs_cac)
{
	struct hostapd_data *hapd = iface->bss[0];

	if (!hapd->driver || !hapd->drv_priv || !hapd->driver->set_skip_dfs_cac)
		return -1;

	return hapd->driver->set_skip_dfs_cac(hapd->drv_priv, skip_dfs_cac);
}

static inline int
hostapd_cancel_dfs_cac(struct hostapd_iface *iface)
{
	struct hostapd_data *hapd = iface->bss[0];

	if (!hapd->driver || !hapd->drv_priv || !hapd->driver->cancel_dfs_cac)
		return -1;

	return hapd->driver->cancel_dfs_cac(hapd->drv_priv);
}
#endif
