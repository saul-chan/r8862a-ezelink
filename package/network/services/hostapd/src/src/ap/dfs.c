...
#define CLS_WIFI_SMART_DFS_CAC_IFACE_MAX_NUM (4)
struct smart_dfs_cac {
	char interface[IFNAMSIZ + 1];
	int channel;
	int chwidth;
	struct os_time start_time;
	int smart_dfs_cac_duration;
};

enum smart_dfs_cac_status {
	SMART_DFS_CAC_STATUS_NEED_CAC,
	SMART_DFS_CAC_STATUS_SKIP_CAC,
	SMART_DFS_CAC_STATUS_MAX
};

static struct smart_dfs_cac
smart_dfs_cac_data[CLS_WIFI_SMART_DFS_CAC_IFACE_MAX_NUM] = {0x00};

#define CLS_WIFI_SMART_DFS_CAC_DURATION (5*60)
int hostapd_init_smart_dfs_cac(struct hostapd_iface *hapd_iface)
{
	int i = 0;
	char *interface;
	...
	interface = hapd_iface->conf->bss[0]->iface;

	if (!hapd_iface->conf->smart_dfs_cac) {
		wpa_printf(MSG_DEBUG, "%s(), %d, smart_dfs_cac is disabled , interface = %s",
				__func__, __LINE__, interface);
		return -1;
	}

	for (i = 0; i <= CLS_WIFI_SMART_DFS_CAC_IFACE_MAX_NUM; i++) {
		if (smart_dfs_cac_data[i].interface[0] != 0) {
			if (!strcmp(smart_dfs_cac_data[i].interface, interface)) {
				wpa_printf(MSG_DEBUG, "%s(), %d, smart_dfs_cac is allready inited.",
								__func__, __LINE__);
				return 0;
			}
			/* continue to check the next item */
		} else {
			/* not found interface, use i */
			break;
		}
	}

	if (i == CLS_WIFI_SMART_DFS_CAC_IFACE_MAX_NUM) {
		wpa_printf(MSG_DEBUG, "%s(), %d, invalid index", __func__, __LINE__);
		return -1;
	}

	/* initlizating */
	os_strlcpy(smart_dfs_cac_data[i].interface, interface, IFNAMSIZ);

	/* init channel and chwidth as -1 */
	smart_dfs_cac_data[i].channel = -1;
	smart_dfs_cac_data[i].chwidth = -1;

	memset(&smart_dfs_cac_data[i].start_time, 0x00, sizeof(struct os_time));

	if (hapd_iface->conf->smart_dfs_cac_duration)
		smart_dfs_cac_data[i].smart_dfs_cac_duration = hapd_iface->conf->smart_dfs_cac_duration;
	else
		smart_dfs_cac_data[i].smart_dfs_cac_duration = CLS_WIFI_SMART_DFS_CAC_DURATION;

	return 0;
}

static int hostapd_find_smart_dfs_cac_iface(struct hostapd_iface *hapd_iface)
{
	int i = 0;
	char *interface;
	...
	for (; i <= CLS_WIFI_SMART_DFS_CAC_IFACE_MAX_NUM; i++) {
		if (!strcmp(smart_dfs_cac_data[i].interface, interface))
			return i;
	}
...
int hostapd_update_smart_dfs_cac(struct hostapd_iface *hapd_iface, int update_timer)
{
	int i;
	char *interface;
	...
	i = hostapd_find_smart_dfs_cac_iface(hapd_iface);
	if (i < 0) {
		wpa_printf(MSG_DEBUG, "%s(), %d, invalid index, interface = %s",
				__func__, __LINE__, interface);
		return -1;
	}

	/* do NOT update channel and chwidth when calling interface_deinit() */
	smart_dfs_cac_data[i].channel = hapd_iface->conf->channel;
	smart_dfs_cac_data[i].chwidth = hostapd_get_oper_chwidth(hapd_iface->conf);

	if (update_timer)
		os_get_time(&(smart_dfs_cac_data[i].start_time));

	wpa_printf(MSG_DEBUG, "%s(), %d, smart.ch = %d, chwidth = %d, update_timer = %d, sec = %ld",
			__func__, __LINE__, smart_dfs_cac_data[i].channel,
			smart_dfs_cac_data[i].chwidth, update_timer, smart_dfs_cac_data[i].start_time.sec);

	return 0;
}

static enum smart_dfs_cac_status
hostapd_check_smart_dfs_cac(struct hostapd_iface *hapd_iface)
{
	int duration;
	struct os_time now;
	char *interface;
	struct hostapd_config *conf = hapd_iface->conf;
	int i = hostapd_find_smart_dfs_cac_iface(hapd_iface);

	if (!(hapd_iface && conf && i >= 0))
		return -1;
	...
	if (smart_dfs_cac_data[i].channel == -1 ||
			smart_dfs_cac_data[i].chwidth == -1) {
		return SMART_DFS_CAC_STATUS_NEED_CAC;
	} else if (smart_dfs_cac_data[i].channel == hapd_iface->conf->channel &&
			smart_dfs_cac_data[i].chwidth >= hostapd_get_oper_chwidth(conf)) {
		if (smart_dfs_cac_data[i].start_time.sec != 0) {
			/*interface is down in the previous step*/
			os_get_time(&now);
			duration = now.sec - smart_dfs_cac_data[i].start_time.sec;

			if (duration > smart_dfs_cac_data[i].smart_dfs_cac_duration)
				return SMART_DFS_CAC_STATUS_NEED_CAC;
			else
				return SMART_DFS_CAC_STATUS_SKIP_CAC;
		} else
			return SMART_DFS_CAC_STATUS_NEED_CAC;
	} else {
		/* changed channal or increased bandwidth in the same channel, etc. , can not skip CAC */
		return SMART_DFS_CAC_STATUS_NEED_CAC;
	}
}
#endif
...
#ifdef CLS_WIFI_CONFIG
	if (iface->conf->smart_dfs_cac &&
		hostapd_check_smart_dfs_cac(iface) == SMART_DFS_CAC_STATUS_SKIP_CAC) {
		/* if smart dfs cac is enabled and skip dfs cac */
		wpa_printf(MSG_DEBUG, "%s(), %d, iface->conf->smart_dfs_cac = %d, skip dfs cac",
				__func__, __LINE__, iface->conf->smart_dfs_cac);
		hostapd_set_smart_dfs_cac(iface, 1);
	}
#endif
...
#ifdef CLS_WIFI_CONFIG
		/* update channel and chwidth of smart_dfs_cac_data after completing DFS CAC */
		hostapd_update_smart_dfs_cac(iface, 0);
#endif
