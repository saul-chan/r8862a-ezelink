...
struct hostapd_bss_config {
	...
	/* disable delivered Random per-device PSKs
	 * for WPS enrollee when wps_psk_file is set in hostapd.conf*/
	int disable_wps_per_dev_psk;

	/* correct wps session status in AP PIN mode
	 * if STA sends NACK message instead of M8 */
	int correct_ap_pin_status_in_nack;

	/*
	 * Whether save wps_cred to hostapd.conf in 'Not Configured' mode or NOT
	 */
	int wps_cred_save_conf;
...
struct hostapd_config {
	...
	size_t num_bss;
	char *radio_config_id;
	char *radio_config_file;
