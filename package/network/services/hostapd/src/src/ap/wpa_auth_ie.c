		     ...
		     const u8 *pmkid)
{
	...
	/* added optional fields from RSNE when using PMF based on WPA3 Spec
	 * group management cipher suites should be BIP-128-CMAC for
	 * WPA3-Enterprise-with-128-bit mode.
	 * */
	if (conf->ieee80211w != NO_MGMT_FRAME_PROTECTION &&
	    ((conf->sae_require_mfp == 1 && (conf->wpa_key_mgmt & WPA_KEY_MGMT_IEEE8021X_SHA256)) ||
