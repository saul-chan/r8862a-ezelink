...
void hostapd_ubus_notify_wps(struct hostapd_data *hapd, const struct wps_credential *cred)
{
	...
	if (auth_type != WPS_AUTH_OPEN &&
	    ...
	    auth_type != WPS_AUTH_WPA2PSK) {
		wpa_printf(MSG_DEBUG, "WPS: Ignored credentials for unsupported authentication type 0x%x",
			   auth_type);
		return;
	}
	...
	if (auth_type == WPS_AUTH_WPAPSK || auth_type == WPS_AUTH_WPA2PSK) {
		if (cred->key_len < 8 || cred->key_len > 2 * PMK_LEN) {
			wpa_printf(MSG_ERROR, "WPS: Reject PSK credential with invalid Network Key length %lu",
	...
	ifname_len = strlen(hapd->conf->iface);
	ifname = blobmsg_alloc_string_buffer(&b, "ifname", ifname_len + 1);
	memcpy(ifname, hapd->conf->iface, ifname_len + 1);
	ifname[ifname_len] = '\0';
	blobmsg_add_string_buffer(&b);
	...
	/* send a broadcast ubus event and /sbin/wps will receive it. */
...
int hostapd_ubus_notify_sta_non_pref(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	blob_buf_init(&b, 0);
	blobmsg_add_macaddr(&b, "sta_mac", sta->addr);
	blobmsg_add_u8(&b, "opclass", sta->non_pref_chan->op_class);
	blobmsg_add_u8(&b, "channel", sta->non_pref_chan->channels[0]);
	blobmsg_add_u8(&b, "pref_value", sta->non_pref_chan->pref);
	blobmsg_add_u8(&b, "reason", sta->non_pref_chan->reason_code);

	if (!hapd->ubus.notify_response) {
		ubus_notify(ctx, &hapd->ubus.obj, "sta_non_pref", b.head, -1);
		return 0;
	}

	if (ubus_notify_async(ctx, &hapd->ubus.obj, "sta_non_pref", b.head, &ureq.nreq))
		return 0;
...
int hostapd_handle_cls_nl80211_event(struct hostapd_data *hapd, uint32_t event, union cls_event_request *request)
{
	...
	switch (event) {
		case CLS_NL80211_CMD_REPORT_VBSS_ROAM_RESULT:
			struct roam_result *result = &(request->roam_result);
			wpa_printf(MSG_DEBUG, "VBSS_ROAM_RESULT: " MACSTR "result=%d", MAC2STR(result->sta_mac), result->result);

			blob_buf_init(&b, 0);
			blobmsg_add_macaddr(&b, "bssid", hapd->own_addr);
			blobmsg_add_macaddr(&b, "sta_mac", result->sta_mac);
			blobmsg_add_u32(&b, "result", result->result);
			ubus_notify(ctx, &hapd->ubus.obj, "vbss.roam.result", b.head, -1);
			break;
		default:
			return -1;
	}
...
#ifdef CLS_WIFI_CONFIG
void hostapd_ubus_notify_wps_state(struct hostapd_data *hapd, enum wps_event event)
{
	if (!ctx)
		return;

	blob_buf_init(&b, 0);
	blobmsg_add_u16(&b, "wps_event", event);
	ubus_send_event(ctx, "hostapd.wps", b.head);
}
#endif
...
int hostapd_ubus_notify_disallow_status(struct hostapd_data *hapd)
{
	...
	blob_buf_init(&b, 0);
	blobmsg_add_macaddr(&b, "bssid", hapd->own_addr);
	blobmsg_add_u8(&b, "disallow", hapd->mbo_assoc_disallow);

	ubus_notify(ctx, &hapd->ubus.obj, "disallow_status", b.head, -1);
#endif
	return 0;
}
