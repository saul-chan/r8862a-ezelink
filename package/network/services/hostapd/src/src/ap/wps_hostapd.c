...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	if (!strncmp(hapd->iface->config_fname, "data:", 5))
		config_fname = hapd->iconf->radio_config_file;
	else
		config_fname = hapd->iface->config_fname;

	tmp_fname = os_malloc(len);
	if (tmp_fname == NULL)
		return -1;
	os_snprintf(tmp_fname, len, "%s-new", config_fname);

	oconf = fopen(config_fname, "r");
	if (oconf == NULL) {
	...
	if (rename(tmp_fname, config_fname) < 0) {
	...
	hostapd_ubus_notify_wps(hapd, cred);
#endif
...
#ifdef CLS_WIFI_CONFIG
	/* send wps event to application via ubus */
	hostapd_ubus_notify_wps_state(hapd, event);
#endif
}
...
int interface_count(struct hostapd_iface *iface)
{
	int count = 0;
	if (iface->interfaces == NULL ||
	    iface->interfaces->for_each_interface == NULL)
		     ...
		     struct hostapd_bss_config *conf)
{
...
#ifdef CLS_WIFI_CONFIG
	if (conf->ssid.wpa_psk_file && !conf->disable_wps_per_dev_psk) {
#else
	...
	cfg.disable_auto_conf = !hapd->conf->wps_cred_save_conf ||
		((hapd->conf->wps_cred_processing == 1) &&
...
static int wps_button_pushed(struct hostapd_data *hapd, void *ctx)
{
	...
	if (hapd->wps) {
		...
		/* double check dualband again while pushbutton is pressed. */
		wps_registrar_set_dualband(hapd);
...
#ifdef CLS_WIFI_CONFIG
int hostapd_cls_wps_get_status(struct hostapd_data *hapd, char *reply, size_t reply_len)
{
	struct cls_wps_stat *cls_wps_status;

	if (sizeof(*cls_wps_status) > reply_len)
		return -1;

	cls_wps_status = (struct cls_wps_stat *)reply;
	os_memset(cls_wps_status, 0x00, sizeof(struct cls_wps_stat));

	/* copy sturct from hostpad's wps_stat to CLS defined wps_stat
	 * to make sure that CLS's wps_stat can work even thought
	 * hostapd changes its definitions of 'struct wps_stat'
	 */
	if (hapd->conf->correct_ap_pin_status_in_nack && wps_ap_pin_failed_in_nack) {
		/* in AP PIN mode, wps status will be WPS_STATUS_FAILURE with some STAs
		 * if its failure_reason is NO_ERROR, set result to Success
		 * */
		if (hapd->wps_stats.failure_reason == WPS_EI_NO_ERROR)
			cls_wps_status->status = WPS_STATUS_SUCCESS;
		else
			cls_wps_status->status = WPS_STATUS_FAILURE;
	} else
		cls_wps_status->status = hapd->wps_stats.status;

	cls_wps_status->failure_reason = hapd->wps_stats.failure_reason;
	cls_wps_status->pbc_status = hapd->wps_stats.pbc_status;
	memcpy(cls_wps_status->peer_addr, hapd->wps_stats.peer_addr, ETH_ALEN);

	return sizeof(*cls_wps_status);
}
#endif
