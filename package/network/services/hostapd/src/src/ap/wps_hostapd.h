...
int hostapd_cls_wps_get_status(struct hostapd_data *hapd,
					char *reply, size_t reply_len);

int interface_count(struct hostapd_iface *iface);
void wps_registrar_set_dualband(struct hostapd_data *hapd);
...
static inline int hostapd_cls_wps_get_status(struct hostapd_data *hapd,
						char *reply, size_t reply_len)
{
	return 0;
}
