...
static void cls_nl80211_vbss_roam_result(struct wpa_driver_nl80211_data *drv,
				       const u8 *data, size_t len)
{
	struct nlattr *tb[CLS_NL80211_ATTR_MAX + 1];
	union cls_event_request request;
	struct roam_result *result = &(request.roam_result);

	wpa_printf(MSG_DEBUG, "nl80211: CLS vendor event VBSS Roam Result received");

	if (nla_parse(tb, CLS_NL80211_ATTR_MAX, (struct nlattr *) data,len, NULL) ||
	    !tb[CLS_NL80211_ATTR_ROAM_RESULT] || !tb[CLS_NL80211_ATTR_MAC_ADDR])
		return;

	memset(&request, 0, sizeof(request));
	memcpy(result->sta_mac, nla_data(tb[CLS_NL80211_ATTR_MAC_ADDR]), ETH_ALEN);
	result->result = nla_get_u32(tb[CLS_NL80211_ATTR_ROAM_RESULT]);
	wpa_printf(MSG_DEBUG, "nl80211: VBSS Roam Result: MAC" MACSTR " %d", MAC2STR(result->sta_mac), result->result);

	/* handle VBSS Roam Result in hostapd */
	/* To-Do */

	/* send the event to ubus */
	hostapd_handle_cls_nl80211_event((struct hostapd_data *)(drv->ctx), CLS_NL80211_CMD_REPORT_VBSS_ROAM_RESULT, &request);
}


static void nl80211_vendor_event_cls(struct wpa_driver_nl80211_data *drv,
				      u32 subcmd, u8 *data, size_t len)
{
	wpa_printf(MSG_DEBUG, "nl80211: Got CLS vendor event %u", subcmd);
	switch (subcmd) {
	case CLS_NL80211_CMD_REPORT_VBSS_ROAM_RESULT:
		wpa_msg(NULL, MSG_INFO, "%s", data);
		cls_nl80211_vbss_roam_result(drv, data, len);
		break;

	default:
		wpa_printf(MSG_DEBUG,
			   "%s: Ignore unsupported CLS vendor event %u",
			   __func__, subcmd);
		break;
	}
}
...
#ifdef CLS_VBSS_CONFIG
	case CLSEMI_OUI:
		nl80211_vendor_event_cls(drv, subcmd, data, len);
		break;
#endif /* CLS_VBSS_CONFIG */
	default:
