...
#ifdef CONFIG_WAPI
	__AKM(WAPI_PSK, WAPI_PSK);
	__AKM(WAPI_CERT, WAPI_CERT);
#endif
				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	if ((params->key_mgmt_suites & WPA_KEY_MGMT_WAPI_PSK ||
		params->key_mgmt_suites & WPA_KEY_MGMT_WAPI_CERT) &&
		nla_put_u16(msg, NL80211_ATTR_CONTROL_PORT_ETHERTYPE, ETH_P_WAPI))
		goto fail;
#endif
	...
	if (ret) {
		...
		wpa_printf(MSG_DEBUG, "nl80211: Beacon set succeeded %d",
			   ret);
		link->beacon_set = 1;
...
static int wpa_driver_nl80211_stop_ap(void *priv)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	const u8 multi_sta[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

	if (!is_ap_interface(drv->nlmode))
		return -1;

	wpa_printf(MSG_DEBUG, "[%s] "MACSTR" multicast deauth ", __func__, MAC2STR(bss->addr));

	i802_sta_deauth(priv, bss->addr, multi_sta, WLAN_REASON_DEAUTH_LEAVING, -1);
...
static int nl80211_set_skip_dfs_cac(void *priv, u8 skip_dfs_cac)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	wpa_printf(MSG_DEBUG, "%s(), %d, skip DFS CAC, skip_dfs_cac = %d",
			__func__, __LINE__, skip_dfs_cac);

	msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR);
	if (msg) {
		nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, CLSEMI_OUI);
		nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, CLS_NL80211_CMD_SET_SKIP_DFS_CAC);

		params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA);
		if (params)
			nla_put_u8(msg, CLS_NL80211_ATTR_SET_SKIP_DFS_CAC, skip_dfs_cac);
		else {
			nlmsg_free(msg);
			return -1;
		}
	...
	ret = send_and_recv_msgs(drv, msg, NULL, NULL, NULL, NULL);
	wpa_printf(MSG_DEBUG,
			"%s(), %d nl80211: set skip dfs cac result: %s", __func__, __LINE__, strerror(-ret));
	return 0;
}

static int nl80211_cancel_dfs_cac(void *priv)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	if (msg) {
		nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, CLSEMI_OUI);
		nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD, CLS_NL80211_CMD_CANCEL_DFS_CAC);

		params = nla_nest_start(msg, NL80211_ATTR_VENDOR_DATA);
		if (params)
			nla_put_u8(msg, CLS_NL80211_ATTR_CANCEL_DFS_CAC, 1);
		else {
			nlmsg_free(msg);
			return -1;
		}
...
#ifdef CONFIG_WAPI
static int cls_vendor_txpn_handler(struct nl_msg *msg, void *arg)
{
	struct nlattr *tb[NL80211_ATTR_MAX + 1];
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
	uint64_t *info = arg;
	...
	if (nl_vend) {
		...
		attr = tb_vendor[CLS_NL80211_ATTR_BCTX_PN];
		if (attr)
			*info = nla_get_u64(attr);
	}

	return NL_SKIP;
}

void get_bctx_pn(void *priv, uint64_t *pn)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	msg = nl80211_drv_msg(drv, 0, NL80211_CMD_VENDOR);
	if (!msg || nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, CLSEMI_OUI)
		|| nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD,
			CLS_NL80211_CMD_GET_BCTX_PN)) {
		nlmsg_free(msg);
		wpa_printf(MSG_INFO, "%s: failed create nl_msg", __func__);

		return;
	}
	wpa_printf(MSG_INFO, "%s: send msg to drv", __func__);
	ret = send_and_recv_msgs(drv, msg, cls_vendor_txpn_handler, pn,
				 NULL, NULL);
	if (ret)
		return;
}
...
static int wpa_driver_set_multi_ap_vlan(void *priv, const char *ifname, u16 vlan, u8 del)
{
	struct i802_bss *bss = priv;
	int err = -1;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	struct bridge_vlan_info vinfo = {.vid = vlan, .flags = BRIDGE_VLAN_INFO_PVID};
	...
	msg = nlmsg_alloc_simple(del ? RTM_DELLINK : RTM_SETLINK, NLM_F_REQUEST);
	if (!msg)
		return -ENOMEM;
	...
	err = nl_wait_for_ack(drv->rtnl_sk);
	if (err < 0) {
		wpa_printf(MSG_DEBUG, "multi_ap_set vlan %d for %s(ifindex=%d) failed: %s",
			   vlan, ifname, index, nl_geterror(err));
	} else {
		wpa_printf(MSG_DEBUG, "multi_ap_set vlan %d for %s(ifindex=%d) ok",
			   vlan, ifname, index);
	}
...
#ifndef CLS_WIFI_CONFIG
	//cls do not use vlan for wds implementation
...
#ifdef CLS_WIFI_CONFIG
	.set_skip_dfs_cac = nl80211_set_skip_dfs_cac,
	.cancel_dfs_cac = nl80211_cancel_dfs_cac,
#endif
...
#endif /* CONFIG_TESTING_OPTIONS */
	.set_multi_ap_vlan = wpa_driver_set_multi_ap_vlan,
};
