...
setup_mld:
	...
	iface->ht_op_mode |= HT_PROT_NON_HT_MIXED;

#ifdef CONFIG_CAPWAP_IPC
	if (capwap_enable) {
		hostapd_wtp_event_socket_init();
		hostapd_wtp_event_send(HOSTAPD_WTP_INTERFACE_INIT, NULL);
		if (!hostapd_wtp_data_socket_init())
			wpa_printf(MSG_ERROR, "Failed to start hostapd data socket with WTP");
	}
#endif
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
#ifndef CONFIG_CAPWAP_IPC
		c = getopt(argc, argv, "b:Bde:f:hi:KP:sSTtu:g:G:qv::");
#else
		c = getopt(argc, argv, "b:Bde:f:hi:KP:sSTtu:g:G:CAqv::");
#endif
		if (c < 0)
			break;
		switch (c) {
