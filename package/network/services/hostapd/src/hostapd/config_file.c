...
#define VENDOR_SUBTYPE  0x00
#define MAX_PATH_LENGTH 256
#define MAX_CONTENT_LENGTH 16
#define MAX_CLS_IE_LEN 64

int cls_read_hw_plat(const char *phy_name)
{
	int result;

	if (strcmp(phy_name, "d2k") == 0) {
		result = 1;
	} else if (strcmp(phy_name, "m2k") == 0) {
		result = 2;
	} else if (strcmp(phy_name, "m3k") == 0) {
		result = 3;
	} else if (strcmp(phy_name, "m3kl") == 0) {
		result = 4;
	} else if (strcmp(phy_name, "d1k") == 0) {
		result = 0;
	} else {
		wpa_printf(MSG_ERROR, "Unknown hardware platform: %s\n", phy_name);
		result = -1;
	}

	return result;
}
static int build_tlv(uint8_t *out, uint8_t type, const void *data, uint8_t len)
{
	uint8_t *p = out;
	*p++ = type;
	*p++ = len;
...
static int build_capability_tlv(uint8_t *out, int s4, int enable_hop_limit,
		int hop_count, int hop_max)
{
	uint32_t capability = 0;

	if (enable_hop_limit) {
		capability |= (s4 & 0x1);                // bit0
		capability |= (hop_count & 0x3f) << 8;   // bit8-13
		capability |= (hop_max & 0x3f) << 14;    // bit14-19
	} else {
		capability |= (s4 & 0x1); // only set support 4addr bit
	}

	uint32_t cap_n = htonl(capability);

	return build_tlv(out, 0x01, &cap_n, 4);
}

int cls_build_vendor_ie(uint8_t *out, int chip_type, int enable_hop_limit,
		int hop_count, int hop_max, int s4)
{
	uint8_t *p = out;

	// IE header
	*p++ = 0xdd;              // Element ID
	uint8_t *len_pos = p++;   // Length, edit later
	uint32_t oui = htonl(CLSEMI_OUI);

	memcpy(p, ((uint8_t *)&oui) + 1, 3);  // lower 3 bytes
	p += 3;
	...
	*p++ = VENDOR_SUBTYPE;
	...
	// ---- TLV1: Chip type ----
	{
		uint8_t chip_type_n = (uint8_t)chip_type;

		p += build_tlv(p, 0x00, &chip_type_n, 1);
	}

	// ---- TLV2: Capability ----
	p += build_capability_tlv(p, s4, enable_hop_limit, hop_count, hop_max);
...
int cls_vendor_ie_gen(char *ie_buf, struct hostapd_config *conf, struct hostapd_bss_config *bss)
{
	int enable_hop_limit;
	int hop_count;
	int hop_max;
	int ie_len;

	wpa_printf(MSG_INFO, "cls_chip_type: %d", conf->cls_chip_type);
	if (!ie_buf)
		return -1;

	enable_hop_limit = bss->enable_hop_limit;
	hop_count = bss->hop_count;
	hop_max = bss->hop_max;

	ie_len = cls_build_vendor_ie(ie_buf, conf->cls_chip_type, enable_hop_limit, hop_count, hop_max, 1);
	wpa_hexdump(MSG_INFO, "cls vender ie:", ie_buf, ie_len);

	return ie_len;
}

int cls_append_vendor_ie(struct hostapd_config *conf, struct hostapd_bss_config *bss)
{
	char ie_buf[MAX_CLS_IE_LEN];
	int ie_len = 0;
	struct wpabuf *vbuf = bss->vendor_elements;

	ie_len = cls_vendor_ie_gen(ie_buf, conf, bss);
	if (ie_len > 0) {
		/*append cls_vendor_ie after*/
		int ret = wpabuf_resize(&bss->vendor_elements, ie_len);

		wpabuf_put_data(bss->vendor_elements, ie_buf, ie_len);
	}
...
#ifdef CONFIG_WAPI
int wapi_parse_cipher(const char *value)
{
	int val = 0, last;
	char *start, *end, *buf;
...
static int hostapd_config_parse_wapi_cipher(int line, const char *value)
{
	int val = wapi_parse_cipher(value);

	if (val < 0) {
		wpa_printf(MSG_ERROR, "Line %d: invalid wapi cipher '%s'.",
			   line, value);
		return -1;
	}
	if (val == 0) {
		wpa_printf(MSG_ERROR, "Line %d: no wapi cipher values configured.",
			   line);
		return -1;
	}
	return val;
...
static int hostapd_config_parse_wapi_key_mgmt(int line, const char *value)
{
	int val = 0, last;
	char *start, *end, *buf;
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if (os_strcmp(start, "WAPI_PSK") == 0)
			val |= WPA_KEY_MGMT_WAPI_PSK;
		else if (os_strcmp(start, "WAPI_CERT") == 0)
			val |= WPA_KEY_MGMT_WAPI_CERT;
		else {
			wpa_printf(MSG_ERROR, "Line %d: invalid wapi key_mgmt '%s'",
				   line, start);
			os_free(buf);
			return -1;
		}
	...
	os_free(buf);
	if (val == 0) {
		wpa_printf(MSG_ERROR, "Line %d: no wapi key_mgmt values "
			   "configured.", line);
		return -1;
	}
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		conf->radio_config_id = os_strdup(pos);
	} else if (os_strcmp(buf, "radio_config_file") == 0) {
		conf->radio_config_file = os_strdup(pos);
	...
	} else if (os_strcmp(buf, "disable_wps_per_dev_psk") == 0) {
		bss->disable_wps_per_dev_psk = atoi(pos);
	} else if (os_strcmp(buf, "correct_ap_pin_status_in_nack") == 0) {
		bss->correct_ap_pin_status_in_nack = atoi(pos);
	} else if (os_strcmp(buf, "wps_cred_save_conf") == 0) {
		bss->wps_cred_save_conf = atoi(pos);
#endif
...
#ifdef CONFIG_WAPI
	} else if (os_strcmp(buf, "wapi") == 0) {
		bss->wapi = atoi(pos);
	} else if (os_strcmp(buf, "wapi_preauth") == 0) {
		bss->wapi_pre_auth = atoi(pos);
	} else if (os_strcmp(buf, "wapi_key_mgmt") == 0) {
		bss->wapi_key_mgmt = hostapd_config_parse_wapi_key_mgmt(line, pos);
	} else if (os_strcmp(buf, "wapi_pairwise") == 0) {
		bss->wapi_pairwise = hostapd_config_parse_wapi_cipher(line, pos);
	} else if (os_strcmp(buf, "wapi_passphrase") == 0) {
		int len = os_strlen(pos);

		if (len < 8 || len > 63) {
			wpa_printf(MSG_ERROR, "Line %d: invalid WAPI passphrase length %d (expected 8..63)",
				   line, len);
			return 1;
		}
		os_free(bss->ssid.wapi_passphrase);
		bss->ssid.wapi_passphrase = os_strdup(pos);
#endif
#ifdef CLS_WIFI_CONFIG
	} else if (os_strcmp(buf, "smart_dfs_cac") == 0) {
		conf->smart_dfs_cac = atoi(pos);
	} else if (os_strcmp(buf, "smart_dfs_cac_duration") == 0) {
		conf->smart_dfs_cac_duration = atoi(pos);
#endif
	} else if (os_strcmp(buf, "sideband") == 0) {
		if (os_strcmp("above", pos) == 0)
			conf->secondary_channel = 1;
		else
			conf->secondary_channel = -1;
#if CLS_WIFI_CONFIG
	} else if (os_strcmp(buf, "enable_hop_limit") == 0) {
		int val = atoi(pos);

		if (val < 0) {
			wpa_printf(MSG_ERROR,
					"Line %d: invalid enable_hop_limit",
					line);
			return 1;
		}
		bss->enable_hop_limit = val;
	} else if (os_strcmp(buf, "hop_count") == 0) {
		int val = atoi(pos);

		if (val < 0) {
			wpa_printf(MSG_ERROR,
					"Line %d: invalid hop_count",
					line);
			return 1;
		}
		bss->hop_count = val;
	} else if (os_strcmp(buf, "hop_max") == 0) {
		int val = atoi(pos);

		if (val < 0) {
			wpa_printf(MSG_ERROR,
					"Line %d: invalid hop_max",
					line);
			return 1;
		}
		bss->hop_max = val;
	} else if (os_strcmp(buf, "cls_chip_type") == 0) {
		if (os_strlen(pos) > 16) {
			wpa_printf(MSG_ERROR, "Line %d: Too long cls_chip_type",
					line);
			return 1;
		}
		conf->cls_chip_type = cls_read_hw_plat(pos);
#endif
	} else {
		wpa_printf(MSG_ERROR,
...
#if CLS_WIFI_CONFIG
		cls_append_vendor_ie(conf, conf->bss[i]);
#endif
	}
