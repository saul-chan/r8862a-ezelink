...
#include "ap/vbss.h"
#include "common/cls_defines.h"
#ifdef CONFIG_WAPI
#include "ap/wapi_auth.h"
#endif
					     ...
					     char *buf, size_t buflen)
{
	...
	if (hapd->conf->correct_ap_pin_status_in_nack
			&& wps_ap_pin_failed_in_nack) {
		/* in AP PIN mode, wps status must be WPS_STATUS_FAILURE
		 * if its failure_reason is NO_ERROR, set result to Success
		 * */
		if (hapd->wps_stats.failure_reason == WPS_EI_NO_ERROR)
			ret = os_snprintf(pos, end - pos, "Last WPS result: Success\n");
		else
			ret = os_snprintf(pos, end - pos, "Last WPS result: Failed\n");
	}
	else
#endif
...
static int hostapd_ctrl_iface_set(struct hostapd_data *hapd, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (0) {
		...
		hostapd_ubus_notify_disallow_status(hapd);
					  ...
					  char *pos)
{
	...
	if (dfs_range || iface->cac_started) {
		if (iface->cac_started) {
			/* cancal CAC */
			hostapd_cancel_dfs_cac(iface);
			wpa_printf(MSG_ERROR, "%s(), %d, cancel CAC", __func__, __LINE__);
			iface->cac_started = 0;
		}
...
#ifdef CLS_WIFI_CONFIG
static int hostapd_cls_ctrl_iface_reconfigure(struct hostapd_data *hapd, char *reply, int reply_len)
{
	int ret = 0;

	ret = hostapd_reload_config(hapd->iface);
	if (ret == 0) {
		ret = os_snprintf(reply, reply_len, "%s\n", "OK");
...
static int hostapd_cls_ctrl_iface_create_bss(struct hostapd_data *hapd,
						char *buf, char *reply, int reply_len)
{
	int ret = 0;

	ret = hostapd_add_bss(hapd->iface, buf);
	if (ret == 0) {
		ret = os_snprintf(reply, reply_len, "%s\n", "OK");
		if (os_snprintf_error(reply_len, ret))
			ret = -1;
...
static int hostapd_cls_ctrl_iface_update_bss(struct hostapd_data *hapd,
						char *buf, char *reply, int reply_len)
{
	int ret = 0;

	ret = hostapd_update_bss(hapd->iface, buf);
	if (ret == 0) {
		ret = os_snprintf(reply, reply_len, "%s\n", "OK");
		if (os_snprintf_error(reply_len, ret))
			ret = -1;
...
static int hostapd_cls_ctrl_iface_channel_survey(struct hostapd_data *hapd,
						char *reply, int reply_len)
{
	int ret = 0;
	struct cls_channel_survey *channel_survey;

	if (sizeof(*channel_survey) > reply_len)
		return -1;

	channel_survey = (struct cls_channel_survey *)reply;
	os_memset(channel_survey, 0x00, sizeof(*channel_survey));

	channel_survey->freq = hapd->iface->freq;
	channel_survey->channel_utilization = hapd->iface->channel_utilization;
	channel_survey->bss_load_update_period = hapd->conf->bss_load_update_period;
	channel_survey->chan_util_avg_period = hapd->conf->chan_util_avg_period;

	return sizeof(*channel_survey);
}

static int hostapd_cls_ctrl_hidden_ssid(struct hostapd_data * hapd, char *buf, char *reply, int reply_len)
{
	int ret = 0;
	int hidden = 0;
	...
	hidden = *buf - '0';
	if (hapd->conf->ignore_broadcast_ssid != hidden) {
		hapd->conf->ignore_broadcast_ssid = hidden;
		wpa_printf(MSG_INFO, "%s:%s hide %d", __func__, hapd->conf->iface, hidden);
...
static int hostapd_ctrl_iface_cls_new_vap(struct hostapd_data *hapd,
				u8 *buf, char *reply, int reply_size)
{
	struct vbss_vap_info *vap = NULL;
	...
	vap = (struct vbss_vap_info *) buf;

	ret = hostapd_cls_ctrl_iface_create_bss(hapd,
						vap->ifname, reply, reply_size);
	if (ret > 0) {
		/*get the hostapd data with vap ifname*/
		bss_idx = hostapd_find_bss(hapd->iface, vap->ifname);
		wpa_printf(MSG_INFO, "%s:vap[%s] role:%d, bss_idx:%d",
					__func__, vap->ifname, vap->role, bss_idx);
		if (bss_idx != 0)
			hapd_data = hapd->iface->bss[bss_idx];
		if (hapd_data) {
			ret = vbss_set_hidden_ssid(hapd_data, vap->role);
			if (!ret)
				ret = vbss_set_hostapd_data_info(hapd_data, vap->role);
		}
	}
	return ret;
}
...
static int hostapd_ctrl_iface_cls_new_sta(struct hostapd_data *hapd,
				      u8 *buf, char *reply, int reply_size)
{
	int ret = 0;
	u8 addr[ETH_ALEN];
	struct sta_info *sta;
	struct vbss_sta_info *sta_info = NULL;
	struct hostapd_data *hapd_data = NULL;

	sta_info = (struct vbss_sta_info *) buf;
	memcpy(addr, sta_info->mac_addr, ETH_ALEN);
	wpa_printf(MSG_DEBUG, "CLS add_vbss_sta " MACSTR, MAC2STR(addr));

	hapd_data = get_hapd_by_bssid(hapd->iface,
					sta_info->bssid);

	if (hapd_data == NULL)
		ret = -1;
	else
		ret = vbss_add_sta(hapd_data, sta_info);

	return ret;
}


/* Handler of CLS ctrl iface
 * Input:
 *	hapd:		hostapd per-BSS data structure
 *	buf:		incoming cmd buffer, cmd formats is defined in macro CLS_CMD_xxx
 *	reply:		buffer to carry reply msg, params
 *	reply_len:	lenght of reply buffer
 * Return value:
 *	<0:	errors, hostapd_ctrl_iface_receive_process() will fill reply with "FAIL\n"
 *	=0:	OK, hostapd_ctrl_iface_receive_process() will fill reply with "OK\n"
 *	>0:	OK, and reply is already filled in CLS sub-handlers
 */
static int hostapd_ctrl_iface_cls(struct hostapd_data *hapd,
		char *buf, char *reply, size_t reply_len)
{
	int res;

	wpa_printf(MSG_DEBUG, "%s CLS %s", __func__, buf);
	if (os_strncmp(buf, CLS_CMD_GET_VBSS_STA, sizeof(CLS_CMD_GET_VBSS_STA) - 1) == 0) {
		res = vbss_get_sta_info(hapd, buf + sizeof(CLS_CMD_GET_VBSS_STA) - 1, reply, reply_len);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_ADD_VBSS_VAP, sizeof(CLS_CMD_ADD_VBSS_VAP) - 1) == 0) {
		res = hostapd_ctrl_iface_cls_new_vap(hapd, buf + sizeof(CLS_CMD_ADD_VBSS_VAP), reply, reply_len);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_DEL_VBSS_VAP, sizeof(CLS_CMD_DEL_VBSS_VAP) - 1) == 0) {
		res = vbss_del_vap(hapd, buf + sizeof(CLS_CMD_DEL_VBSS_VAP) - 1);
		return res;
	}  else if (os_strncmp(buf, CLS_CMD_GET_VBSS_VAP, sizeof(CLS_CMD_GET_VBSS_VAP) - 1) == 0) {
		res = vbss_get_vap_info(hapd, buf + sizeof(CLS_CMD_GET_VBSS_VAP) - 1, reply, reply_len);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_ADD_VBSS_STA, sizeof(CLS_CMD_ADD_VBSS_STA) - 1) == 0) {
		res = hostapd_ctrl_iface_cls_new_sta(hapd, buf + sizeof(CLS_CMD_ADD_VBSS_STA), reply, reply_len);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_DEL_VBSS_STA, sizeof(CLS_CMD_DEL_VBSS_STA) - 1) == 0) {
		res = vbss_del_sta(hapd, buf + sizeof(CLS_CMD_DEL_VBSS_STA) - 1);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_TRG_VBSS_SWITCH, sizeof(CLS_CMD_TRG_VBSS_SWITCH) - 1) == 0) {
		res = vbss_trigger_switch(hapd, buf + sizeof(CLS_CMD_TRG_VBSS_SWITCH) - 1);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_SET_VBSS_SWITCH_DONE, sizeof(CLS_CMD_SET_VBSS_SWITCH_DONE) - 1) == 0) {
		res = vbss_set_switch_done(hapd, buf + sizeof(CLS_CMD_SET_VBSS_SWITCH_DONE) - 1);
		return res;
	} else if (os_strncmp(buf, CLS_CMD_DISABLE_VBSS_REKEY, sizeof(CLS_CMD_DISABLE_VBSS_REKEY) - 1) == 0) {
		res = vbss_disable_rekey(hapd, buf + sizeof(CLS_CMD_DISABLE_VBSS_REKEY) - 1, reply, reply_len);
		return res;
	} else if (os_strcmp(buf, "GET_WPS_STATUS") == 0) {
		res = hostapd_cls_wps_get_status(hapd, reply, reply_len);
		return res;
	} else if (os_strcmp(buf, "WPS_CANCEL") == 0) {
		res = hostapd_wps_cancel(hapd);
		return res;
	} else if (os_strcmp(buf, "AP_WPS_PBC") == 0) {
		res = hostapd_wps_button_pushed(hapd, NULL);
		return res;
	} else if (os_strcmp(buf, "CHANNEL_SURVEY") == 0) {
		res = hostapd_cls_ctrl_iface_channel_survey(hapd, reply, reply_len);
		return res;
	} else {
		wpa_printf(MSG_ERROR, "%s() unknow command %s\n", __func__, buf);
	}
...
#ifdef CONFIG_WAPI
int is_wapid_data(struct hostapd_data *hapd, char *buf)
{
	u8 type = *((u8 *)buf);
	u8 reply = *((u8 *)buf + 1);
	...
	if (type >= COMMAND_WAPI_CONFIG && type <= EVENT_STA_WAPI_DEAUTH) {
		if (reply == 0 || reply == 1)
			return 1;
	}
	return 0;
...
int product_reply(u8 type, u8 dir, int result, char *reply)
{
	int len = 0;

	*reply = type;
	*(reply + 1) = dir;
	*(reply + 2) = WAPI_REPLY_LEN;
	*(reply + 3) = 0;
	*(reply + 4) = result;
	len = MINI_WAPI_DATA_LEN;
	return len;
}

void printf_buf(uint8_t *data, int len)
{
	int i = 0;
	char *strbuf = NULL;
	...
	for (i = 0; i < len; i++)
		os_snprintf(&strbuf[i * 3], 4, " %02x", data[i]);

	wpa_printf(MSG_INFO, "data is %s\n", strbuf);
	if (strbuf) {
		forced_memzero(strbuf, 1 + 3 * len);
		os_free(strbuf);
	}

}

int hostapd_handle_wapi_data(struct hostapd_data *hapd, u8 type, u8 dir, void *data, char *reply)
{
	int res = 0;
	struct wapi_key_info *key_info = NULL;
	struct sta_info *sta = NULL;
	struct wapi_reply *p_reply = NULL;
	struct sta_state *p_state = NULL;
	struct wapid_config *wapid_config;
	int result = 0;

	switch (type) {
	case COMMAND_WAPI_CONFIG:
		if (dir == MESSAGE_REQUEST) {
			// handle wapi config
			wapid_config = (struct wapid_config *)data;
			wpa_printf(MSG_INFO, "%s:get COMMAND_WAPI_CONFIG requet: ifname[%s],len=%d\n",
					__func__, wapid_config->iface, wapid_config->wie_len);

			printf_buf(wapid_config->wie, wapid_config->wie_len);

			if (0 == hostapd_get_config_from_wie(&hapd->wapi_auth->wapi_config,
					wapid_config->wie + 2, wapid_config->wie_len - 2)) {
				if (0 == hostapd_get_wie_from_wapid(hapd, wapid_config)) {
					ieee802_11_set_beacon(hapd);
					res = product_reply(type, MESSAGE_REPLY, result, reply);
				} else
					res = product_reply(type, MESSAGE_REPLY, !result, reply);
			} else {
				res = product_reply(type, MESSAGE_REPLY, !result, reply);
		...
		break;
	case COMMAND_STA_WAPI_SET_UNI_KEY:
		if (dir == MESSAGE_REQUEST) {
			key_info = (struct wapi_key_info *)data;
			wpa_printf(MSG_INFO, "%s:get COMMAND_STA_WAPI_SET_UNI_KEY requet, key_index[%d]\n",
				__func__, key_info->key_index);
			printf_buf(key_info->key, key_info->key_len);
			sta = ap_get_sta(hapd, key_info->sta_mac);
			if (sta)
				wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_PTK, (void *)key_info);
			res = product_reply(type, MESSAGE_REPLY, result, reply);
		}

		break;
	case COMMAND_STA_WAPI_SET_MULTI_KEY:
		if (dir == MESSAGE_REQUEST) {
			key_info = (struct wapi_key_info *)data;
			wpa_printf(MSG_INFO, "%s:get COMMAND_STA_WAPI_SET_MULTI_KEY requet key_indx[%d]\n",
				__func__, key_info->key_index);
			printf_buf(key_info->key, key_info->key_len);
			hostapd_wapi_auth_set_key(hapd, 0, WPA_ALG_SMS4,
							broadcast_ether_addr, key_info->key_index,
							key_info->key, key_info->key_len, KEY_FLAG_GROUP_TX_DEFAULT);
			res = product_reply(type, MESSAGE_REPLY, result, reply);
		}
		break;
	case EVENT_STA_HAPD_DEAUTH:
	case EVENT_STA_HAPD_ASSOC_SUCCESS:
	case EVENT_STA_HAPD_ASSOC_FAIL:
		if (dir == MESSAGE_REPLY) {
			p_reply = (struct wapi_reply *)data;
			wpa_printf(MSG_INFO, "%s:recevie type[%d] reply %s", __func__, type,
					!p_reply->status ? "success" : "fail");
		}

		break;
	case EVENT_STA_WAPI_CERT_AUTH_SUCCESS:
		if (dir == MESSAGE_REQUEST) {
			wpa_printf(MSG_INFO, "%s:get EVENT_STA_WAPI_AUTH_SUCCESS requet\n",
				__func__);
			p_state = (struct sta_state *)data;
			sta = ap_get_sta(hapd, p_state->sta_mac);
			if (sta)
				wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_CERT_SUCCESS, (void *)p_state);
			res = product_reply(type, MESSAGE_REPLY, result, reply);
		}
		break;
	case EVENT_STA_WAPI_DEAUTH:
		if (dir == MESSAGE_REQUEST) {
			wpa_printf(MSG_INFO, "%s:get EVENT_STA_WAPI_DEAUTH requet\n",
				__func__);
			p_state = (struct sta_state *)data;
			sta = ap_get_sta(hapd, p_state->sta_mac);
			if (sta)
				wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_AUTH_FAIL, (void *)p_state);
			res = product_reply(type, MESSAGE_REPLY, result, reply);
		}
		break;
	case EVENT_STA_WAPI_PORT_OPEN:
		if (dir == MESSAGE_REQUEST) {
			wpa_printf(MSG_INFO, "%s:get EVENT_STA_WAPI_PORT_OPEN requet\n",
				__func__);
			p_state = (struct sta_state *)data;
			sta = ap_get_sta(hapd, p_state->sta_mac);
			if (sta) {
				wpa_printf(MSG_INFO, "sta mac=" MACSTR "\n",
						MAC2STR(sta->addr));
				wapi_auth_sm_event(sta->wapi_sm, WAPI_EVENT_GTK, (void *)p_state);
			}
			res = product_reply(type, MESSAGE_REPLY, result, reply);
		}
		break;
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = hostapd_cls_ctrl_iface_reconfigure(hapd, reply, reply_size);
	} else if ((os_strncmp(buf, "CREATE_BSSCONFIG ", 17) == 0)) {
		reply_len = hostapd_cls_ctrl_iface_create_bss(hapd, buf + 17, reply, reply_size);
	} else if ((os_strncmp(buf, "UPDATE_BSSCONFIG ", 17) == 0)) {
		reply_len = hostapd_cls_ctrl_iface_update_bss(hapd, buf + 17, reply, reply_size);
	} else if ((os_strncmp(buf, "HIDDEN_SSID ", 12) == 0)) {
		reply_len = hostapd_cls_ctrl_hidden_ssid(hapd, buf + 12, reply, reply_size);
	} else if ((os_strncmp(buf, "CLS ", 4) == 0)) {
		res = hostapd_ctrl_iface_cls(hapd, buf + 4, reply, reply_size);
		if (res != 0)
			reply_len = res;
#endif
#ifdef CONFIG_WAPI
	} else if (is_wapid_data(hapd, buf)) {
		wpa_printf(MSG_INFO, "%s: handle wapid message typ[%d]\n", __func__, *buf);

		if (from->ss_family == AF_UNIX)
			wpa_printf(MSG_INFO, "family is %d, sun_path=%s", from->ss_family,
					((struct sockaddr_un *)from)->sun_path);


		hostapd_get_wapid_addr(hapd->wapi_auth, *buf, *(buf + 1), from, fromlen);

		res = hostapd_handle_wapi_data(hapd, *buf, *(buf + 1), buf + 4, reply);
		if (res != 0)
			reply_len = res;
		return res;
				       ...
				       void *sock_ctx)
{
	...
	set_reconf_ctrl_sock(sock, &from, fromlen);
	reply_len = hostapd_ctrl_iface_receive_process(hapd, pos,
						       reply, reply_size,
						       &from, fromlen);
	set_reconf_ctrl_sock(-1, NULL, 0);
...
#ifdef CLS_WIFI_CONFIG
/* Handler of CLS global ctrl iface
 * Input:
 *	interfaces:
 *	buf:		incoming cmd buffer, cmd formats is defined in macro CLS_CMD_xxx
 *	reply:		buffer to carry reply msg, params
 *	reply_len:	lenght of reply buffer
 * Return value:
 *	<0:	errors, hostapd_global_ctrl_iface_receive() will fill reply with "FAIL\n"
 *	=0:	OK, hostapd_global_ctrl_iface_receive() will fill reply with "OK\n"
 *	>0:	OK, and reply is already filled in CLS sub-handlers
 */
static int hostapd_global_ctrl_iface_cls(struct hapd_interfaces *interfaces,
		char *buf, char *reply, size_t reply_len)
{
	int res;
	int onoff;
	...
	if (os_strncmp(buf, CLS_CMD_GET_VBSS_ENABLED, sizeof(CLS_CMD_GET_VBSS_ENABLED) - 1) == 0) {
		res = vbss_get_enabled(buf, reply + sizeof(CLS_CMD_GET_VBSS_ENABLED) - 1, reply_len);

		return res;
	} else if (os_strncmp(buf, CLS_CMD_SET_VBSS_ENABLED, sizeof(CLS_CMD_SET_VBSS_ENABLED) - 1) == 0) {
		res = vbss_set_enabled(interfaces, buf + sizeof(CLS_CMD_SET_VBSS_ENABLED) - 1, reply, reply_len);
					      ...
					      void *sock_ctx)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		res = hostapd_global_ctrl_iface_cls(interfaces, buf + 4, reply, reply_size);
		if (res != 0)
			reply_len = res;
#endif
	} else {
