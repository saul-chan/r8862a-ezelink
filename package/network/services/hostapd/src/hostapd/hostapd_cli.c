...
static int hostapd_cli_cmd_update_bssconfig(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc == 0) {
		printf("Invalid 'update_bss' command - need to specify the name"
		       " of BSS.\n");
		return -1;
	}
...
static int hostapd_cli_cmd_channel_survey(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char buf[128];

	if (snprintf(buf, sizeof(buf), "CLS CHANNEL_SURVEY") > sizeof(buf)) {
		printf("parameter too long\n");
		return -1;
	}
...
#endif /* ANDROID */
#ifdef CLS_WIFI_CONFIG
	{"reconfigure", hostapd_cli_cmd_reconfigure, NULL,
	  "= reconfig the current radio config"},
	{"create_bss", hostapd_cli_cmd_create_bssconfig, NULL,
	  "<ifname> = create a new bss which name is ifname"},
	{"update_bss", hostapd_cli_cmd_update_bssconfig, NULL,
	  "<ifname> = update the bss which name is ifname"},
	{"channel_survey", hostapd_cli_cmd_channel_survey, NULL,
	  "<ifname> = get channel survey info. which name is ifname"},
