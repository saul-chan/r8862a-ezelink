...
hostapd_common_add_device_config() {
	...
	config_add_int smart_dfs_cac smart_dfs_cac_duration
	config_add_int rts
	config_add_int rssi_reject_assoc_rssi
	config_add_int rssi_ignore_probe_request
	config_add_int maxassoc

	config_add_string acs_chan_bias acs_freqlist
...
hostapd_get_cls_chip_type() {
	local phy="$1"
	local chip_type=$(cat /sys/kernel/debug/ieee80211/$phy/cls_wifi/hw_plat | awk '{print $1}')
...
hostapd_prepare_device_config() {
	...
	json_get_vars country country3 country_ie beacon_int:100 doth require_mode legacy_rates \
		...
		rts beacon_rate rssi_reject_assoc_rssi rssi_ignore_probe_request maxassoc \
		smart_dfs_cac smart_dfs_cac_duration acs_freqlist \
		mbssid:0
	...
	[ -n "$acs_freqlist" ] && append base_cfg "freqlist=$acs_freqlist" "$N"
	append base_cfg "cls_chip_type=$(hostapd_get_cls_chip_type "$phy")" "$N"
	...
	[ -n "$rts" ] && append base_cfg "rts_threshold=$rts" "$N"
	[ "$airtime_mode" -gt 0 ] && append base_cfg "airtime_mode=$airtime_mode" "$N"
	[ -n "$maxassoc" ] && append base_cfg "iface_max_num_sta=$maxassoc" "$N"
	[ "$mbssid" -gt 0 ] && [ "$mbssid" -le 2 ] && append base_cfg "mbssid=$mbssid" "$N"

	[ -n "$smart_dfs_cac" ] && append base_cfg "smart_dfs_cac=$smart_dfs_cac" "$N"
	if [ -n "$smart_dfs_cac" ] && [ -n "$smart_dfs_cac_duration" ]; then
		append base_cfg "smart_dfs_cac_duration=$smart_dfs_cac_duration" "$N"
	fi
...
hostapd_common_add_bss_config() {
	...
	config_add_boolean disassoc_low_ack short_preamble skip_inactivity_poll
	...
	config_add_string eap_type ca_cert client_cert identity anonymous_identity auth priv_key priv_key_pwd ca_cert2 client_cert2 priv_key2 priv_key2_pwd
	...
	config_add_int force_3addr_mode

	config_add_int update_config

	config_add_boolean wps_pushbutton wps_label ext_registrar wps_pbc_in_m1
	config_add_int wps_ap_setup_locked wps_independent disable_wps_per_dev_psk wps_cred_save_conf correct_ap_pin_status
	config_add_string wps_device_type wps_device_name wps_manufacturer wps_pin wps_state wps_uuid
...
hostapd_create_vlan_bridge() {
	local br_vlan_name="br-hapdvlan.$1"
	local vlan_ifname=$2

	if [ -n "$(uci show network | grep "@device.*name='$br_vlan_name'")" ]; then
		echo "vlan device $br_vlan_name is already created"
		return
	fi
	...
	uci -q batch <<-EOF >/dev/null
		...
		set network.@device[-1].name='${br_vlan_name}'
		set network.@device[-1].ports='${vlan_ifname}'
	EOF
}

hostapd_create_vlan_interface() {
	local vid=$1
	local vlan_network="HAPDVLAN$vid"
	local br_vlan_name="br-hapdvlan.$vid"
	local ipaddr="192.168.$vid.1"
	local netmask="255.255.255.0"

	if [ -n "$(uci -q get network.$vlan_network)" ]; then
		echo "network interface $vlan_network is already created"
		return
	fi

	NETWORK_CHANGED=1
	uci -q batch <<-EOF >/dev/null
		set network.${vlan_network}=interface
		set network.${vlan_network}.proto=static
		set network.${vlan_network}.device='${br_vlan_name}'
		set network.${vlan_network}.ipaddr='${ipaddr}'
		set network.${vlan_network}.netmask='${netmask}'
	EOF
}

hostapd_create_firewall_zone() {
	local vlan_network="HAPDVLAN$1"

	if [ -n "$(uci show firewall | grep "@zone.*name='$vlan_network'")" ]; then
		echo "firewall zone for $vlan_network is already created"
		return
	fi
	...
	uci -q batch <<-EOF >/dev/null
		add firewall zone
		set firewall.@zone[-1].name='${vlan_network}'
		set firewall.@zone[-1].input=ACCEPT
		set firewall.@zone[-1].output=ACCEPT
		set firewall.@zone[-1].forward=ACCEPT
		add_list firewall.@zone[-1].network='${vlan_network}'
	EOF
}

hostapd_create_firewall_forwarding() {
	local vlan_network="HAPDVLAN$1"

	if [ -n "$(uci show firewall | grep "@forwarding.*src='$vlan_network'")" ]; then
		echo "firewall forwarding for $vlan_network is already created"
		return
	fi
	...
	uci -q batch <<-EOF >/dev/null
		...
		set firewall.@forwarding[-1].src='${vlan_network}'
		set firewall.@forwarding[-1].dest=wan
	EOF
}

hostapd_create_vlan_dhcp() {
	local vid=$1
	local network="HAPDVLAN$vid"

	if [ -n "$(uci -q get dhcp.$network)" ]; then
		echo "dhcp for $network is already created"
		return
	fi
	...
	uci -q batch <<-EOF
		...
		set dhcp.$network.start=100
		set dhcp.$network.limit=150
		set dhcp.$network.leasetime=12h
		set dhcp.$network.dhcpv6=server
		set dhcp.$network.ra=server
	EOF
}

hostapd_del_vlan_br_in_network() {
	local ifname
	local section

	for ifname in $(uci show network | grep '^network\.HAPDVLAN.*device' | awk -F "'" '{print $2}');
	do
		section=$(uci show network | grep "@device.*name='$ifname'" | awk -F '.' '{print $2}')
...
hostapd_del_vlan_if_in_network() {
	local ifname
...
hostapd_del_zone_in_firewall() {
	local zone
	local section

	for zone in $(uci show firewall | grep "@zone.*name='HAPDVLAN" | awk -F "'" '{print $2}');
	do
		section=$(uci show firewall | grep "@zone.*name='$zone'" | awk -F '.' '{print $2}')
		uci delete firewall.$section
		FIREWALL_CHANGED=1
	done
}

hostapd_del_forwarding_in_firewall() {
	local section
	local forwarding

	for forwarding in $(uci show firewall | grep "@forwarding.*src='HAPDVLAN" | awk -F "'" '{print $2}');
	do
		section=$(uci show firewall | grep "@forwarding.*src='$forwarding'" | awk -F '.' '{print $2}')
		uci delete firewall.$section
		FIREWALL_CHANGED=1
	done
}

hostapd_del_dhcp_in_dhcp() {
	local dhcpname

	for dhcpname in $(uci show dhcp | grep '^dhcp\.HAPDVLAN.*interface' | awk -F "'" '{print $2}');
	do
...
hostapd_set_vlan_file() {
	...
	hostapd_create_vlan_bridge $vid "${ifname}-${name}"
	hostapd_create_vlan_interface $vid

	#create firewall for VLAN
	hostapd_create_firewall_zone $vid
	hostapd_create_firewall_forwarding $vid

	#create dhcp for VLAN
	hostapd_create_vlan_dhcp $vid
}
...
hostapd_set_vlan() {
	...
	#delete all created bridge and interface for VLAN
	hostapd_del_vlan_br_in_network
	hostapd_del_vlan_if_in_network

	#delete all created zone and forwarding in firewall
	hostapd_del_zone_in_firewall
	hostapd_del_forwarding_in_firewall

	#delete all created dhcp
	hostapd_del_dhcp_in_dhcp

	#set vlan file in loop
	for_each_vlan hostapd_set_vlan_file ${ifname}

	#commit and apply
	[ $NETWORK_CHANGED -eq 1 ] && uci commit network
	[ $FIREWALL_CHANGED -eq 1 ] && uci commit firewall
	[ $DHCP_CHANGED -eq 1 ] && uci commit dhcp

	[ $NETWORK_CHANGED -eq 1 ] && ubus call service event '{ "type": "config.change", "data": { "package": "network" }}'
	[ $FIREWALL_CHANGED -eq 1 ] && ubus call service event '{ "type": "config.change", "data": { "package": "firewall" }}'
	[ $DHCP_CHANGED -eq 1 ] && ubus call service event '{ "type": "config.change", "data": { "package": "dhcp" }}'
}
...
hostapd_set_sae_mpsklist() {
	local sae_mpsk="$1"
	append bss_conf "$sae_mpsk" "$N"
}
...
# Generate a RFC 4122 version 5 UUID from MAC address using MD5
# Returns:
#   UUID string in 8-4-4-4-12 format
gen_wps_uuid() {
	local mac_addr=$(cat /sys/class/ieee80211/phy0/addresses)

	# NSID bytes (RFC 4122 namespace identifier)
	nsid_hex="526480f8c99b4be5a65558ed5f5d6084"

	# Remove any colons from MAC address
	mac_hex=$(echo "$mac_addr" | tr -d ':')

	# Calculate MD5 hash of concatenated binary data from
	# two hex strings ($nsid_hex and $mac_hex)
	# Convert $nsid_hex from hex to binary:
	# - Replace spaces with \x (e.g., "a1 b2" -> "\xa1\xb2")
	# - Interpret escape sequences with %b
	hash=$(
		printf "%s%s" \
		"$(printf "%b" "\\x${nsid_hex// /\\x}")" \
		"$(printf "%b" "\\x${mac_hex// /\\x}")" \
		| md5sum | awk '{print $1}'
		)

	# Take first 16 bytes (32 hex chars) as UUID
	uuid=$(echo "$hash" | cut -c 1-32)

	# Set version bits (4 MSB of byte 6 to 0101 for version 5)
	byte6_hex=$(echo "$uuid" | cut -c 13)
	byte6_dec=$(( 0x$byte6_hex & 0x0f | 0x50 ))
	byte6_hex_new=$(printf "%x" "$byte6_dec")
	uuid=$(echo "$uuid" | sed "s/^\(.\{12\}\)$byte6_hex/\1$byte6_hex_new/")

	# Set variant bits (2 MSB of byte 8 to 10 per RFC 4122)
	byte8_hex=$(echo "$uuid" | cut -c 17-18)
	byte8_dec=$(( 0x$byte8_hex & 0x3f | 0x80 )) # 0x80 = 10xxxxxx
	byte8_hex_new=$(printf "%02x" "$byte8_dec")
	uuid=$(echo "$uuid" | sed "s/^\(.\{16\}\)$byte8_hex/\1$byte8_hex_new/")

	# Format as standard UUID (8-4-4-4-12)
	printf "%s-%s-%s-%s-%s" \
		"$(echo "$uuid" | cut -c 1-8)" \
		"$(echo "$uuid" | cut -c 9-12)" \
		"$(echo "$uuid" | cut -c 13-16)" \
		"$(echo "$uuid" | cut -c 17-20)" \
		"$(echo "$uuid" | cut -c 21-32)"
}
...
hostapd_set_bss_options() {
	...
	local wps_possible wps_not_configured
	...
	json_get_vars \
		...
		wps_independent disable_wps_per_dev_psk wps_cred_save_conf wps_device_type wps_device_name wps_manufacturer \
		wps_pin wps_state correct_ap_pin_status wps_uuid \
		macfilter ssid utf8_ssid wmm uapsd hidden short_preamble rsn_preauth \
		iapp_interface eapol_version dynamic_vlan ieee80211w nasid \
		acct_secret acct_port acct_interval \
		bss_load_update_period chan_util_avg_period sae_require_mfp sae_pwe wps_cred_add_sae \
		multi_ap multi_ap_profile multi_ap_vlanid multi_ap_backhaul_ssid multi_ap_backhaul_key skip_inactivity_poll \
	...
	set_default ppsk 0
	set_default multi_ap_profile 1
	set_default multi_ap_vlanid 0
	...
	[ "$max_listen_interval" -gt 0 ] && append bss_conf "max_listen_interval=$max_listen_interval" "$N"
	...
	case "$auth_type" in
		sae|owe|eap2|eap192|wpa3-eap-128)
			set_default ieee80211w 2
			set_default sae_require_mfp 1
			set_default sae_pwe 2
			set_default wps_cred_add_sae 1
		;;
		psk-sae|eap-eap2)
	...
	case "$auth_type" in
		...
		psk|sae|psk-sae)
			...
			[ "$auth_type" = "psk" -o "$auth_type" = "psk-sae" ] && wps_possible=1
		;;
		eap|eap2|eap-eap2|eap192|wpa3-eap-128)
			json_get_vars \
	...
	case "$auth_type" in
		sae|psk-sae)
			json_get_vars sae_mpsklist
			json_for_each_item hostapd_set_sae_mpsklist sae_mpsklist
		;;
	esac
	...
	[ "$multi_ap" -gt 0 ] && append bss_conf "multi_ap_profile=$multi_ap_profile" "$N"
	[ "$multi_ap_vlanid" -gt 0 ] && append bss_conf "multi_ap_vlanid=$multi_ap_vlanid" "$N"
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		set_default disable_wps_per_dev_psk 1
		set_default correct_ap_pin_status 1

		set_default wps_state 2

		# in OPEN or OWE mode, if WPS is enabled,
		# wps_state must be 'not configured'
		...
		[ -z "$wps_uuid" -a "$iftype" = "interface" ] && wps_uuid=$(gen_wps_uuid)
		...
		append bss_conf "disable_wps_per_dev_psk=$disable_wps_per_dev_psk" "$N"
		append bss_conf "correct_ap_pin_status_in_nack=$correct_ap_pin_status" "$N"

		[ -n "$wps_cred_save_conf" ] && append bss_conf "wps_cred_save_conf=$wps_cred_save_conf" "$N"
	...
	if [ "$ieee80211k" -eq "1" ]; then
		...
		[ "$bss_load_update_period" -eq 0 ] && bss_load_update_period=60
	else
	...
	[ "$bss_load_update_period" -gt 0 ] && append bss_conf "bss_load_update_period=$bss_load_update_period" "$N"
	[ "$chan_util_avg_period" -gt 0 ] && append bss_conf "chan_util_avg_period=$chan_util_avg_period" "$N"
		...
		case "$ieee80211w" in
			[012])
				...
				[ "$ieee80211w" -gt "0" ] && {
					...
					elif [ "$auth_type" = "wpa3-eap-128" ]; then
						append bss_conf "group_mgmt_cipher=AES-128-CMAC" "$N"
					else
...
wpa_supplicant_add_network() {
	...
	set_default multi_ap 0
	set_default multi_ap_profile 1
	set_default force_3addr_mode 0
	set_default default_disabled 0
	set_default wps_cred_add_sae 1
	set_default update_config 1
	...
	[ "$_w_mode" = "sta" ] && {
		...
		[ "$multi_ap_profile" -gt 0 ] && append network_data "multi_ap_profile=${multi_ap_profile}" "$N$T"
		[ "$force_3addr_mode" -gt 0 ] && append network_data "force_3addr_mode=${force_3addr_mode}" "$N$T"
	...
	[ "$wpa_cipher" = GCMP-256 ] && {
		append network_data "pairwise=GCMP-256" "$N$T"
		append network_data "group=GCMP-256" "$N$T"
	}
	...
	[ -n "$wps_cred_add_sae" ] && echo "wps_cred_add_sae=$wps_cred_add_sae" >> "$_config"
	echo "update_config=$update_config" >> "$_config"
