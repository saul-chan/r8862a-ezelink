...
function iface_freq_info(iface, config, params)
{
	...
	hostapd.printf(`iface_freq_info: freq ${freq} sec_offset ${sec_offset} width ${width}`);
	return hostapd.freq_info(freq, sec_offset, width);
}


function iface_freq_info_directly(iface, config, params)
{
	hostapd.printf(`iface_freq_info_directly: params ${params}`);
	...
	let width = 0;
	if (params.chan_width == 160)
		width = 2;
	else if (params.chan_width == 80)
		width = 1;
	else
		width = 0;


	hostapd.printf(`iface_freq_info_directly: freq ${freq} sec_offset ${sec_offset} width ${width}`);
...
let main_obj = {
	...
	apsta_state: {
		...
		call: ex_wrap(function(req) {
			...
			let freq_info = iface_freq_info_directly(iface, config, req.args);
			if (!freq_info)
				return libubus.STATUS_UNKNOWN_ERROR;
