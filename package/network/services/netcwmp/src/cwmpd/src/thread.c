...
#endif
{
    return cwmp_agent_run_tasks(cwmp);
}
...
//    CreateThread(NULL, 0, cwmp_worker_thread_tasks, cwmp, 0, &tid2);
    ...
    pthread_create(&th2, NULL, (void*)cwmp_worker_thread_udpd, cwmp);
    pthread_create(&th3, NULL, (void*)cwmp_worker_thread_tasks, cwmp);
#endif
    cwmp_event_listen(cwmp);
    cwmp_agent_session(cwmp);
}
