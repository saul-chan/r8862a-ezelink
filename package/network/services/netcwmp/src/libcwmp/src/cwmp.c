...
/* supported list of SOAP-ENV when parsing message, case-insensitive,
 * if ACS's SOAP-ENV is neither SOAP_ENV_DEFAULT, nor not in the list,
 * you can:
 *	option #1 (preferred): support it in uci config netcwmp.cwmp.soap_env
 *	option #2: add it in following list */
static char SOAP_ENV_PARSE_LIST[SOAP_ENV_PARSE_NUM][CWMP_NAME_MAX] = {
	"soap",			/* appeared in TR-069 spec */
	"soapenv",
	"soap env",
	"env",			/* for SOAP 1.2 @ https://www.w3.org/TR/soap12-part1/ */
};
static char SOAP_ENV_HEADER_PARSE_LIST[SOAP_ENV_PARSE_NUM][CWMP_NAME_MAX] = {{0}};
static char SOAP_ENV_BODY_PARSE_LIST[SOAP_ENV_PARSE_NUM][CWMP_NAME_MAX] = {{0}};

const char empty_string[] = "";
const char *soap_bool_allowed_values[] = {"0", "1", "true", "false"};
...
//	TYPE_BASE64,	//base
    ...
    {
        return TYPE_STRING128;
    }
    else if(! TRstrcasecmp(type, "s256"))
    {
        return TYPE_STRING256;
    }
    else if(! TRstrcasecmp(type, "s512"))
    {
        return TYPE_STRING512;
    }
    else if(! TRstrcasecmp(type, "s1024"))
    {
        return TYPE_STRING1024;
    }
 ...
 *	0:	fail
 */
static int value_check_by_type(const cwmp_byte_t type, const char *value)
{
	if (type == TYPE_BOOLEAN) {
		int i;
		for (i=0; i<(sizeof(soap_bool_allowed_values)/sizeof(soap_bool_allowed_values[0])); i++)
			if (strcasecmp(value, soap_bool_allowed_values[i]) == 0)
				return 1;
		return 0;
	}
	else if (TYPE_STRING16 <= type && type <= TYPE_STRING1024) {
		if (strlen(value) > (16 << (type - TYPE_STRING16)))
			return 0;
	}
	else if (type == TYPE_UNSIGNEDINT) {
		if (atoi(value) < 0)
			return 0;
	}
...
xmlnode_t * cwmp_xml_create_child_node(env_t * env ,  xmlnode_t * parentNode, const char * ns, const char * nodeName, const char * nodeValue)
{
    ...
    else
    {
        ...
        if (nodeValue == NULL || strcmp(nodeValue, "") == 0)
            XmlNodeSetNodeValue(pool , (xmlnode_t *)newNode, NULL );
        else
...
void cwmp_set_envelope_ns(const char * envstr, const char * encstr)
{
    ...
    for (i=0; i<SOAP_ENV_PARSE_NUM; i++) {
        if (SOAP_ENV_PARSE_LIST[i][0]) {
            TRsnprintf(SOAP_ENV_HEADER_PARSE_LIST[i], CWMP_NAME_MAX, "%s:%s", SOAP_ENV_PARSE_LIST[i], SOAP_XML_HEADER);
            TRsnprintf(SOAP_ENV_BODY_PARSE_LIST[i], CWMP_NAME_MAX, "%s:%s", SOAP_ENV_PARSE_LIST[i], SOAP_XML_BODY);
            cwmp_log_debug("header_list[%d]='%s' body_list[%d]='%s'", i, SOAP_ENV_HEADER_PARSE_LIST[i], i, SOAP_ENV_BODY_PARSE_LIST[i]);
        }
    }
}
...
int cwmp_get_object_instance(char *name)
{
	char fullpath[CWMP_FULL_PATH] = {0};
	int instance = -1;
	char *last_dot = NULL;
	char *prelast_dot = NULL;
	...
	*last_dot = '\0';
	...
	prelast_dot = strrchr(fullpath, '.');
	if(prelast_dot == last_dot)
		return -1;
	prelast_dot++;
	instance = atoi(prelast_dot);
	cwmp_log_debug("get instance %d for object %s\n", instance, name);
	return instance;
}
...
xmlnode_t * cwmp_get_header_node(xmldoc_t *  doc)
{
    ...
    if (node == NULL)
    {
        int i=0;
        for (i=0; i<SOAP_ENV_PARSE_NUM; i++) {
            if (SOAP_ENV_HEADER_PARSE_LIST[i][0] && (node = cwmp_xml_get_child_with_name(root, SOAP_ENV_HEADER_PARSE_LIST[i])))
                return node;
        }
...
xmlnode_t * cwmp_get_body_node(xmldoc_t *  doc)
{
    ...
    if (node == NULL)
    {
        int i=0;
        for (i=0; i<SOAP_ENV_PARSE_NUM; i++) {
            if (SOAP_ENV_BODY_PARSE_LIST[i][0] && (node = cwmp_xml_get_child_with_name(root, SOAP_ENV_BODY_PARSE_LIST[i])))
                return node;
        }
...
parameter_node_t * cwmp_get_parameter_node(parameter_node_t * root, const char * param_name)
{
    ...
    while (*dot)
    {
        ...
        {
            if (TRstrcmp(node->name, CWMP_INDEX) == 0 && is_digit(name) == 0)
            {
                break;
            }
...
parameter_node_t * cwmp_get_parameter_path_node(parameter_node_t * parent, const char * param_name)
{
    ...
    parameter_node_t *index_node = NULL;
    const char * dot;
    char  name[256];
    ...
    while (*dot)
    {
        ...
        if (!param_node && !index_node)
        {
            return NULL;
        }
        if(index_node && !index_node->next_sibling)
        {
            return index_node;
        }
        if ((dot) && (*dot == 0))
...
int cwmp_set_parameter_node_value(cwmp_t * cwmp, parameter_node_t * node, const char * name, const char * value, int value_length)
{
    ...
    if (value_check_by_type(node->type, value) == 0)
        return CWMP_ERROR;
    if (node->set)
    {
...
static int cwmp_getparammetervalue(env_t *env, const char *path_name, const int is_partial_path,
	parameter_node_t *param_node, parameter_list_t **ppl, fault_code_t *fault)
{
	int rc = CWMP_ERROR;
	...
	if (param_node->get) {
		rc = (*param_node->get)(env->cwmp, path_name, &value, pool);
		parameter = cwmp_create_parameter(env, path_name, value, TRstrlen(value), param_node->type);
	}
	else {
		parameter = cwmp_create_parameter(env, path_name, param_node->value, param_node->value_length, param_node->type);
		rc = CWMP_OK;
	}
	...
	if (rc == FAULT_CODE_OK) {
		if (parameter) {
			...
			*((*ppl)->parameters + (*ppl)->count++) = parameter;
		}
	}
	//else if (is_partial_path && rc == FAULT_CODE_9005) {
	else if (is_partial_path) {
		/* if query on object, only reply values for supported params */
		rc = CWMP_OK;
	}
...
static int cwmp_getvalues_from_node(env_t *env, const char *path_name, const int is_partial_path,
	parameter_node_t *param_node, parameter_list_t **ppl, fault_code_t *fault)
{
	int rc = CWMP_OK;
	char new_path[256];
	...
	if (param_node->type != TYPE_OBJECT) {
		return cwmp_getparammetervalue(env, path_name, is_partial_path, param_node, ppl, fault);
	}
	...
	for (param_child = param_node->child; param_child; param_child = param_child->next_sibling) {
		...
		if (param_child->type == TYPE_OBJECT)
			snprintf(new_path, sizeof(new_path), "%s%s.", path_name, param_child->name);
		else
			snprintf(new_path, sizeof(new_path), "%s%s", path_name, param_child->name);

		rc = cwmp_getvalues_from_node(env, new_path, is_partial_path, param_child, ppl, fault);
		if (rc != CWMP_OK) {
			break;
		}
	}
	...
	*ppl = cwmp_create_parameter_list(env );
	...
	while (parameterNode) {
		...
		if (!name || !pn) {
			...
			rc = cwmp_getvalues_from_node(env, name, (pn->type == TYPE_OBJECT), pn, ppl, fault);
			if (rc)
				goto RETURN;
		}
    ...
    *ppl = cwmp_create_parameter_list(env);
    ...
    while (parameterNode)
    {
        ...
        else
        {
            ...
            if (value_check_by_type(pn->type, value) == 0) {
                parameter->fault_code = FAULT_CODE_9007;
            }
            else {
                if(pn->set)
                {
...
int cwmp_parse_deleteobject_message(env_t * env , xmldoc_t *doc, parameter_node_t * root, int* status, fault_code_t *fault)
{
    ...
    *status = 1;
    ...
    instance_num = cwmp_get_object_instance(object_name);
...
xmlnode_t * cwmp_create_event_node(env_t * env ,  xmlnode_t * parent, const event_list_t * eventlist)
{
    ...
    while (count < eventlist->count)
    {
        ...
        if (pe[count]->event == INFORM_MREBOOT || pe[count]->event == INFORM_MDOWNLOAD) //|| pe[count]->event == INFORM_BOOTSTRAP)
        {
            /* blank string, "", will set node value to be gabage, set NULL instead */
            if (strcmp(pe[count]->command_key, "") == 0)
...
xmldoc_t * cwmp_create_transfercomplete_message(env_t * env ,  header_t * header, event_code_t * evcode)
{
    ...
    ESA(node, cwmp_xml_create_child_node(env ,  rpcNode, NULL, "StartTime", TRitoa(evcode->start)));
    ESA(node, cwmp_xml_create_child_node(env ,  rpcNode, NULL, "CompleteTime", TRitoa(evcode->end)));
