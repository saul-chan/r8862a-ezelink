...
static void cwmp_append_param_to_env(cwmp_session_t * session, pool_t * pool, parameter_list_t * pl,
	int node_cnt, const char *lvl1_name, const char *lvl2_name, const char *lvl3_name, cwmp_type_t param_type)
{
	char * name;
	char * value;
	parameter_t * parameter;

	name	= CWMP_APPEND_PARAMETER_NAME(pool, node_cnt, lvl1_name, lvl2_name, lvl3_name);
	value	= cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
	parameter = cwmp_create_parameter(session->env,  name, value, 0, param_type);
...
parameter_list_t * cwmp_session_create_inform_parameters(cwmp_session_t * session, pool_t * pool)
{
    ...
    char * name;
    char * value;
    char * rootName = session->cwmp->root->name;
...
#if 0 /* This parameter is DEPRECATED */
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 2, rootName, DeviceSummaryModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
#endif

    /* common parameters among TR-098 and TR-181 */
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, ManufacturerModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, ManufacturerOUIModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, ProductClassModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, SerialNumberModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
    ...
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, SpecVersionModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, HardwareVersionModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, SoftwareVersionModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, DeviceInfoModule, ProvisioningCodeModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, ConnectionRequestURLModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name,	value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, ParameterKeyModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


#if 0 //Username and Password should NOT send to ACS
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, UsernameModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, PasswordModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, ConnectionRequestUsernameModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);


    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, ConnectionRequestPasswordModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
#endif
    ...
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, URLModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, AliasBasedAddressing);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_BOOLEAN);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, PeriodicInformEnableModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_BOOLEAN);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name	= CWMP_APPEND_PARAMETER_NAME(pool, 3, rootName, ManagementServerModule, PeriodicInformIntervalModule);
    value	= cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_UNSIGNEDINT);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    /* UDPConnReq:1 Profile */
    /* This profile only applies to Internet Gateway Devices that are acting as CPE behind a NAT gateway as described
    in [Annex G/TR-069], and implies support for all of the CPE requirements defined within Annex G. */
    if (session->cwmp->stun.state == CWMP_STUN_STATE_RUNNING) {
        cwmp_log_debug("[STUN] add stun params\n");
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, UDPConnectionRequestAddressModule, TYPE_STRING256);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNEnableModule, TYPE_BOOLEAN);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNServerAddressModule, TYPE_STRING256);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNServerPortModule, TYPE_UNSIGNEDINT);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNUsernameModule, TYPE_STRING256);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNPasswordModule, TYPE_STRING256);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNMaxKeepAlivePeriod, TYPE_INT);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, STUNMinKeepAlivePeriod, TYPE_INT);
        cwmp_append_param_to_env(session, pool, pl, 3, rootName, ManagementServerModule, NATDetectedModule, TYPE_BOOLEAN);
    }

    if (strncmp(rootName, ROOT_DATA_MODEL_TR181, strlen(ROOT_DATA_MODEL_TR181)) == 0) {
        name    = CWMP_APPEND_PARAMETER_NAME(pool, 2, rootName, DMTR181Param_DeviceRootDMVer);
        value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
        parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
        cwmp_add_parameter_to_list(session->env,  pl, parameter);
    }
    else if (strncmp(rootName, ROOT_DATA_MODEL_TR098, strlen(ROOT_DATA_MODEL_TR098)) == 0) {
        name    = cwmp_conf_pool_get(pool, "cwmp:wan_interface");
        if (name == NULL || TRstrcmp(name, "") == 0 ) {
            cwmp_log_debug("cwmp:wan_interface name is: [%s]\n", name);
            name    = CWMP_APPEND_PARAMETER_NAME(pool, 8, rootName, WANDeviceModule, "1", WANConnectionDeviceModule, "1", WANIPConnectionModule, "1", ExternalIPAddressModule);
        }
...
int cwmp_session_handle_informresponse_message(cwmp_session_t * session, xmldoc_t * doc,  pool_t * pool)
{
	cwmp_t * cwmp = session->cwmp;
	event_global_t *evt_global = &(cwmp->event_global);
	int need_save = 0;

	/* clear events confirmed in InformResponse */
	if (evt_global->event_flag & EVENT_REBOOT_BOOTSTRAP_FLAG) {
		cwmp_event_set_value(cwmp, INFORM_BOOTSTRAP, 0, NULL, 0, 0, 0);
		need_save = 1;
	}

	if (evt_global->event_flag & EVENT_REBOOT_ACS_FLAG) {
		cwmp_event_set_value(cwmp, INFORM_MREBOOT, 0, NULL, 0, 0, 0);
		need_save = 1;
	}
...
int cwmp_session_handle_transfercompleteresponse_message(cwmp_session_t * session, xmldoc_t * doc,  pool_t * pool)
{
	cwmp_t * cwmp = session->cwmp;
	event_global_t *evt_global = &(cwmp->event_global);

	/* clear "7 TRANSFER COMPLETE" after transfercompleteresponse */
	if (evt_global->event_flag & EVENT_REBOOT_TRANSFERCOMPLETE_FLAG) {
		cwmp_event_set_value(cwmp, INFORM_MDOWNLOAD, 0, NULL, 0, 0, 0);
		cwmp_event_set_value(cwmp, INFORM_TRANSFERCOMPLETE, 0, NULL, 0, 0, 0);
		cwmp_event_save_global(cwmp);
	}
...
xmldoc_t *  cwmp_session_create_reboot_response_message(cwmp_session_t * session, xmldoc_t * doc, pool_t * pool)
{
    ...
    cwmp_t * cwmp = session->cwmp;
    char *cmd_key = (char *)MALLOC(COMMAND_KEY_LEN + 1);
    if (cmd_key) {
        if (key) {
            strncpy(cmd_key, key, COMMAND_KEY_LEN);
            cmd_key[COMMAND_KEY_LEN] = '\0';
        }
        else
            cmd_key[0] = '\0';
    }

    queue_push(cwmp->queue, (void *)cmd_key, TASK_REBOOT_TAG);
...
#if 0 /* challenge-response scheme requires Challenge goes before Response */
...
{
    int respcode, parse_status = CWMP_ERROR;
    ...
    if(respcode != HTTP_200)
    {
        ...
        if(response->status == 401 ||response->status == 407)
        {
            ...
            if(auth)
            {
                ...
                parse_status = http_parse_digest_auth(auth, &session->dest->auth, &session->dest->auth_type);
                if (parse_status == CWMP_OK) {
    ...
    else
    {
        ...
        if (session->dest->auth.state == AUTH_STATE_TX_CHALLENGE_RESPONSE)
            session->dest->auth.state = AUTH_STATE_AUTHORIZED;
    }
    ...
    if(respcode == HTTP_200 || parse_status == CWMP_OK)
    {
        return CWMP_OK;
    }
    else
