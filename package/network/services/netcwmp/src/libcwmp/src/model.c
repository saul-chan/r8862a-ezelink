...
int cwmp_model_create_child_parameter(parameter_node_t * parent_param, parameter_node_t * child_param, xmlnode_t * child_node, model_func_t * func_list, int func_count, pool_t * pool)
{
    //	FUNCTION_TRACE();
    ...
    while(next_node)
    {
        ...
        next_param->parent = parent_param;
...
int cwmp_model_create_parameter(parameter_node_t * param, xmlnode_t * node, model_func_t * func_list, int func_count,  pool_t * pool)
{
    ...
    cwmp_model_create_child_parameter(param, child_param, child_node, func_list, func_count,  pool);
...
int cwmp_model_get_index(const char *param_name, int *param_index)
{
	...
	strcpy(fullpath, param_name);
	param_path = fullpath;
	sub_node = strtok(param_path, path_delim);

	while ((sub_node = strtok(NULL, path_delim))) {
		if (is_digit(sub_node) == 0) {
			*param_index = atoi(sub_node);
			ret = FAULT_CODE_OK;
			break;
		}
	}
...
int cwmp_model_get_index_by_level(const char *param_name, int *param_index, int level)
{
	char fullpath[CWMP_FULL_PATH] = {0};
	const char path_delim[] = ".";
	char *param_path = NULL;
