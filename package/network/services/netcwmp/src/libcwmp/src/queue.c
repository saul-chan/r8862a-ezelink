...
void queue_push(queue_t *q, void * data, int type) {
	...
	if(type != TASK_PERIODIC_INFORM){
		cwmp_log_debug("send condition signal to task thread");
		pthread_cond_signal(&task_cond);
	}
...
void queue_wait(queue_t *q, unsigned int interval){
	struct timeval now;
	struct timespec timeout;

	gettimeofday(&now, NULL);
	if(interval == 0){
		timeout.tv_sec = now.tv_sec + atoi(TR181_VALUE_MGNTSRV_PERIOD_INFORM_INTERVAL);
	}else{
		timeout.tv_sec = inform_time.tv_sec + interval;
	}
	timeout.tv_nsec = now.tv_usec * 1000;
	pthread_mutex_lock(&q->mutex);
	pthread_cond_timedwait(&task_cond, &q->mutex, &timeout);
	pthread_mutex_unlock(&q->mutex);
}

void queue_cond_sig(queue_t *q) {
	pthread_mutex_lock(&q->mutex);
	pthread_cond_signal(&task_cond);
	pthread_mutex_unlock(& q->mutex);
}
