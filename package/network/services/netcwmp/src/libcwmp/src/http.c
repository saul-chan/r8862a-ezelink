...
void http_sockaddr_set(http_sockaddr_t * addr, int family, int port, const char * host)
{
    ...
    if (host)
    {
        ...
        if (he == NULL) {
            cwmp_log_error("ERROR: can't resolve host %s\n", host);
            return;
        }

        //inet_aton(host, &addr->sin4.sin_addr);
        memcpy(&addr->sin4.sin_addr, he->h_addr_list[0], sizeof(addr->sin4.sin_addr));
    }
    else
    {
        addr->sin4.sin_addr.s_addr = htonl(INADDR_ANY);
    }
}
                ...
                char * response)
{
    ...
    if (TRstrcasecmp(qop, "auth") == 0 || TRstrcasecmp(qop, "auth-int") == 0 ) {
...
int http_calc_basic_response(const char *user, const char *pwd, char *response)
{
    char input[MIN_BUFFER_LEN] = {0};
    char *str;

    TRsnprintf(input, MIN_BUFFER_LEN, "%s:%s", user, pwd);
    str = base64_encode(input);
    TRstrncpy(response, str, strlen(str));
    return CWMP_OK;
}
...
int http_write_request(http_socket_t * sock , http_request_t * request, cwmp_chunk_t * chunk, pool_t * pool)
{
    ...
    const char * header_fmt =
        ...
        "Content-Length: %d\r\n"
        ;
    const char * auth_fmt = "Authorization: Digest username=\"%s\",realm=\"%s\",cnonce=\"%s\",nonce=\"%s\",uri=\"%s\",qop=\"%s\",nc=%s,response=\"%s\"\r\n";
    //qop=%s, nc=%s, cnonce=\"%s\"
    const char *basic_auth_fmt = "Authorization: Basic %s\r\n";
    ...
    len1 = TRsnprintf(buffer, HTTP_DEFAULT_LEN, header_fmt,
        ...
        len2);
    //if(len2 > 0)
    /*the client will send blank content request after received authorization response "HTTP/1.1 200 OK"*/
    /*server can't identify it without digest body,so remove the length limit*/

    //if((dest->auth.active == CWMP_FALSE) && (dest->auth_type == HTTP_DIGEST_AUTH) &&
    if((dest->auth_type == HTTP_DIGEST_AUTH) &&(dest->auth.state == AUTH_STATE_RX_CHALLENGE_REQUEST))
    {
        if (TRstrlen(dest->auth.nc) == 0)
        {
            strcpy(dest->auth.nc, "00000001");
        }
        else
        {
            nc = strtol(dest->auth.nc, NULL, 16);
            nc++;
            snprintf(dest->auth.nc, sizeof(dest->auth.nc) - 1, "%08x", nc);
        }
        ...
        len1 += TRsnprintf(buffer + len1, HTTP_DEFAULT_LEN - len1, auth_fmt,
            dest->user,
            dest->auth.realm, dest->auth.cnonce, dest->auth.nonce,
            dest->auth.uri, dest->auth.qop, dest->auth.nc, dest->auth.response
            //dest->auth.cnonce
            );
        dest->auth.state = AUTH_STATE_TX_CHALLENGE_RESPONSE;
        /*send follow-up requests with http.authorization:Digest part after received authorization response ¡±HTTP/1.1 200 OK¡± */
    }
    if ((dest->auth_type == HTTP_BASIC_AUTH) &&(dest->auth.state == AUTH_STATE_RX_CHALLENGE_REQUEST))
    {
        http_calc_basic_response(dest->user, dest->password, dest->auth.response);
        len1 += TRsnprintf(buffer + len1, HTTP_DEFAULT_LEN - len1, basic_auth_fmt, dest->auth.response);
        dest->auth.state = AUTH_STATE_TX_CHALLENGE_RESPONSE;
    }
