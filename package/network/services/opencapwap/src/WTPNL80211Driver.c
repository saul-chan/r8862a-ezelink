...
//	NLA_PUT_STRING(msg, NL80211_ATTR_WIPHY_NAME, "phy1");
	...
	if( ret != 0)
	{
		CWLog("[NL80211 ERROR] Get phy info for %s error: %d, %s", singlePhyInfo->phyName, ret, strerror(-ret));
		return CW_FALSE;
	}
...
CWBool nl80211CmdGetInterface(int indexPhy, WTPInterfaceInfo * interfaceInfo){
	...
	genlmsg_put(msg, 0, 0, globalNLSock.nl80211_id, 0, 0, NL80211_CMD_GET_INTERFACE, 0);
	//NLA_PUT_U32(msg, NL80211_ATTR_WIPHY, gPhyInterfaceIndex[indexPhy]);
	index = if_nametoindex(interfaceInfo->ifName);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, index);

	ret = nl80211_send_recv_cb_input(&(globalNLSock), msg, CB_getInterfaceInfo, interfaceInfo);
	if( ret != 0)
	{
		CWLog("[NL80211 ERROR] Get interface for %s error: %d, %s", interfaceInfo->ifName, ret, strerror(-ret));
		return CW_FALSE;
	}
	...
	CWLog("[NL80211 INFO] Get interface %s successfully", interfaceInfo->ifName);
	return CW_TRUE;
...
CWBool nl80211CmdGetAuthType(char *interface, int *authtype){
	...
	int ret = nl80211_send_recv_cb_input(&(globalNLSock), msg, CB_getAuthTypeInterface, authtype);
	if( ret != 0) {
		CWLog("[NL80211 ERROR] Get auth type error: %d, %s", ret, strerror(-ret));
		return CW_FALSE;
	}
