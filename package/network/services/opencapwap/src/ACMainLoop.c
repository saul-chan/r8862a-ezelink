...
void CWACManageIncomingPacket(CWSocket sock,
	...
	CWDebugLog("CWACManageIncomingPacket: receive management packet from WTP");
/*
 ...
 */
	...
	if(wtpPtr != NULL) {
		/* known WTP */
		/* Clone data packet */
		CWDebugLog("receive management packet from knwon WTP");
		CW_CREATE_OBJECT_SIZE_ERR(pData, readBytes, { CWLog("Out Of Memory"); return; });
		memcpy(pData, buf, readBytes);
		...
		CWDebugLog("receive management packet from unknown WTP");
		...
		if(CWErr(CWParseDiscoveryRequestMessage(buf, readBytes, &seqNum, &values))) {
			 ...
			 */
			CWDebugLog("Assemble discovery response");
			...
			CWDebugLog("Send discovery response");
			...
			CWDebugLog("Not discovery request");
