...
void CW80211FrameProcess(WTPBSSInfo * WTPBSSInfoPtr, char *frameReceived, int frameLen)
{
...
void CW80211EventProcess(WTPBSSInfo * WTPBSSInfoPtr, int cmd, struct nlattr **tb, unsigned char * frameBuffer)
{
	...
	CW80211FrameProcess(WTPBSSInfoPtr, frameReceived, frameLen);
}
...
WTPSTAInfo * addSTABySA(WTPBSSInfo * WTPBSSInfoPtr, unsigned char * sa) {
	...
	for(indexSTA=0; indexSTA < WTP_MAX_STA; indexSTA++)
	{
		if(WTPBSSInfoPtr->staList[indexSTA].address == NULL || WTPBSSInfoPtr->staList[indexSTA].state == CW_80211_STA_OFF)
		{
			...
			WTPBSSInfoPtr->staList[indexSTA].eapol_conn.sock = 0;
			WTPBSSInfoPtr->staList[indexSTA].eapol_state = WPA_PTK_INITIALIZE;
...
CWBool delSTABySA(WTPBSSInfo * WTPBSSInfoPtr, unsigned char * sa) {
	...
	for(indexSTA=0; indexSTA < WTP_MAX_STA; indexSTA++)
	{
		if(WTPBSSInfoPtr->staList[indexSTA].address != NULL && !strcmp(WTPBSSInfoPtr->staList[indexSTA].address, sa))
		{
			...
			if (WTPBSSInfoPtr->staList[indexSTA].eapol_conn.sock > 0) {
				close(WTPBSSInfoPtr->staList[indexSTA].eapol_conn.sock);
				WTPBSSInfoPtr->staList[indexSTA].eapol_conn.sock = 0;
			}
			WTPBSSInfoPtr->staList[indexSTA].eapol_state = WPA_PTK_INITIALIZE;
			//Altro da liberare?
...
CWBool CWWTPHandleSTADisconnectEvent(unsigned char *addr) {
	WTPBSSInfo *WTPBSSInfoPtr;
	WTPSTAInfo *sta;
	int deleteRadioID;
	unsigned char deleteStaAddr[ETH_ALEN];

	WTPBSSInfoPtr = CWWTPGetBSSByStation(addr);
	sta = CWWTPGetStation(addr);

	if (!WTPBSSInfoPtr || !sta) {
		CWLog("No station found: "MACSTR, MAC2STR(addr));
		return CW_TRUE;
	}
	deleteRadioID = WTPBSSInfoPtr->phyInfo->radioID;
	CW_COPY_MEMORY(deleteStaAddr, addr, ETH_ALEN);

	if (sta->eapol_conn.sock > 0) {
		close(sta->eapol_conn.sock);
		sta->eapol_conn.sock = 0;
	}
	if(sta->radioAdd==CW_TRUE) {
		if(CWWTPDelStation(WTPBSSInfoPtr, sta))
		{
			CWPrintEthernetAddress(addr, "[Event HOSTAPD_WTP_STA_DISCONNECTED] STA deleted");
			CWWTPEventRequestDeleteStation(deleteRadioID, deleteStaAddr);
		}
		else
			CWPrintEthernetAddress(addr, "[Event HOSTAPD_WTP_STA_DISCONNECTED] STA NOT deleted");
	}
	return CW_TRUE;
}

CWBool CWHandleFrameMgmt(WTPBSSInfo * WTPBSSInfoPtr, char *frameReceived, int frameLen) {
	WTPSTAInfo * thisSTA;
	u64 cookie_out;
	int frameRespLen=0, offsetFrameReceived=0;
	short int fc, stateSTA = CW_80211_STA_OFF;
...
///		CWLog("[CW80211] TRY TO STA %02x:%02x:%02x:%02x:%02x:%02x", (int) authRequest.SA[0], (int) authRequest.SA[1], (int) authRequest.SA[2], (int) authRequest.SA[3], (int) authRequest.SA[4], (int) authRequest.SA[5]);
	...
	if (WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ASSOC_REQ)
	{
		...
		CWLog("ASSOCIATION REQUEST: searching STA");
		thisSTA = findSTABySA(WTPBSSInfoPtr, assocRequest.SA);
		if(thisSTA)
		{
			if(thisSTA->state == CW_80211_STA_AUTH || thisSTA->state == CW_80211_STA_ASSOCIATION)
		...
		CWLog("ASSOCIATION REQUEST: READ VALUES");
		thisSTA->capabilityBit = assocRequest.capabilityBit;
		thisSTA->listenInterval = assocRequest.listenInterval;
		thisSTA->lenSupportedRates = assocRequest.supportedRatesLen;
		thisSTA->extSupportedRatesLen = assocRequest.extSupportedRatesLen;
	...
	if (WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_REASSOC_REQ)
	{
		...
		if(thisSTA)
		{
			CWPrintEthernetAddress(assocRequest.SA, "[CW80211] Add Station");
			CWLog("[CW80211] pid:%d: Add Station:for interface: %s", WTPBSSInfoPtr->threadBSS, WTPBSSInfoPtr->interfaceInfo->ifName);
			if(thisSTA->state == CW_80211_STA_AUTH || thisSTA->state == CW_80211_STA_ASSOCIATION)
				thisSTA->state = CW_80211_STA_ASSOCIATION;
			else
			{
	...
	/* +++ Association Response +++ */
	if (WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ASSOC_RESP)
	{
		CWLog("[80211] ______ Association Response Received");
		struct CWFrameAssociationResponse assocResponse;
		if(!CW80211ParseAssociationResponse(frameReceived, &assocResponse))
		{
			CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL);
			return CW_FALSE;
		}

		CWPrintEthernetAddress(assocResponse.DA, "[CW80211] find Station");
		CWLog("[CW80211] pid:%d: find Station:for interface: %s", WTPBSSInfoPtr->threadBSS, WTPBSSInfoPtr->interfaceInfo->ifName);
		thisSTA = findSTABySA(WTPBSSInfoPtr, assocResponse.DA);
		if(thisSTA)
		{
			if(!thisSTA->state == CW_80211_STA_ASSOCIATION)
			{
				CWPrintEthernetAddress(assocResponse.DA, "[CW80211] STA hasn't send Assoc Request before receive Association Response");
				return CW_FALSE;
			}
		}
		else
		{
			CWPrintEthernetAddress(assocResponse.DA, "[CW80211] Problem adding STA");
			return CW_FALSE;
		}
		//Send Association Response Frame
		if(!CWSendFrameMgmtFromWTPtoAC(frameReceived, frameLen))
			return CW_FALSE;
		CWLog("send  association Response to AC successfully");
	}

	/* +++ Reassociation Response +++ */
	if (WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT && WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_REASSOC_RESP)
	{
		CWLog("[80211] ______ Reassociation Response Received");
		struct CWFrameAssociationResponse assocResponse;
		if(!CW80211ParseAssociationRequest(frameReceived, &assocResponse))
		{
			CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL);
			return CW_FALSE;
		}
		...
		if(thisSTA)
		{
			if(!thisSTA->state == CW_80211_STA_ASSOCIATION)
			{
				CWPrintEthernetAddress(assocResponse.DA, "[CW80211] STA hasn't send Assoc Request before receive Reassociation Response");
				return CW_FALSE;
			}
		}
		else
		...
		//Send Reassociation Response Frame
		if(!CWSendFrameMgmtFromWTPtoAC(frameReceived, frameLen))
			return CW_FALSE;
	}
...
CWBool CWWTPHandleEapol(char *frameReceived, int frameLen) {
	WTPSTAInfo *sta = NULL;
	char addr[ETH_ALEN] = {0};
	int offset = 0;
	int ret = -1;
	char buf[LEN_EAPOL_MAX] = {0};
	int len = 0;
	...
	if (sta->eapol_state == WPA_PTK_INITIALIZE) {
		sta->eapol_state = WPA_PTK_PTKSTART;
	}
	offset += ETH_ALEN;
	if (sta->eapol_conn.sock <= 0) {
		ret = wtp_eapol_connect(&(sta->eapol_conn), addr);
		if (ret)
			return CW_FALSE;
	}
	ret = wtp_send_recv_eapol(sta->eapol_conn.sock, &(frameReceived[offset]), frameLen - ETH_ALEN, &(buf[LEN_EAPOL_HEADER+ETH_ALEN]), &len);
	if (ret || len <= 0) {
		CWDebugLog("WTP send and receive eapol from hostapd error\n");
		return CW_FALSE;
	}

	CW_COPY_MEMORY(&(buf[0]), rfc1042_header, LEN_EAPOL_HEADER);
	CW_COPY_MEMORY(&(buf[LEN_EAPOL_HEADER]), addr, ETH_ALEN);
	frame.data_msgType = CW_IEEE_802_11_FRAME_TYPE;
	msglen = len + LEN_EAPOL_HEADER + ETH_ALEN;
	frame.msg = buf;
	frame.offset = msglen;
	CWDebugLog("Send eapol to AC\n");
	if(!CWSendFrameMgmtFromWTPtoAC(frame.msg, frame.offset))
		return CW_FALSE;
	CWDebugLog("Send eapol to AC successfully\n");

	if (sta->eapol_state == WPA_PTK_PTKSTART) {
		CWDebugLog("Set eapol to WPA_PTK_PTKCALCNEGOTIATING2\n");
		sta->eapol_state = WPA_PTK_PTKCALCNEGOTIATING2;
	} else {
		CWDebugLog("Close eapol socket in WTP\n");
		close(sta->eapol_conn.sock);
		sta->eapol_conn.sock = 0;
		sta->eapol_state = WPA_PTK_INITIALIZE;
	}
	return CW_TRUE;
}
