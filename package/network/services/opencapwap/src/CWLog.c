...
void CWDump(const u8 *buf, size_t len) {
	char *logStr = NULL;
	time_t now;
	char *nowReadable = NULL;
	int hlen = 0;
	...
	// return in case of memory err: we're not performing a critical task
	CW_CREATE_STRING_ERR(logStr, (3*len+strlen(nowReadable)+100), return;);
	sprintf(logStr, "[CAPWAP::%s]\t%08x\t hexdump(len=%lu)\n", nowReadable, (unsigned int)CWThreadSelf(), (unsigned long) len);
	if(gLogFile != NULL) {
#ifndef CW_SINGLE_THREAD
		CWThreadMutexLock(&gFileMutex);
		fseek(gLogFile, 0L, SEEK_END);
		...
		hlen = strlen(logStr);
		for (i = 0; i < len; i++)
			os_snprintf(&logStr[hlen + i * 3], 4, " %02x", buf[i]);
		fwrite(logStr, strlen(logStr), 1, gLogFile);
		sprintf(logStr, "\n");
		fwrite(logStr, strlen(logStr), 1, gLogFile);
		fflush(gLogFile);
		if(!checkResetFile()) {
