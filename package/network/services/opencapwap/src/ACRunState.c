...
CWBool CWACAssembleDataAndSend(int WTPIndex, char *data, int data_len) {
	CWProtocolMessage *eapolMsgPtr = NULL;
	...
	if(!CWAssembleDataMessage(&eapolMsgPtr,
		&fragmentsNum,
		...
		CW_PACKET_PLAIN
		,0)) {
		CWLog("Failure assembling eapol frame");
		ret = CW_FALSE;
		goto fail;
	}
	...
	for(i = 0; i < fragmentsNum; i++) {
		...
		if(!(CWSecuritySend(gWTPs[WTPIndex].sessionData, eapolMsgPtr[i].msg, eapolMsgPtr[i].offset)))
#else
		if(!CWNetworkSendUnsafeUnconnected(dataSocket, &(address), eapolMsgPtr[i].msg, eapolMsgPtr[i].offset))
#endif
		{
			CWLog("Failure sending eapol frame");
			ret = CW_FALSE;
			break;
		}
	}
	CWLog("Assemble and send data result: %d", ret);
fail:
	if(eapolMsgPtr) {
		for(i = 0; i < fragmentsNum; i++) {
			CW_FREE_PROTOCOL_MESSAGE(eapolMsgPtr[i]);
		}
	}
	CW_FREE_OBJECT(eapolMsgPtr);
	CW_FREE_PROTOCOL_MESSAGE(frame);
	return ret;
}

CWBool CWACHandleEapol(int WTPIndex, char *data) {
	char addr[ETH_ALEN] = {0};
	char *eapol3 = NULL;
	int eapol3_len = 0;
	CWBool ret = CW_TRUE;

	CW_COPY_MEMORY(addr, data, ETH_ALEN);

	eapol3 = CW80211AssembleEapol3AC(addr, NULL, 0, &eapol3_len);
	if (eapol3 == NULL)
		return CW_FALSE;
	ret = CWACAssembleDataAndSend(WTPIndex, eapol3, eapol3_len);
	return ret;
}

CWBool CWACAssembleEapol(int WTPIndex, char *addr) {
	char *eapol1 = NULL;
	int eapol1_len = 0;
	CWBool ret = CW_TRUE;

	eapol1 = CW80211AssembleEapol1AC(addr, NULL, 0, &eapol1_len);
	if (eapol1 == NULL)
		return CW_FALSE;
	ret = CWACAssembleDataAndSend(WTPIndex, eapol1, eapol1_len);
	return ret;
}
...
//Elena
		...
		/* Elena Agostini: 80211 Frame Management or Data */
			...
			if(isEAPOL_FrameNoHeader(msgPtr->msg)) {
				CWLog("CW80211: receive eapol message from wtp");
				if (isEAPOL4(&(msgPtr->msg[LEN_EAPOL_HEADER+ETH_ALEN+LEN_EAPOL_FIX_LEN+LEN_EAPOL_KEY_TYPE]))) {
					CWLog("CW80211: receive eapol4 message from wtp, done");
					return CW_TRUE;
				}
				if (!CWACHandleEapol(WTPIndex, &(msgPtr->msg[LEN_EAPOL_HEADER])))
					return CW_FALSE;
				return CW_TRUE;
			}
...
struct timeval tval_before, tval_after, tval_result;
				...
				if(subtype == WLAN_FC_STYPE_ASSOC_RESP)
				{
					...
					if(!CW80211ParseAssociationResponse(msgPtr->msg, &(assocResponse)))
						return CW_FALSE;
					CWACAssembleEapol(WTPIndex, assocResponse.DA);
					/* ... e poi inviare il Station Request ed inviarlo */
					int seqNum = CWGetSeqNum();
...
CWBool CWSaveChangeStateEventRequestMessage(CWProtocolChangeStateEventRequestValues *valuesPtr,
	...
	if((valuesPtr->radioOperationalInfo.radiosCount) >0) {
		...
		for(i=0; i<(valuesPtr->radioOperationalInfo.radiosCount); i++) {
			...
			if(!found) {
				CWErrorRaise(CW_ERROR_INVALID_FORMAT, "No radio found!");
				retValue= CW_FALSE;
			}
		}
	}
