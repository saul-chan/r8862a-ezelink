...
CWBool CWAssembleDiscoveryResponse(CWProtocolMessage **messagesPtr, int seqNum, WTPglobalPhyInfo * tmpPhyInfo) {
	...
	msgElemCount = msgElemCount + tmpPhyInfo->numPhyActive;
	...
	CWLog("Assemble Discovery Response");
...
CWBool CWParseDiscoveryRequestMessage(char *msg, 
	...
	*seqNumPtr = controlVal.seqNum;
	...
	CWDebugLog("Parse Discovery Request message elements");
...
*/
	CWDebugLog("Parse Discovery Request successfully");
	return CW_TRUE;
}
