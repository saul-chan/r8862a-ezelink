...
CWBool CWWTPGetRadioGlobalInfo(void) {
	...
	for(indexPhy=0; indexPhy < gRadiosInfo.radioCount; indexPhy++)
	{
		...
		CWWTPDefaultInitInterface(indexPhy);
		CWWTPInitInterfaceForRadio(indexPhy);
	}
...
CWBool CWWTPDefaultInitInterface(int indexPhy) {
	int indexWlan = 0;
	int BSSId = 0;

	for(indexWlan = 0; indexWlan < WTP_MAX_INTERFACES; indexWlan++) {
		gRadiosInfo.radiosInfo[indexPhy].gWTPPhyInfo.interfaces[indexWlan].ifName = NULL;
		gRadiosInfo.radiosInfo[indexPhy].gWTPPhyInfo.interfaces[indexWlan].SSID = NULL;
		gRadiosInfo.radiosInfo[indexPhy].gWTPPhyInfo.interfaces[indexWlan].BSSID = NULL;
	}
	return CW_TRUE;
}

CWBool CWWTPUpdateInterface(char *ifname) {
	int indexPhy;
	int indexWlan;
	char *ptr;
	...
	ptr = strchr(ifname, '_');
	if (ptr != NULL) {
		sscanf(ifname, "wlan%d-%d", &indexPhy, &indexWlan);
	} else {
		sscanf(ifname, "wlan%d", &indexPhy);
		indexWlan = 0;
	}
	...
	if (indexWlan == 0) {
		...
		if(!CWWTPGetWlanInterface(indexPhy,  indexWlan)) {
			CWLog("[NL80211 ERROR] Get WLAN interface. RadioID: %d, WLAN ID: %d", gPhyInterfaceIndex[indexPhy], indexWlan);
			CW_FALSE;
		}
		...
		if(!CWWTPipcHostapdThread(indexPhy, indexWlan, &(gRadiosInfo.radiosInfo[indexPhy].gWTPPhyInfo.interfaces[indexWlan])))
			return CW_FALSE;
	}
...
CWBool CWWTPInitInterfaceForRadio(int indexPhy) {
	int indexWlan;
...
CWBool CWWTPGetWlanInterface(int radioIndex, int wlanIndex)//WTPInterfaceInfo * interfaceInfo)
{
	//pid_t wtpPid = getpid();

	//Create ifname: WTPWlan+radioIndex+wlanIndex+WTPpid
	if (!gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].ifName)
	...
	if (wlanIndex == 0) {
		snprintf(gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].ifName, (WTP_NAME_WLAN_PREFIX_LEN+2), "%s%d", WTP_NAME_WLAN_PREFIX, gPhyInterfaceIndex[radioIndex]);
	} else {
		snprintf(gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].ifName, (WTP_NAME_WLAN_PREFIX_LEN+WTP_NAME_WLAN_SUFFIX_LEN+1), WIFI_IFNAME_FMT, WTP_NAME_WLAN_PREFIX, gPhyInterfaceIndex[radioIndex], wlanIndex);
	}

	if(!nl80211CmdGetInterface(radioIndex, &(gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex]))) {
		CW_FREE_OBJECT(gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].ifName);
		return CW_FALSE;
	}
	...
	if (gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].typeInterface == NL80211_IFTYPE_STATION) {
		gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].typeInterface = CW_STA_MODE;
	} else {
		gRadiosInfo.radiosInfo[radioIndex].gWTPPhyInfo.interfaces[wlanIndex].typeInterface = CW_AP_MODE;
	}
	//RFC wlanIndex > 0
...
CWBool CWWTPCreateNewBSS(int radioIndex, int wlanIndex)
{
	...
	if (WTPGlobalBSSList[BSSId] == NULL) {
		...
		WTPGlobalBSSList[BSSId]->staList = NULL;
	...
	if (WTPGlobalBSSList[BSSId]->staList == NULL)
...
WTPSTAInfo *CWWTPGetStation(unsigned char *addr)
{
	int indexPhy = 0;
	int indexWlan = 0;
	WTPSTAInfo *sta = NULL;
	int bssid = 0;
	WTPBSSInfo *WTPBSSInfoPtr = NULL;

	for(indexPhy=0; indexPhy < gRadiosInfo.radioCount; indexPhy++) {
		for(indexWlan=0; indexWlan < WTP_MAX_INTERFACES; indexWlan++) {
			bssid = getBSSIndex(indexPhy, indexWlan);
			WTPBSSInfoPtr = (WTPBSSInfo *)WTPGlobalBSSList[bssid];
			if ((WTPBSSInfoPtr != NULL) && (WTPBSSInfoPtr->active == CW_TRUE)) {
				sta = findSTABySA(WTPBSSInfoPtr, addr);
				if (sta != NULL)
					return sta;
			}
		}
...
WTPBSSInfo *CWWTPGetBSSByStation(unsigned char *addr)
{
	int indexPhy = 0;
	int indexWlan = 0;
	WTPSTAInfo *sta = NULL;
