...
/* Elena Agostini - 04/2014: DTLS Data Channel || DTLS Control Channel */
	...
	CWThread thread_event_with_wtp_hostapd;
	if (!CWErr(CWCreateThread(&thread_event_with_wtp_hostapd, wtp_event_server, NULL))) {
...
CWBool CWWTPInitConfiguration() {
	...
	if(CWWTPGetRadioGlobalInfo() == CW_FALSE)
		CWLog("WiFi interface isn't ready");
