...
CWBool CWParseSettingsFile()
{
	char *line = NULL;
	int indexPhy=0;
	char filename[CW_NAME_LEN] = {0};

	sprintf(filename, "%s/%s", gConfigFilePath, CW_SETTINGS_FILE);
	gSettingsFile = fopen (filename, "rb");
	...
	while((line = (char*)CWGetCommand(gSettingsFile)) != NULL) 
	{
		...
		*/
		...
		if (!strncmp(startTag+1, "WTP_HOSTAPD_AUTH_ENABLE", endTag-startTag-1))
		{
			char* startValue=NULL;
			char* endValue=NULL;
			int offset = 0;
			...
			strncpy(auth_str, startValue, offset);
			auth_str[offset] ='\0';
			gHostapd_auth_enable = atoi(auth_str);
			CWLog(": %d", gHostapd_auth_enable);
			CW_FREE_OBJECT(line);
			continue;
		}
		...
		if (!strncmp(startTag+1, "ECHO_INTERVAL", endTag-startTag-1))
		{
			char* startValue=NULL;
			char* endValue=NULL;
			int offset = 0;
			...
			strncpy(port_str, startValue, offset);
			port_str[offset] ='\0';
			gEchoInterval = atoi(port_str);
			CWLog("echo interval: %d",gEchoInterval);
			CW_FREE_OBJECT(line);
			continue;
		}
		if (!strncmp(startTag+1, "ECHO_RETRANSMIT_MAX", endTag-startTag-1))
		{
			char* startValue=NULL;
			char* endValue=NULL;
			int offset = 0;
			...
			strncpy(port_str, startValue, offset);
			port_str[offset] ='\0';
			gWTPMaxRetransmitEcho = atoi(port_str);
			CWLog("echo retransmit max: %d",gWTPMaxRetransmitEcho);
			CW_FREE_OBJECT(line);
			continue;
		}
		CW_FREE_OBJECT(line);
