...
{
	struct WTPInterfaceInfo *intfInfo = (struct WTPInterfaceInfo *) arg;
	struct nlattr *tb_msg[NL80211_ATTR_MAX + 1];
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
	...
	if(tb_msg[NL80211_ATTR_IFINDEX])
	{
		...
		intfInfo->realWlanID = nla_get_u32(tb_msg[NL80211_ATTR_IFINDEX]);
	}
	if(tb_msg[NL80211_ATTR_IFTYPE])
	{
		intfInfo->typeInterface = nla_get_u32(tb_msg[NL80211_ATTR_IFTYPE]);
	}
	if(tb_msg[NL80211_ATTR_AUTH_TYPE])
	{
		intfInfo->authType = nla_get_u32(tb_msg[NL80211_ATTR_AUTH_TYPE]);
	}
	if(tb_msg[NL80211_ATTR_SSID])
	{
		CWLog("[NL80211 INFO] interface SSID: %s", nla_get_string(tb_msg[NL80211_ATTR_SSID]));
		CW_CREATE_STRING_FROM_STRING_ERR(intfInfo->SSID, nla_get_string(tb_msg[NL80211_ATTR_SSID]), return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	return NL_SKIP;
}
...
int CB_getAuthTypeInterface(struct nl_msg *msg, void *arg)
{
	...
	if (tb_msg[NL80211_ATTR_AUTH_TYPE]) {
		*authtype = nla_get_u32(tb_msg[NL80211_ATTR_AUTH_TYPE]);
	}

	CWLog("authtype: %d", *authtype);
