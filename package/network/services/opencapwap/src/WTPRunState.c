			...
			if (msgPtr.data_msgType == CW_DATA_MSG_KEEP_ALIVE_TYPE) {
				...
				//	addr.sll_pkttype = PACKET_HOST;
					...
					if(isEAPOL_FrameNoHeader(msgPtr.msg)) {
						if (gHostapd_auth_enable) {
							CWLog("Recevie eapol from AC, send to hostapd");
							CWWTPHandleEapol(&(msgPtr.msg[LEN_EAPOL_HEADER]), msgPtr.offset - LEN_EAPOL_HEADER);
						}
					} else {
				...
				if((CWSaveIEEEConfigurationRequestMessage(&(interfaceACInfo)))) {
					...
					if(interfaceACInfo.operation == CW_OP_ADD_WLAN)
					{
						...
						)
						{
							...
							if (!gRadiosInfo.radiosInfo[radioIDindex].gWTPPhyInfo.interfaces[wlanIDindex].BSSID) {
								CW_CREATE_ARRAY_CALLOC_ERR(gRadiosInfo.radiosInfo[radioIDindex].gWTPPhyInfo.interfaces[wlanIDindex].BSSID, ETH_ALEN+1, char, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
								CW_COPY_MEMORY(gRadiosInfo.radiosInfo[radioIDindex].gWTPPhyInfo.interfaces[wlanIDindex].BSSID,
									gRadiosInfo.radiosInfo[radioIDindex].gWTPPhyInfo.interfaces[0].BSSID, ETH_ALEN);
								gRadiosInfo.radiosInfo[radioIDindex].gWTPPhyInfo.interfaces[wlanIDindex].BSSID[5] += 1;
							}
