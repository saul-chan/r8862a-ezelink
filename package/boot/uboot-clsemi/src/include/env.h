...
/**
 * sysinfo_env_get() - Look up the value of an system information variable
 *
 * In U-Boot proper this can be called before relocation (which is when the
 * sysinfo is loaded from storage, i.e. GD_FLG_SYSINFO_READY is 0). In that
 * case this function calls sysinfo_env_get_f().
 *
 * @varname:	Variable to look up
 * Return: value of variable, or NULL if not found
 */
char *sysinfo_env_get(const char *varname);

/**
 * sysinfo_env_get_f() - Look up the value of an system information variable (early)
 *
 * This function is called from sysinfo_env_get() if the sysinfo has not been
 * loaded yet (GD_FLG_SYSINFO_READY flag is 0). Some sysinfo locations will
 * support reading the value (slowly) and some will not.
 *
 * @varname:	Variable to look up
 * Return: actual length of the variable value excluding the terminating
 *	NULL-byte, or -1 if the variable is not found
 */
int sysinfo_env_get_f(const char *name, char *buf, unsigned int len);

/**
 * sysinfo_env_init() - Set up the pre-relocation system information variables
 *
 * This locates the system information or uses the default if nothing is available.
 * This must be called before env_get() will work.
 *
 * Return: 0 if OK, -ENODEV if no system information drivers are enabled
 */
int sysinfo_env_init(void);

/**
 * sysinfo_env_relocate() - Set up the post-relocation system information variables
 *
 * This loads the system information into RAM so that it can be modified. This is
 * called after relocation, before the system information variables is used
 */
void sysinfo_env_relocate(void);

/**
 * sysinfo_env_import() - Import from a binary representation into hash table
 *
 * This imports the system information variables from a buffer. The format 
 * for each variable is var=value\0 with a double \0 at the end of the buffer.
 *
 * @buf: Buffer containing the system information (struct environemnt_s *)
 * @check: non-zero to check the CRC at the start of the system information, 0 to
 *	ignore it
 * @flags: Flags controlling matching (H_... - see search.h)
 * Return: 0 if imported successfully, -ENOMSG if the CRC was bad, -EIO if
 *	something else went wrong
...
/**
 * sysinfo_env_export() - Export the system information variables to a buffer
 *
 * Export from hash table into binary representation
 *
 * @env_out: Buffer to contain the system information (must be large enough!)
 * Return: 0 if OK, 1 on error
 */
int sysinfo_env_export(struct environment_s *env_out);

/**
 * sysinfo_env_import_redund() - Select and import one of two redundant system information variables
 *
 * @buf1: First system information
 * @buf1_read_fail: 0 if buf1 is valid, non-zero if invalid
 * @buf2: Second system information
 * @buf2_read_fail: 0 if buf2 is valid, non-zero if invalid
 * @flags: Flags controlling matching (H_... - see search.h)
 * Return: 0 if OK, -EIO if no system information is valid, -ENOMSG if the CRC was bad
 */
int sysinfo_env_import_redund(const char *buf1, int buf1_read_fail,
		      const char *buf2, int buf2_read_fail,
		      int flags);

/* [re]set to the default system information variables */
void sysinfo_env_set_default(const char *s, int flags);

/**
 * sysinfo_load() - Load the system information variables from storage
 *
 * Return: 0 if OK, -ve on error
 */
...
/**
 * sysinfo_env_reload() - Re-Load the system information variables from current storage
 *
 * Return: 0 if OK, -ve on error
 */
...
/**
 * sysinfo_env_save() - Save the system information variables to storage
 *
 * Return: 0 if OK, -ve on error
 */
