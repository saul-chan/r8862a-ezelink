...
#ifdef CONFIG_SYS_REDUNDAND_ENVIRONMENT
# define SYSINFO_HEADER_SIZE    (sizeof(uint32_t) + 1)
#else
# define SYSINFO_HEADER_SIZE    (sizeof(uint32_t))
#endif

#define SYSINFO_SIZE (CONFIG_SYSINFO_SIZE - SYSINFO_HEADER_SIZE)

extern const char default_sysinfo[];

/* Declare a new system infomation location driver */
#define U_BOOT_SYSINFO_LOCATION(__name)                                 \
        ll_entry_declare(struct env_driver, __name, sysinfo_driver)

/* Declare the name of a location */
#ifdef CONFIG_CMD_SAVESYSINFO
#define SYSINFO_NAME(_name) .name = _name,
#else
#define SYSINFO_NAME(_name)
#endif

#ifdef CONFIG_CMD_SAVESYSINFO
#define sysinfo_save_ptr(x) x
#else
#define sysinfo_save_ptr(x) NULL
#endif

#define SYSINFO_SAVE_PTR(x)  (CONFIG_IS_ENABLED(CMD_SAVESYSINFO) ? (x) : NULL)
#define SYSINFO_ERASE_PTR(x) (CONFIG_IS_ENABLED(CMD_ERASESYSINFO) ? (x) : NULL)

extern struct hsearch_data sysinfo_htab;
#endif
