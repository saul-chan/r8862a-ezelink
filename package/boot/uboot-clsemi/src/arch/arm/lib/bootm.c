...
#endif
// 
static void add_cls_hw_ver(uint32_t ver)
{
#define HW_STR_LEN (20)
	char *bootargs_new;
	int len;
	char  hw_str[HW_STR_LEN] = {0};
	char *hex = &ver;
    ...
    snprintf(hw_str, HW_STR_LEN, " HW=C%02XV%02XP%02XS%02X",
			hex[3], hex[2], hex[1], hex[0]);

	len = strlen(bootargs) + strlen(hw_str) + 1;
	bootargs_new = malloc(len);
	if (!bootargs_new) {
		printf("%s malloc %d size buffer failed\n", __func__, len);
		return;
	}

	strcpy(bootargs_new, bootargs);
	strncat(bootargs_new, hw_str, len - strlen(bootargs_new));
