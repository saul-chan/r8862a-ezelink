...
static int do_ddr_diagstest(struct cmd_tbl *cmdtp, int flag, int argc,
                     char *const argv[])
{
	printf("perpare enter into ddr diagstest mode... \r\n");
	//Swith DDR Diagstest mode Next reset
	writel(0x5a5aa5a5, (0x90444000 + 0x420));
	writel(0x44445254, (0x90444000 + 0x424));
	//Set Golable Reset
	writel(0xa5a5a5a5, 0x90444200);
	writel(0x5a765a76, 0x90444200);
	while(1);
	return CMD_RET_SUCCESS;
}
...
U_BOOT_CMD(
	ddrtest,    1,      1,      do_ddr_diagstest,
	"dubhe1000 ddr daigs test",
	"[.b, .w, .l" HELP_Q "] "
);
