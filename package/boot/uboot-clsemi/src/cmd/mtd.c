...
extern void spinand_mtd_dump_features_reg(struct mtd_info *mtd);
static int do_mtd_features(struct cmd_tbl *cmdtp, int flag, int argc,
                       char *const argv[])
{
        struct mtd_info *mtd;
	...
	spinand_mtd_dump_features_reg(mtd);
	#endif
        return CMD_RET_SUCCESS;
}
...
static int spinand_debug_noecc_ooblayout_ecc(struct mtd_info *mtd, int section,
				       struct mtd_oob_region *region)
{
	return -ERANGE;
}

static int spinand_debug_noecc_ooblayout_free(struct mtd_info *mtd, int section,
					struct mtd_oob_region *region)
{
	if (section)
		return -ERANGE;
...
static const struct mtd_ooblayout_ops spinand_debug_noecc_ooblayout = {
	.ecc = spinand_debug_noecc_ooblayout_ecc,
	.rfree = spinand_debug_noecc_ooblayout_free,
};

static int do_mtd_io(struct cmd_tbl *cmdtp, int flag, int argc,
		     char *const argv[])
{
	bool dump, read, raw, woob, write_empty_pages, has_pages = false, noecc;
	...
	struct mtd_ooblayout_ops *spinand_ooblayout;
	u32 oob_len, mtd_oobavail;
	u8 *buf;
	int ret;
	...
	/* Debug Feature NO ECC RAW/OOB */
	noecc = strstr(cmd, ".noecc");
	if (noecc) {
		woob = true;
		/* save current mtd->oobavail and mtd->ooblayout */
		mtd_oobavail = mtd->oobavail;
		spinand = mtd_to_spinand(mtd);
		spinand_ooblayout = (struct mtd_ooblayout_ops *)spinand->eccinfo.ooblayout;
		/* eccinfo.ooblayout set to null for disable ECC*/
		spinand->eccinfo.ooblayout = NULL;
		if (read)
			mtd_set_ooblayout(mtd, &spinand_debug_noecc_ooblayout);
		else
...
out_put_mtd:
        if (noecc) {
                /* recover mtd->oobavail and mtd->ooblayout */
		mtd->oobavail = mtd_oobavail;
		spinand->eccinfo.ooblayout = spinand_ooblayout;
                mtd_set_ooblayout(mtd, spinand_ooblayout);
        }
	put_mtd_device(mtd);
...
static char mtd_help_text[] =
	...
	"mtd list\n"
	"mtd read[.raw][.oob][.noecc]                  <name> <addr> [<off> [<size>]]\n"
	"mtd dump[.raw][.oob][.noecc]                  <name>        [<off> [<size>]]\n"
	"mtd write[.raw][.oob][.noecc][.dontskipff]    <name> <addr> [<off> [<size>]]\n"
...
U_BOOT_CMD_WITH_SUBCMDS(mtd, "MTD utils", mtd_help_text,
		...
		U_BOOT_SUBCMD_MKENT_COMPLETE(bad, 2, 1, do_mtd_bad,
					     mtd_name_complete),
		U_BOOT_SUBCMD_MKENT_COMPLETE(feature, 2, 1, do_mtd_features,
					     mtd_name_complete));
