...
yt_phy_drv_func_t yt8531_drv_func =
{
    ...
    .phy_crossover_mode_set = yt8531_crossover_mode_set,
    .phy_crossover_mode_get = yt8531_crossover_mode_get,
    .phy_crossover_status_get = yt8531_crossover_status_get,
    .phy_loopback_set = yt8531_loopback_set,
};
...
yt_ret_t yt8531_crossover_mode_set(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_mode_t mode)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;

    CMM_ERR_CHK(HALSWDRV_FUNC(unit)->switch_extif_read(unit, phy_addr, PHY_FUNCTION_CTRL_REG_16, &phydata), ret);
    ...
    HALSWDRV_FUNC(unit)->switch_extif_write(unit, phy_addr, PHY_FUNCTION_CTRL_REG_16, phydata);
    yt8531_restart(unit, phy_addr);

    return CMM_ERR_OK;
}

yt_ret_t yt8531_crossover_mode_get(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_mode_t *pMode)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;
...
yt_ret_t yt8531_crossover_status_get(yt_unit_t unit, uint8_t phy_addr, yt_utp_crossover_status_t *pStatus)
{
    cmm_err_t ret = CMM_ERR_OK;
    uint16_t phydata = 0;
...
yt_ret_t yt8531_loopback_set(yt_unit_t unit, uint8_t phy_addr, yt_phy_loopback_mode_t mode)
{
    yt_ret_t ret;
    uint16_t data;
    ...
    switch(mode)
    {
        case YT_PHY_LOOPBACK_MODE_INTERNAL:
            CMM_ERR_CHK(HALSWDRV_FUNC(unit)->switch_extif_read(unit, phy_addr, PHY_BASE_CTRL_REG_0, &data), ret);
            SET_BIT(data, 14);
            CMM_ERR_CHK(HALSWDRV_FUNC(unit)->switch_extif_write(unit, phy_addr, PHY_BASE_CTRL_REG_0, data), ret);
            break;
        case YT_PHY_LOOPBACK_MODE_EXTERNAL:
            /* disable sleep */
            CMM_ERR_CHK(ext_phy_ext_reg_read(unit, phy_addr, 0x27, &data), ret);
            CLEAR_BIT(data, 15);
            CMM_ERR_CHK(ext_phy_ext_reg_write(unit, phy_addr, 0x27, data), ret);

            /* enable external loopback */
            CMM_ERR_CHK(ext_phy_ext_reg_read(unit, phy_addr, 0xa, &data), ret);
            SET_BIT(data, 4);
            CMM_ERR_CHK(ext_phy_ext_reg_write(unit, phy_addr, 0xa, data), ret);

            /* reset */
            CMM_ERR_CHK(yt8531_restart(unit, phy_addr), ret);

            break;
        case YT_PHY_LOOPBACK_MODE_REMOTE:
            CMM_ERR_CHK(ext_phy_ext_reg_read(unit, phy_addr, 0xa006, &data), ret);
            SET_BIT(data, 15);
            CMM_ERR_CHK(ext_phy_ext_reg_write(unit, phy_addr, 0xa006, data), ret);
