...
{
#define MIIM_YT8531_RX_DELAY		BIT(8)
	u16 reg;
#if (YT8531A_XTAL_INIT)
	int ret = 0;
	...
	phy_write(phydev, MDIO_DEVAD_NONE, 0x1e, 0xa001);

	/* enable RX-delay for rgmii-id and rgmii-rxid, otherwise disable it */
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 0x1f);
	if (phydev->interface == PHY_INTERFACE_MODE_RGMII_ID ||
			phydev->interface == PHY_INTERFACE_MODE_RGMII_RXID)
		reg |= MIIM_YT8531_RX_DELAY;
	else
		reg &= ~MIIM_YT8531_RX_DELAY;

	phy_write(phydev, MDIO_DEVAD_NONE, 0x1f, reg);
