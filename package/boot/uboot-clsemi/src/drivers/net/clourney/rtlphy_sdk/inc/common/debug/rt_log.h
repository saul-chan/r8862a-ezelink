...
#undef _RT_ERR
  ...
  //#define RT_LOG(level, module, args,...)                    do {osal_printf(args, ##__VA_ARGS__);printf("\n");} while(0)
  #define RT_LOG(level, module, args,...)                    do {} while(0)
  #define RT_ERR(error_code, module, fmt, args...)          do { osal_printf("function %s, line %d,Err:0x%x:\n"fmt,__FUNCTION__,__LINE__, error_code, ##args); } while (0)
