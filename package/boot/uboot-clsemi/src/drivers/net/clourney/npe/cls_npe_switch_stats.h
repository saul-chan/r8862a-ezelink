...
#ifndef _CLS_NPE_SWITCH_STATS_H_
#define _CLS_NPE_SWITCH_STATS_H_

#include "cls_npe.h"
#include "cls_npe_switch_conf.h"

#define CLS_NPE_SWITCH_PORT_MAX			6
...
struct cls_swicth_txpkt_table {
	u32 head;
	u32 tail;
};

struct cls_switch_ingress_stats {
...
struct cls_switch_ippdrop_stats {
	/*  unknown reasons */
	u32 unkonwn_ingress;
...
struct cls_switch_ip_stats { // per Port
...
struct cls_switch_preEppDrop_stats { //per Port
...
struct cls_switch_ipp_stats {
	struct cls_switch_ippdrop_stats ippDrop;
	struct cls_switch_smon_stats smon;
	...
	struct cls_switch_preEppDrop_stats preEppDrop;
	struct cls_switch_ip_stats ip;
	...
	u32 ipmOverflow;
	struct cls_swicth_txpkt_table ippTxPkt;

	u32 mmp;
};

struct cls_switch_shared_bm_stats {
	u32 erm[EGRESS_RESOURCE_MANAGER_DROP_MAX];
	u32 bmOverflow;
	u32 irm;
	struct cls_swicth_txpkt_table pbTxPkt;
};

struct cls_epppDrop_cnt {
...
struct cls_switch_epp_stats {
	/* epppDrop */
	struct cls_epppDrop_cnt epppDrop;
	...
	u32 epmOverflow;
	u32 rqOverflow;
	struct cls_swicth_txpkt_table eppTxPkt;
};

struct cls_switch_egress_stats {
	u32 psError_underrun[PS_ERROR_COUNTER_MAX];
	u32 psError_overflow[PS_ERROR_COUNTER_MAX];
	struct cls_swicth_txpkt_table psTxPkt;
...
struct cls_switch_stats {
	struct cls_switch_ingress_stats ingress_stats;
	struct cls_switch_ipp_stats ipp_stats;
	struct cls_switch_shared_bm_stats shared_bm_stats;
	struct cls_switch_epp_stats epp_stats;
	struct cls_switch_egress_stats egress_stats;
};

void cls_switch_stats_dump(struct cls_eth_priv *adapter, u8 option);
void cls_switch_clear_stats(struct cls_eth_priv *adapter);
#endif /* _CLS_NPE_SWITCH_STATS_H_ */
