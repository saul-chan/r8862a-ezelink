...
/* TX */
struct cls_xgmac_stats_param xgmac_tx_stats[] = {
	...
	{"Tx_Pause_Packets_Low", XGMAC_STATS_PARAM_TYPE_64_BIT, 0x894},
...
/* RX */
struct cls_xgmac_stats_param xgmac_rx_stats[] = {
...
void cls_xgmac_stats_dump_option(struct cls_eth_priv *adapter, u8 xgmac_index, u8 option)
{
	int i, size;
	void __iomem *address = adapter->xgmac[xgmac_index].ioaddr;
	char fieldname[64] = {0};
	struct cls_xgmac_stats_param *xgmac_stats;

	if (option == XGMAC_STATS_OPTION_TX) {
		size = sizeof(xgmac_tx_stats) / sizeof(struct cls_xgmac_stats_param);
		xgmac_stats = xgmac_tx_stats;
		printf("############Port%d XGMAC TX STATISTICS############\n", xgmac_index);
	} else if (option == XGMAC_STATS_OPTION_RX) {
		size = sizeof(xgmac_rx_stats) / sizeof(struct cls_xgmac_stats_param);
	...
	address = adapter->xgmac[xgmac_index].ioaddr;
			...
			u8 xgmac_index, u8 option)
{
	...
	if (option == XGMAC_STATS_OPTION_TX || option == XGMAC_STATS_OPTION_RX) {
		cls_xgmac_stats_dump_option(adapter, xgmac_index, option);
	} else {
		cls_xgmac_stats_dump_option(adapter, xgmac_index, XGMAC_STATS_OPTION_TX);
		cls_xgmac_stats_dump_option(adapter, xgmac_index, XGMAC_STATS_OPTION_RX);
	}
	printf("#######################################################\n");
}
