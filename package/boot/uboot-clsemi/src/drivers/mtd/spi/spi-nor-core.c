...
static int disable_quad_bit(struct spi_nor *nor)
{
	int ret;
	u8 tmp_val = 0;
	...
	if (ret < 0) {
		dev_dbg(nor->dev,
			"error while reading Status Register2\n");
		return -EINVAL;
	}

	if (ret & CR_QUAD_EN_SPAN) {
		tmp_val = (ret & ~CR_QUAD_EN_SPAN);

		ret = write_sr2(nor, tmp_val);
		if (ret)
			return ret;
		...
		if (ret > 0 && (ret & CR_QUAD_EN_SPAN)) {
			dev_dbg(nor->dev,
				"Disable Quad bit faili (RDSR2:%02x)\n", ret);
			return -EINVAL;
		}
	}
	return 0;
...
static int eanble_hardware_reset_pin(struct spi_nor *nor)
{
	int ret;
	u8 tmp_val = 0;

	/* Check current Enable Hardware Reset bit value. */
	ret = read_sr3(nor);
	if (ret < 0) {
		dev_dbg(nor->dev,
			"error while reading Status Register3\n");
		return -EINVAL;
	}

	if (!(ret & SR3_HRESET_EN)) {
		tmp_val = ret | SR3_HRESET_EN;
		ret = write_sr3(nor, tmp_val);
		if (ret)
			return ret;

		/* Read back and check it. */
		ret = read_sr3(nor);
		if (!(ret > 0 && (ret & SR3_HRESET_EN))) {
			dev_dbg(nor->dev,
				"Enable Hardware Reset Pin fail (RDSR3:%02x)\n", ret);
			return -EINVAL;
		}
	}
	return 0;
...
static int spi_nor_init(struct spi_nor *nor)
{
	...
	if (nor->addr_width == 4 &&
		...
		if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND) {
			disable_quad_bit(nor);
			eanble_hardware_reset_pin(nor);
		}
	}
