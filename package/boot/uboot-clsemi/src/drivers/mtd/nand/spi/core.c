...
static int spinand_ecc_err_bit_printf = 0;
...
void spinand_mtd_dump_features_reg(struct mtd_info *mtd)
{
	struct spinand_device *spinand = mtd_to_spinand(mtd);
	u8 status = 0;

	printf("Register\tAddr.\t\r\n");
	spinand_read_reg_op(spinand, REG_BLOCK_LOCK, &status);
	printf("Protection\t%2XH (%02X)\t\r\n", REG_BLOCK_LOCK, status);
	spinand_read_reg_op(spinand, REG_CFG, &status);
	printf("Feature\t\t%2XH (%02X)\t\r\n", REG_CFG, status);
	spinand_read_reg_op(spinand, REG_STATUS, &status);
	printf("Status\t\t%2XH (%02X)\t\r\n", REG_STATUS, status);
	spinand_read_reg_op(spinand, 0xD0, &status);
	printf("Feature\t\t%2XH (%02X)\t\r\n", 0xD0, status);
	spinand_read_reg_op(spinand, REG_STATUS2, &status);
	printf("Status\t\t%2XH (%02X)\t\r\n", REG_STATUS2, status);

	spinand_ecc_err_bit_printf ++;
	spinand_ecc_err_bit_printf = (spinand_ecc_err_bit_printf % 2);
	printf("\n");
	if (spinand_ecc_err_bit_printf)
		printf("spinand ecc bit error debug 'on'\n");
	else
		printf("spinand ecc bit error debug 'off'\n");
}
EXPORT_SYMBOL_GPL(spinand_mtd_dump_features_reg);
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	nanddev_io_for_each_page(nand, from, ops, &iter) {
		...
		if (ret == -EBADMSG) {
			...
			ret = 0;
			dev_err(spinand->slave->dev, "Uncorrectable bits error at 0x%llx\n", from);
		} else {
			mtd->ecc_stats.corrected += ret;
			max_bitflips = max_t(unsigned int, max_bitflips, ret);
			if (ret && spinand_ecc_err_bit_printf)
				dev_warn(spinand->slave->dev, "corrected %d bit error at 0x%llx\n", ret, from);
		}
...
static const struct spinand_manufacturer *spinand_manufacturers[] = {
	...
	&fudan_micro_spinand_manufacturer,
	&longsys_foresee_spinand_manufacturer,
};
...
static int spinand_detect(struct spinand_device *spinand)
{
	...
	dev_info(spinand->slave->dev,
		 "%s DID(%2X) SPI NAND was found.\n", spinand->manufacturer->name, spinand->id.data[2]);
	dev_info(spinand->slave->dev,
