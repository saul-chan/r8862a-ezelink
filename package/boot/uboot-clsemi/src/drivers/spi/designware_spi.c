...
struct dw_spi_priv {
	...
	unsigned int mode;
	unsigned int rx_sample_dly;     /* RX sample delay */
...
{
	dw_write(priv, DW_SPI_SSIENR, 0);
#ifdef CONFIG_CLS_SPI_FLASH //apply uboot commit bae4d9fbd97623ccb96ad634128d1aa98faa30f2
...
static int dw_spi_probe(struct udevice *bus)
{
	...
	ret = dev_read_u32(bus, "busclk", &priv->bus_clk_rate);
	if (ret) {
		dev_err(bus,"missing busclk setting\n");
		return ret;
	}

	rx_sample_dly_ns = dev_read_u32_default(bus, "rx-sample-delay-ns", 0);
	priv->rx_sample_dly = DIV_ROUND_CLOSEST(rx_sample_dly_ns,
						NSEC_PER_SEC /
						priv->bus_clk_rate);
#else
...
static int dw_spi_exec_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
	...
	struct dw_spi_priv *priv = dev_get_priv(bus);
#ifdef CONFIG_CLS_SPI_FLASH //apply uboot commit d56dfc90c7bb2f29a38a98fc2e170ebba6fc6a40
