...
#define CLS_UBOOT_VER
#ifdef CLS_UBOOT_VER
static int cls_save_info(void)
{
	char *cls_uboot_version = env_get("cls_uboot_version");
	int need_save_ver = 1;

	if (cls_uboot_version) {
		/*cls_uboot_version exist and cls_uboot_version == CONFIG_LOCALVERSION
		 * not saveenv */
		if (0 == strcmp(cls_uboot_version, CONFIG_LOCALVERSION))
			need_save_ver = 0;
	}

	if (need_save_ver) {
		env_set("cls_uboot_version", CONFIG_LOCALVERSION);
		env_save();
	}
}
#endif
...
static int initr_env(void)
{
	...
	sysinfo_env_init();
	sysinfo_env_relocate();

	char *boardtype_sysinfo = sysinfo_env_get("boardtype");
	if (boardtype_sysinfo != NULL) {

		char *boardtype_env = malloc(strlen(boardtype_sysinfo));
		if (boardtype_sysinfo != NULL) {

			strcpy(boardtype_env,boardtype_sysinfo);

			env_set("boardtype", convertUpperToLower(boardtype_env));
