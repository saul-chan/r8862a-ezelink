...
struct gpio_keys_button_data {
	...
	unsigned int current_trigger_type;
};
...
static inline void set_opposite_trigger_type(int irq, struct gpio_keys_button_data *bdata)
{
	/* IRQ_TYPE_*: Bits 0-7 are the same as the IRQF_* bits in linux/interrupt.h */
	if (bdata->current_trigger_type == IRQF_TRIGGER_FALLING) {
		irq_set_irq_type(irq, IRQ_TYPE_EDGE_RISING);
		bdata->current_trigger_type = IRQF_TRIGGER_RISING;
	} else {
		irq_set_irq_type(irq, IRQ_TYPE_EDGE_FALLING);
		bdata->current_trigger_type = IRQF_TRIGGER_FALLING;
	}
}
...
static irqreturn_t button_handle_irq(int irq, void *_bdata)
{
	...
	set_opposite_trigger_type(irq, bdata);
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (!of_property_read_u32(pp, "pinmux-reg", &pinmux_reg)) {
			virtual_addr = ioremap(pinmux_reg, 4);
			if (virtual_addr) {
				/* bit 12- bit 14 is for pin mux function selection.The value is 0x5 in GPIO mode */
				value = readl(virtual_addr) & 0xffff8fff;
				value |= 0x5000;
				writel(value, virtual_addr);
				iounmap(virtual_addr);
			} else
				pr_err(PFX "pinmux reg ioremap error\n");
		}
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (!button->irq) {
			...
			/* Workaround for D2K GPIO irq can not be triggered by both
			 * edages at the same time. We may introduce another node
			 * in DTS if same code needs to be shared with other platforms
			 * without this issue in the future, but for now let's use
			 * this way.
			 */
