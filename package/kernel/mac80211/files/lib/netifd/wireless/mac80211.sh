...
drv_mac80211_init_device_config() {
	...
	config_add_string pwrmode
	config_add_int beacon_int chanbw frag rts mbssid
	config_add_int rxantenna txantenna txpower min_tx_power txpower_ratio
...
mac80211_hostapd_setup_base() {
	...
	if [ "$enable_ax" != "0" ]; then
		...
		if [ "$he_bss_color_enabled" -gt 0 ] && [ -n "$he_bss_color" ]; then
...
${op_mode:+#op_mode=$op_mode}
${pwrmode:+#pwrmode=$pwrmode}
${hostapd_noscan:+noscan=1}
${tx_burst:+tx_queue_data2_burst=$tx_burst}
$base_cfg

radio_config_file=$hostapd_conf_file
EOF
	radio_config_id=$(md5sum $hostapd_conf_file | awk '{print $1}')
	echo "radio_config_id=$radio_config_id" >> $hostapd_conf_file
	json_select ..
}
...
mac80211_hostapd_setup_bss() {
	...
	hostapd_set_bss_options hostapd_cfg "$phy" "$vif" "$type" || return 1
	json_get_vars wds wds_bridge dtim_period:2 max_listen_int start_disabled
	...
	#[ "$has_sta" -eq 1 -o "$start_disabled" -eq 1 ] && append hostapd_cfg "start_disabled=1" "$N"

	[ "$dtim_period" -lt 1 -o "$dtim_period" -gt 255 ] && dtim_period=2
...
#generate MAC address for vif based on MBSSID policy
max_mbssid_ind=0
mac80211_mbssid_get_indicator() {
	local num_bss_nontx=$(($iface_num-1))

	while [ $num_bss_nontx -gt 0 ]; do
		max_mbssid_ind=$(($max_mbssid_ind+1))
		num_bss_nontx=$(($num_bss_nontx >> 1))
	done
}

mac80211_mbssid_generate_mac() {
	local phy=$1
	local ifidx=$(($2-1))
	...
	local addr="$(cat /sys/class/ieee80211/${phy}/macaddress)"
	...
	local oIFS="$IFS"; IFS=":"; set -- $addr; IFS="$oIFS"

	mbssid_N=$((2**$max_mbssid_ind))
	mbssid_b=$(($mask6%$mbssid_N))

	mbssid_a5=$(($mbssid_a5 - $mbssid_b + (($mbssid_b + $ifidx) % $mbssid_N)))

	[ "$((0x$mbssid_a5))" -lt 255 ] && {
		printf "%s:%s:%s:%s:%s:%02x" $1 $2 $3 $4 $5 $(( 0x$mbssid_a5 ))
		return
	}
}

mac80211_calc_iface_num() {
...
refresh_vif_info() {
	...
	grep -s "^$section " $vif_info_file >> $vif_info_file.tmp
}

cls_get_ifname() {
	ifname=$(grep "^$section " $vif_info_file | awk '{print $2}')
	[ -n "$ifname" ] && return
	...
	while true; do
		...
		if_idx="$(($if_idx + 1))"
		if ! grep -m1 -q -s " $ifname " $vif_info_file; then
			return
		fi
	done
}
...
cls_get_mac() {
	macaddr=$(grep "^$section " $vif_info_file | awk '{print $3}')
	if [ -n "$macaddr" ]; then
		return
	fi
	...
	while true; do
		...
		macidx="$(($macidx + 1))"
		if ! grep -m1 -q -s " $macaddr" $vif_info_file; then
			return
		fi
	done
}
...
mac80211_prepare_vif() {
	...
	[ -z "$ifname" ] && cls_get_ifname

	/sbin/wifi_iface_apply_brvlan $ifname $section
	...
	if [ "$mbssid" == "1" ] || [ "$mbssid" == "2" ]; then
		[ "$mode" == "ap" ] && {
			macaddr="$(mac80211_mbssid_generate_mac $phy $if_idx)"
		}
	fi
	...
	if [ -z "$(grep -w $section $vif_info_file)" ]; then
		echo "$section $ifname $macaddr" >> $vif_info_file
	fi
...
drv_mac80211_setup() {
	...
	json_get_vars \
		...
		num_global_macaddr:1 multiple_bssid mbssid
	...
	max_interfaces=$(iw $phy info | awk '/#\{ AP \}/{print substr($6,1,length($6)-1)}')
	[ -z "$max_interfaces" ] && max_interfaces=$(iw $phy info | awk '/#\{ managed, AP \}/{print substr($7,1,length($7)-1)}')
	max_interfaces=${max_interfaces:-8}
	...
	set_default pwrmode default
	/usr/sbin/power_tbl $band $pwrmode > /dev/null

	hostapd_conf_file="/var/run/hostapd-$phy.conf"
	vif_info_file="/var/run/vif_info-$phy.conf"
	...
	for_each_interface "ap" mac80211_calc_iface_num
	mac80211_mbssid_get_indicator

	if [ -n "$has_ap" -a -n "$has_sta" ]; then
		op_mode="repeater"
	...
	mac80211_prepare_iw_htmode
	active_ifnames=
	for_each_interface "ap sta adhoc mesh monitor" refresh_vif_info
	if [ -f $vif_info_file.tmp ]; then
		mv $vif_info_file.tmp $vif_info_file
	else
		cat /dev/null > $vif_info_file
	fi
	for_each_interface "ap sta adhoc mesh monitor" mac80211_prepare_vif
	for_each_interface "ap sta adhoc mesh monitor" mac80211_setup_vif

	echo "$op_mode" > /sys/kernel/debug/ieee80211/$phy/cls_wifi/op_mode
	...
	start-stop-daemon -S -b -m -p /tmp/finetune_$phy.pid -x /usr/bin/finetune_phy.sh $mode ${phy#phy} $band
	wireless_set_up

        # To connect the hidden bssid in dfs channel, sta will always send probe
	if [ "$band" = "5g" ]; then
		if [ -n "$has_sta" ]; then
			echo "$band has_sta enbale force probe_tx"
			sh /usr/psbin/dht_msg_with_params.sh band=${phy#phy} id=0x21 msg="0x0:u32"
		else
			echo "$band non-has_sta disable force probe_tx"
			sh /usr/psbin/dht_msg_with_params.sh band=${phy#phy} id=0x21 msg="0x1:u32"
		fi
	fi

	if [ -n "$txpower_ratio" ]; then
		clsapi set txpower_ratio "$phy" "$txpower_ratio"
	fi
}
...
drv_mac80211_teardown() {
	...
	start-stop-daemon -K -p /tmp/finetune_$phy.pid
	rm -f /tmp/finetune_$phy.pid
	rm -f /var/run/vif_info-$phy.conf
}
