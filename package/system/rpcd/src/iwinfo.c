...
static void rpc_iwinfo_call_hw_ht_mode(int hwmodelist)
{
	...
	if (htmode_str)
	{
		if (iwinfo_htmode_is_ht(htmode))
			...
			hwmode_str = "ax";
		else if (iwinfo_htmode_is_g(htmode))
			hwmode_str = "g";
		else if (iwinfo_htmode_is_a(htmode))
			hwmode_str = "a";
		else
			hwmode_str = "unknown";
	} else
                    ...
                    struct blob_attr *msg)
{
	...
	if (!iw->freqlist(ifname, res, &len) && (len > 0))
	{
		...
		for (i = 0; i < len; i += sizeof(struct iwinfo_freqlist_entry))
		{
			...
			if (f->bandwidth & IWINFO_FREQ_20MHZ)
				strcat(bandwidth, "20M/");

			if (f->bandwidth & IWINFO_FREQ_HT40MINUS)
				strcat(bandwidth, "40M-/");

			if (f->bandwidth & IWINFO_FREQ_HT40PLUS)
				strcat(bandwidth, "40M+/");

			if (f->bandwidth & IWINFO_FREQ_80MHZ)
				strcat(bandwidth, "80M/");

			if (f->bandwidth & IWINFO_FREQ_160MHZ)
				strcat(bandwidth, "160M/");

			if (strlen(bandwidth) != 0)
				bandwidth[strlen(bandwidth) - 1] = 0;

			blobmsg_add_string(&buf, "bandwidth", bandwidth);
