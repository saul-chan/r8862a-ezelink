...
static int
nfct_build_clsmark(struct nlmsghdr *nlh, const struct nf_conntrack *ct)
{
	mnl_attr_put_u64(nlh, CTA_CLSMARK, htobe64(ct->clsmark));
	return 0;
}
...
nfct_nlmsg_build(struct nlmsghdr *nlh, const struct nf_conntrack *ct)
{
	...
	if (test_bit(ATTR_CLSMARK, ct->head.set))
		nfct_build_clsmark(nlh, ct);
			    ...
			    const struct nfct_filter_dump *filter_dump)
{
	...
	if (filter_dump->set & (1 << NFCT_FILTER_DUMP_CLSMARK)) {
		mnl_attr_put_u64(nlh, CTA_CLSMARK, htobe64(filter_dump->clsmark.val));
		mnl_attr_put_u64(nlh, CTA_CLSMARK_MASK, htobe64(filter_dump->clsmark.mask));
	}
