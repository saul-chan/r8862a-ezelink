...
static void
set_filter_dump_attr_clsmark(struct nfct_filter_dump *filter_dump,
			  const void *value)
{
	const struct nfct_filter_dump_clsmark *this = value;

	filter_dump->clsmark.val = this->val;
	filter_dump->clsmark.mask = this->mask;
}
...
const set_filter_dump_attr set_filter_dump_attr_array[NFCT_FILTER_DUMP_MAX] = {
	...
	[NFCT_FILTER_DUMP_CLSMARK]	= set_filter_dump_attr_clsmark,
