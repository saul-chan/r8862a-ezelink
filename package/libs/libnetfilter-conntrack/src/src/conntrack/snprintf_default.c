...
static int
__snprintf_clsmark(char *buf, unsigned int len, const struct nf_conntrack *ct)
{
	return (snprintf(buf, len, "clsmark=0x%016llx ", ct->clsmark));
}
				 ...
				 struct nfct_labelmap *map)
{
	...
	if (test_bit(ATTR_CLSMARK, ct->head.set)) {
		ret = __snprintf_clsmark(buf+offset, len, ct);
		BUFFER_SIZE(ret, size, len, offset);
	}
