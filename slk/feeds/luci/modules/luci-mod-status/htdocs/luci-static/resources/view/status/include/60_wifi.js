...
return baseclass.extend({
	...
	handleWPSPinConnect: function(wifinet, bssid, ev) {
		var pin_code = ev.srcElement.previousElementSibling.value;

		if (!/\d{8}$/.test(pin_code)) {
			ev.srcElement.previousElementSibling.classList.add('cbi-input-invalid');
			return;
		} else {
			ev.srcElement.previousElementSibling.classList.remove('cbi-input-invalid');
			this.pinCode[bssid] = pin_code;
		}
		...
		this.callWPSPinConnection(wifinet, 'any', pin_code).then(function(res) {});
		return;
	},

	callWPSPinConnection: rpc.declare({
		object: 'clsapi.wifi',
		method: 'trigger_wps_pin_connection',
		params: [ 'ifname', 'uuid', 'pin' ],
		result: [ ]
	}),

	callWPSPinGenerate: rpc.declare({
		object: 'clsapi.wifi',
		method: 'generate_wps_pin',
	}),

	callUCISet: rpc.declare({
		object: 'uci',
		method: 'set',
		params: [ 'config', 'section', 'values' ]
	}),

	callUCIApply: rpc.declare({
		object: 'uci',
		method: 'apply'
	}),

	handleWPSPinGenerate: function(section_id, ev) {
		ev.currentTarget.classList.add('spinning');
		ev.currentTarget.disabled = true;
		ev.currentTarget.blur();

		this.callWPSPinGenerate().then((res) => {
			if (res.pin) {
				this.callUCISet('wireless', section_id, { 'wps_pin': res.pin });
				this.callUCIApply();
			}
		});
	},
	...
	renderbox: function(radio, networks) {
		...
		for (var i = 0; i < networks.length; i++) {
			...
			var WPS_button = null, WPS_pin_show = null, WPS_pin_connect = null, WPS_pin_confirm = null,
				wps_pin = net?._ubusdata?.net?.config?.wps_pin, WPS_pin_generate = null,
				pin_connect = net.getActiveBSSID();
			...
			WPS_pin_show = E('span', { 'id': 'wps-pin-show' }, [ wps_pin? wps_pin: _('unset') ]);

			WPS_pin_connect = E('form', [
				E('input', {
					'id': 'wps-pin-connect' + i,
					'class': 'cbi-input',
					'type': 'text',
					'pattern': '\d{8}',
					'maxlength': 8,
					'title': '请输入由8位数字组成的pin码！',
					'value': this.pinCode[pin_connect],
					'required': true
				}),
				E('button', {
					'class' : 'cbi-button',
					'type': 'submit',
					'click': L.bind(this.handleWPSPinConnect, this, net.getIfname(), net.getActiveBSSID()),
				}, [ _('Connect') ])
			]);

			WPS_pin_generate = E('button', {
				'class': 'cbi-button cbi-button-apply',
				'click': L.bind(this.handleWPSPinGenerate, this, net.sid),
			}, [ _('Generate WPS PIN') ]);
			...
			var badge = renderBadge(
				...
				net.hasWPSLabel? _('WPS PIN Code'): '', net.hasWPSLabel? WPS_pin_show: null,
				'', net.hasWPSLabel? WPS_pin_generate: null,
				net.hasWPSKeypad? _('WPS PIN connect'): '', net.hasWPSKeypad? WPS_pin_connect: null,
				'', WPS_button
			);
	...
	load: function() {
		return Promise.all([
			...
			for (var i = 0; i < radios_networks_hints.length; i++) {
				...
				if (hasWPS && uci.get('wireless', radios_networks_hints[i].sid, 'wps_label') == '1')
					radios_networks_hints[i].hasWPSLabel = true;

				if (hasWPS && uci.get('wireless', radios_networks_hints[i].sid, 'wps_keypad') == '1')
					radios_networks_hints[i].hasWPSKeypad = true;
