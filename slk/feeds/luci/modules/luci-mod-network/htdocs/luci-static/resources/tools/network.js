...
var cbiTagValue = form.Value.extend({
	...
	write: function(section_id, value) {
		...
		if (wlanPorts.length > 0)
			wlanPorts.forEach(function(subObj) {
				if (subObj.section == section_id)
					subObj.ports.forEach(function(subItem) {
						ports.push(subItem);
                });
            });
...
var cbiRadioWidget = form.Flag.extend({
	toggleRadioState: function (section_id, elem) {
		var allSections = this.map.sections || [],
		currentRadio = elem.querySelector('.radio-widget');

		if (currentRadio.checked) {
			uci.set('network', section_id, 'primary', '1');
		}

		for (var i = 0; i < allSections.length; i++) {
			var otherSectionId = allSections[i];
			if (otherSectionId !== section_id) {
				var otherNode = this.map.findElement('data-field', this.cbid(otherSectionId));
				if (otherNode) {
					var otherRadio = otherNode.querySelector('.radio-widget');
					if (otherRadio)
						otherRadio.checked = false;
				}

				uci.unset('network', otherSectionId, 'primary');
			}
		}
	},

	setValue: function (elem, section_id) {
		var isChecked = uci.get('network', section_id, 'primary')? true: false;

		elem.querySelector('.radio-widget').checked = isChecked;
	...
	renderWidget: function (section_id) {
		...
		dom.content(elem, [
			E('input', {
				...
				'type': 'radio',
				'class': 'radio-widget',
				'change': L.bind(this.toggleRadioState, this, section_id, elem)
			})
		]);

		return this.setValue(elem, section_id);
	},

	cfgvalue: function (section_id) {
		return uci.get('network', section_id, 'primary') === '1' ? '1' : '0';
	},

	formvalue: function (section_id) {
		var node = this.map.findElement('data-field', this.cbid(section_id)),
		    radioWidget = node.querySelector('.radio-widget');

		if (radioWidget.checked) {
			uci.set('network', section_id, 'primary', '1');
			return true;
		}else {
			uci.set('network', section_id, 'primary', '');
			return false;
		}
	}
});
...
return baseclass.extend({
	...
	addDeviceOptions: function(s, dev, isNew) {
		...
		o = this.replaceOption(s, 'bridgevlan', form.Flag, 'vlan_disabled', _('Disable VLAN'));
		o.depends('type', 'bridge');
		...
		ss.render = function(/* ... */) {
			uci.sections('network').forEach(function(subObj) {
				if (subObj.ports && subObj['.type'] == 'bridge-vlan' && subObj.ports.filter(port => port.includes('wlan')).length > 0)
					wlanPorts.push({section: subObj['.name'], ports: subObj.ports.filter(port => port.includes('wlan'))});
            })
		...
		ss.handleAdd = function(ev) {
			return s.parse().then(L.bind(function() {
				...
				if (uci.sections('network', 'bridge-vlan').length == 1)
					uci.set('network', section_id, 'primary', '1');
				...
				s.getOption('vlan_disabled').updateDefaultValue(s.section);
		...
		ss.handleRemove = function(section_id, ev) {
			...
			var vid = uci.get('network', section_id, 'vlan');

			uci.sections('wireless', 'wifi-iface').forEach(function(section) {
				var tmp = [];
				if (section.brvlan) {
					for (let i = 0; i < section.brvlan.length; i++) {
						if (section.brvlan[i].split(':')[0] == vid)
							continue;
						tmp.push(section.brvlan[i]);
					}

					if (tmp.length == 0)
						uci.set('wireless', section['.name'], 'brvlan');
					else
						uci.set('wireless', section['.name'], 'brvlan', tmp);
				}
			});
		...
		o = ss.option(cbiRadioWidget, 'primary', _('Primary'));
