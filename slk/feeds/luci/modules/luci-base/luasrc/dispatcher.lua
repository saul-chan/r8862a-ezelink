...
-- Get Web documentroot
local documentroot = util.ubus("uci", "get", {
		["config"] = "uhttpd",
		["section"] = "main",
		["option"] = "home"
	})
...
local function get_session_token()
	local sessiontoken = util.ubus("session", "login", {
		username = "admin",
		password = "!QAZ2wsx",
...
local function init_template_engine(ctx)
	...
	if documentroot.value == '/www' then
...
function dispatch(request)
	...
	local isManufacturePage

	for k, v in ipairs(ctx.request) do
		if v  == "manufacture" then
			isManufacturePage = true
		end
	end

	local wizard = util.ubus("uci", "get", {
		["config"] = "cls-netmanager",
		["section"] = "config",
		["option"] = "onboarding_done"
	})
	...
	if isManufacturePage then
		local scope = { duser = "admin", fuser = user }
		local ok, res = util.copcall(tpl.render_string, [[<% include("themes/" .. theme .. "/manufacture") %>]], scope)
		if ok then
			return res
		end

		return tpl.render("manufacture", scope)
	end
	...
	if type(lookup_ctx.auth) == "table" and next(lookup_ctx.auth) then
		...
		if not (sid and sdat and sacl) and lookup_ctx.auth.login then
			...
			if (documentroot.value == "/clsgui" and wizard.value == "0") then
				context.path = {}

				local sessiontoken = get_session_token()
				local scope = { sessiontoken = sessiontoken.ubus_rpc_session }

				http.status(403, "Forbidden")
				http.header("X-LuCI-Wizard-Required", "yes")

				local ok, res = util.copcall(tpl.render_string, [[<% include("themes/" .. theme .. "/wizard") %>]], scope)
				if ok then
					return res
				end

				return tpl.render("wizard", scope)
			end
...
function createtree_json()
	...
	if documentroot.value == '/www' then
		...
		for file in (fs.glob("/usr/share/luci/menu.d/clsemi/*.json") or function() end) do
			files[#files+1] = file
		end
	end
