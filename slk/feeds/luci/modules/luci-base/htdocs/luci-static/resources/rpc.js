	...
	handleCallReply: function(req, msg) {
		...
		try {
			...
			if (L.isObject(msg.error) && msg.error.code && msg.error.message) {
				if (req.object == 'clmesh.api')
					L.raise('clmeshRPCErrorTip',
						'VIP-QoS can only work on UltraMesh controller,please set device to UltraMesh controller mode.');
				else
		...
		if (!req.object && !req.method) {
			...
			if (req.raise && req.params.config == 'wireless' && msg.result[0] !== 0)
				L.raise('WirelessPageErrorTip', 
					'You need to wait until the wireless interface is up to access the wireless configuration page normally, Please wait a moment and manually refresh the page again!');
			else if (req.raise && msg.result[0] !== 0)
