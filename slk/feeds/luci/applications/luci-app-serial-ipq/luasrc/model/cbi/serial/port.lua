...
local m, s, m2, s2, o
hostname=luci.sys.exec("uci -q get system.@system[0].hostname")

m = Map("nport", translate("Configuration"))
m:chain("luci")

s = m:section(TypedSection, "ttyS1", translate("Network Settings"))
s.anonymous = true
s.addremove = false

enable = s:option(Flag, "enable", translate("Enable"), translate("Take effect after reboot."))
...
serverhost = s:option(Value, "serverhost", translate("Server"))
serverhost.placeholder = "broker.example.com"
serverhost:depends("netpro", "mqtt")
...
serverport:depends("netpro", "mqtt")
...
username:depends("netpro", "mqtt")

password = s:option(Value, "password", translate ("Password"))
password.password=true
password:depends("netpro", "mqtt")

topic1 = s:option(Value, "mqtt_publish_topic", translate ("Publish Topic"))
topic1.default = hostname.."/serial/data"
topic1:depends("netpro", "mqtt")

topic2 = s:option(Value, "mqtt_subscribe_topic", translate ("Subscribe Topic"))
topic2.default = hostname.."/mqtt/data"
topic2:depends("netpro", "mqtt")

enable_tls = s:option(Flag, "en_tls", translate ("SSL/TLS"))
enable_tls.default = "0"
enable_tls:depends("netpro", "mqtt")

ca = s:option(FileUpload, "ca", translate ("CA File"))
ca:depends("en_tls", "1")

cert = s:option(FileUpload, "cert", translate ("Client Certificate File"))
cert:depends("en_tls", "1")

key = s:option(FileUpload, "key", translate ("Client Key File"))
key:depends("en_tls", "1")

serial_timeout = s:option(Value, "serial_timeout", translate("Serial Timeout(ms)"), translate("Serial packet timeout in milliseconds (default: 100)"))
serial_timeout.default = "100"
serial_timeout:depends("netpro", "mqtt")

s2 = m:section(TypedSection, "ttyS1", translate("Serial Settings"))
s2.anonymous = true
s2.addremove = false
