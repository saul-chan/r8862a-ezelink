...
function rule_proto_txt(s, ctHelpers) {
	var family = (uci.get('redirect', s, 'family') || '').toLowerCase().replace(/^(?:all|\*)$/, 'any');
	var dip = uci.get('redirect', s, 'dest_ip') || '';
	var proto = L.toArray(uci.get('redirect', s, 'proto')).filter(function(p) {
		...
		return {
			num:   pr[0],
			name:  pr[1],
			types: (pr[0] == 1 || pr[0] == 58) ? L.toArray(uci.get('redirect', s, 'icmp_type')) : null
		};
	});

	var m = String(uci.get('redirect', s, 'helper') || '').match(/^(!\s*)?(\S+)$/);
	var h = m ? {
		val:  m[0].toUpperCase(),
		inv:  m[1],
	...
	m = String(uci.get('redirect', s, 'mark')).match(/^(!\s*)?(0x[0-9a-f]{1,8}|[0-9]{1,10})(?:\/(0x[0-9a-f]{1,8}|[0-9]{1,10}))?$/i);
	var f = m ? {
...
function rule_src_txt(s, hosts) {
	...
	return fwtool.fmt(_('From %{src}%{src_ip?, IP %{src_ip#%{next?, }<var%{item.inv? data-tooltip="Match IP addresses except %{item.val}."}>%{item.ival}</var>}}%{src_port?, port %{src_port#%{next?, }<var%{item.inv? data-tooltip="Match ports except %{item.val}."}>%{item.ival}</var>}}%{src_mac?, MAC %{src_mac#%{next?, }<var%{item.inv? data-tooltip="Match MACs except %{item.val}%{item.hint.name? a.k.a. %{item.hint.name}}.":%{item.hint.name? data-tooltip="%{item.hint.name}"}}>%{item.ival}</var>}}'), {
		...
		src_ip: fwtool.map_invert(uci.get('redirect', s, 'src_ip'), 'toLowerCase'),
		src_mac: fwtool.map_invert(uci.get('redirect', s, 'src_mac'), 'toUpperCase').map(function(v) { return Object.assign(v, { hint: hosts[v.val] }) }),
		src_port: fwtool.map_invert(uci.get('redirect', s, 'src_port'))
	});
}

function rule_dest_txt(s) {
	return fwtool.fmt(_('%{dest_port? %{dest_port#%{next?, }<var%{item.inv? data-tooltip="Match ports except %{item.val}."}>%{item.ival}</var>}}'), {
		dest: E('span', { 'class': 'zonebadge', 'style': fwmodel.getZoneColorStyle(null) }, [E('em', _('this device'))]),
		dest_ip: fwtool.map_invert(uci.get('redirect', s, 'src_dip'), 'toLowerCase'),
		dest_port: fwtool.map_invert(uci.get('redirect', s, 'src_dport'))
	});
}

function rule_limit_txt(s) {
	var m = String(uci.get('redirect', s, 'limit')).match(/^(\d+)\/([smhd])\w*$/i),
	    l = m ? {
			num:   +m[1],
			unit:  ({ s: _('second'), m: _('minute'), h: _('hour'), d: _('day') })[m[2]],
			burst: uci.get('redirect', s, 'limit_burst')
		} : null;
...
function rule_target_txt(s) {
	var z = uci.get('redirect', s, 'dest');

	return fwtool.fmt(_('%{dest_ip? <var>%{dest_ip}</var>}'), {
		...
		dest_ip: (uci.get('redirect', s, 'dest_ip') || '').toLowerCase(),
		dest_port: uci.get('redirect', s, 'dest_port')
	});
}

function rule_target_port_txt(s) {
	var z = uci.get('redirect', s, 'dest');

	return fwtool.fmt(_('%{dest_port? <var>%{dest_port}</var>}'), {
		dest: E('span', { 'class': 'zonebadge', 'style': 'background-color:' + fwmodel.getColorForName((z && z != '*') ? z : null) }, [(z == '*') ? E('em', _('any zone')) : (z ? E('strong', z) : E('em', _('this device')))]),
		dest_ip: (uci.get('redirect', s, 'dest_ip') || '').toLowerCase(),
		dest_port: uci.get('redirect', s, 'dest_port')
	});
...
return view.extend({
	...
	renderForwards: function(data) {
		...
		m = new form.Map('redirect', _('Port Forwards'),
		...
		s = m.section(form.GridSection, 'redirect', _('Forward List'));
		s.addremove = true;
		s.anonymous = true;
		s.sortable  = true;
		...
		s.filter = function(section_id) {
			return (uci.get('redirect', section_id, 'target') != 'SNAT');
		};

		s.sectiontitle = function(section_id) {
			return uci.get('redirect', section_id, 'name') || _('Unnamed forward');
		};
		...
		o = s.option(form.DummyValue, '_src_dport', _('External port'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_dest_txt(s)
		...
		o = s.option(form.DummyValue, '_dest_port', _('Internal port'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_target_port_txt(s)
			]);
		};
		
		o = s.option(form.ListValue, '_dest', _('Internal IP'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_target_txt(s)
		...
		o = s.taboption('advanced', form.ListValue, 'src', _('External zone'));
		o.modalonly = true;
		o.value('wan');
		o.default = 'wan';

		// o = s.taboption('advanced', form.Value, 'ipset', _('Use ipset'));
		// uci.sections('redirect', 'ipset', function(s) {
		...
		o = s.taboption('advanced', form.ListValue, 'dest', _('Internal zone'));
		o.modalonly = true;
		o.value('lan');
		o.default = 'lan';
		...
		// o.write = function(section_id, value) {
			// uci.set('redirect', section_id, 'reflection_src', (value != 'internal') ? value : null);
		// };
