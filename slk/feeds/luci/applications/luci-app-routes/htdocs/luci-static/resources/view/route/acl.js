...
function rule_proto_txt(s, ctHelpers) {
	var f = (uci.get('bwlist', s, 'family') || '').toLowerCase().replace(/^(?:any|\*)$/, '');

	var proto = L.toArray(uci.get('bwlist', s, 'proto')).filter(function(p) {
		...
		return {
			num:   pr[0],
			name:  pr[1],
			types: (pr[0] == 1 || pr[0] == 58) ? L.toArray(uci.get('bwlist', s, 'icmp_type')) : null
		};
	});

	var m = String(uci.get('bwlist', s, 'helper') || '').match(/^(!\s*)?(\S+)$/);
	var h = m ? {
		val:  m[0].toUpperCase(),
		inv:  m[1],
	...
	m = String(uci.get('bwlist', s, 'mark')).match(/^(!\s*)?(0x[0-9a-f]{1,8}|[0-9]{1,10})(?:\/(0x[0-9a-f]{1,8}|[0-9]{1,10}))?$/i);
	var w = m ? {
	...
	m = String(uci.get('bwlist', s, 'dscp')).match(/^(!\s*)?(?:(CS[0-7]|BE|AF[1234][123]|EF)|(0x[0-9a-f]{1,2}|[0-9]{1,2}))$/);
	var d = m ? {
		val:  m[0],
		inv:  m[1],
		name: m[2],
	...
	return fwtool.fmt(_(' %{ipv6?%{ipv4?<var>IPv4</var> & <var>IPv6</var>:<var>IPv6</var>}:<var>IPv4</var>}%{proto? , %{proto#%{next?, }%{item.types?<var class="cbi-tooltip-container">%{item.name}<span class="cbi-tooltip">ICMP with types %{item.types#%{next?, }<var>%{item}</var>}</span></var>:<var>%{item.name}</var>}}:, <var>Any</var>}%{mark?, mark <var%{mark.inv? data-tooltip="Match fwmarks except %{mark.num}%{mark.mask? with mask %{mark.mask}}.":%{mark.mask? data-tooltip="Mask fwmark value with %{mark.mask} before compare."}}>%{mark.val}</var>}%{dscp?, DSCP %{dscp.inv?<var data-tooltip="Match DSCP classifications except %{dscp.num?:%{dscp.name}}">%{dscp.val}</var>:<var>%{dscp.val}</var>}}%{helper?, helper %{helper.inv?<var data-tooltip="Match any helper except &quot;%{helper.name}&quot;">%{helper.val}</var>:<var data-tooltip="%{helper.name}">%{helper.val}</var>}}'), {
		ipv4: (!f || f == 'ipv4'),
		ipv6: (!f || f == 'ipv6'),
		src:  uci.get('bwlist', s, 'src'),
		dest: uci.get('bwlist', s, 'dest'),
		proto: proto,
		helper: h,
		mark:   w,
		dscp:   d
...
function rule_src_txt(s, hosts) {
	var z = uci.get('bwlist', s, 'src'),
	    d = (uci.get('bwlist', s, 'direction') == 'in') ? uci.get('bwlist', s, 'device') : null;

	return fwtool.fmt(_('%{src_ip? %{src_ip#%{next?, }<var%{item.inv? data-tooltip="Match IP addresses except %{item.val}."}>%{item.ival}</var>}: <var>Any</var>}%{src_port?, port %{src_port#%{next?, }<var%{item.inv? data-tooltip="Match ports except %{item.val}."}>%{item.ival}</var>}}%{src_mac?, MAC %{src_mac#%{next?, }<var%{item.inv? data-tooltip="Match MACs except %{item.val}%{item.hint.name? a.k.a. %{item.hint.name}}.":%{item.hint.name? data-tooltip="%{item.hint.name}"}}>%{item.ival}</var>}}'), {
		src: E('span', { 'class': 'zonebadge', 'style': fwmodel.getZoneColorStyle(z) }, [(z == '*') ? E('em', _('any zone')) : (z ? E('strong', z) : E('em', _('this device')))]),
		src_ip: fwtool.map_invert(uci.get('bwlist', s, 'src_ip'), 'toLowerCase'),
		src_mac: fwtool.map_invert(uci.get('bwlist', s, 'src_mac'), 'toUpperCase').map(function(v) { return Object.assign(v, { hint: hosts[v.val] }) }),
		src_port: fwtool.map_invert(uci.get('bwlist', s, 'src_port')),
		src_device: d
	});
}

function rule_dest_txt(s) {
	var z = uci.get('bwlist', s, 'dest'),
	    d = (uci.get('bwlist', s, 'direction') == 'out') ? uci.get('bwlist', s, 'device') : null;

	return fwtool.fmt(_('%{dest_ip? %{dest_ip#%{next?, }<var%{item.inv? data-tooltip="Match IP addresses except %{item.val}."}>%{item.ival}</var>}: <var>Any</var>}%{dest_port?, port %{dest_port#%{next?, }<var%{item.inv? data-tooltip="Match ports except %{item.val}."}>%{item.ival}</var>}}'), {
		dest: E('span', { 'class': 'zonebadge', 'style': fwmodel.getZoneColorStyle(z) }, [(z == '*') ? E('em', _('any zone')) : (z ? E('strong', z) : E('em', _('this device')))]),
		dest_ip: fwtool.map_invert(uci.get('bwlist', s, 'dest_ip'), 'toLowerCase'),
		dest_port: fwtool.map_invert(uci.get('bwlist', s, 'dest_port')),
		dest_device: d
	});
}

function rule_limit_txt(s) {
	var m = String(uci.get('bwlist', s, 'limit')).match(/^(\d+)\/([smhd])\w*$/i),
	    l = m ? {
			num:   +m[1],
			unit:  ({ s: _('second'), m: _('minute'), h: _('hour'), d: _('day') })[m[2]],
			burst: uci.get('bwlist', s, 'limit_burst')
		} : null;
...
function rule_target_txt(s, ctHelpers) {
	var t = uci.get('bwlist', s, 'target'),
	    h = (uci.get('bwlist', s, 'set_helper') || '').toUpperCase(),
	    s = {
	    	target: t,
	    	src:    uci.get('bwlist', s, 'src'),
	    	dest:   uci.get('bwlist', s, 'dest'),
	    	set_helper: h,
	    	set_mark:   uci.get('bwlist', s, 'set_mark'),
	    	set_xmark:  uci.get('bwlist', s, 'set_xmark'),
	    	set_dscp:   uci.get('bwlist', s, 'set_dscp'),
	    	helper_name: (ctHelpers.filter(function(ctH) { return ctH.name.toUpperCase() == h })[0] || {}).description
	    };

	switch (t) {
	case 'ACCEPT':
		return fwtool.fmt(_('<var data-tooltip="ACCEPT">Accept</var>'), s);

	case 'REJECT':
		return fwtool.fmt(_('<var data-tooltip="REJECT">Reject</var>'), s);
...
return view.extend({
	...
	renderRules: function(data) {
		...
		m = new form.Map('bwlist', _('Access Control List'),
			_('By filtering IP addresses and MAC addresses, black and white lists can help manage the network connection status of access devices.'));

		s = m.section(form.TypedSection, 'bwlist', _('Mode Configuration'));
		s.anonymous = true;
		s.addremove = false;

		o = s.option(form.Flag, 'enabled', _('Enable'));

		o = s.option(form.ListValue, 'mode', _('Mode'), _('White List:Only allow devices in the following list to connect to the Internet.<br />Black List:Devices in the following list are prohibited from connecting to the Internet.'));
		o.value('white', _('White List'));
		o.value('black', _('Black List'));

		s = m.section(form.GridSection, 'rule', _('Control List'));
		s.addremove = true;
		s.anonymous = true;
		s.sortable  = true;
		...
		s.filter = function(section_id) {
			return (uci.get('bwlist', section_id, 'target') != 'SNAT');
		};

		s.sectiontitle = function(section_id) {
			return uci.get('bwlist', section_id, 'name') || _('Unnamed rule');
		};
		...
		o = s.option(form.DummyValue, '_proto', _('Protocol'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_proto_txt(s, ctHelpers)
			]);
		};
		
		o = s.option(form.DummyValue, '_src', _('Local'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_src_txt(s, hosts)
			]);
		};
		
		o = s.option(form.DummyValue, '_dest', _('Destination'));
		o.modalonly = false;
		o.textvalue = function(s) {
			return E('small', [
				rule_dest_txt(s)
		...
		o.tooltip = function(section_id) {
			var weekdays = uci.get('bwlist', section_id, 'weekdays');
			var monthdays = uci.get('bwlist', section_id, 'monthdays');
			var start_time = uci.get('bwlist', section_id, 'start_time');
			var stop_time = uci.get('bwlist', section_id, 'stop_time');
			var start_date = uci.get('bwlist', section_id, 'start_date');
			var stop_date = uci.get('bwlist', section_id, 'stop_date');
		...
		// o.cfgvalue = function(section_id) {
			// var val = uci.get('bwlist', section_id, 'direction');
			// switch (val) {
				// case 'in':
				// case 'ingress':
					// return 'in';
		...
		o = s.taboption('advanced', form.ListValue, 'src', _('Local zone'));
		o.modalonly = true;
		o.value('lan');
		o.default = 'lan';

		// o = s.taboption('advanced', form.Value, 'ipset', _('Use ipset'));
		// uci.sections('bwlist', 'ipset', function(s) {
		...
		fwtool.addMACOption(s, 'advanced', 'src_mac', _('Local MAC address'), null, hosts);
		fwtool.addIPOption(s, 'general', 'src_ip', _('Local IP address'), null, '', hosts, true);

		o = s.taboption('general', form.Value, 'src_port', _('Local port'));
		o.modalonly = true;
		...
		o = s.taboption('advanced', form.ListValue, 'dest', _('Destination zone'));
		o.modalonly = true;
		o.value('wan');
		o.default = 'wan';

		fwtool.addIPOption(s, 'general', 'dest_ip', _('Destination IP address'), null, '', hosts, true);
		...
		o.cfgvalue = function(section_id) {
			var t = uci.get('bwlist', section_id, 'target'),
			    m = uci.get('bwlist', section_id, 'set_mark');
