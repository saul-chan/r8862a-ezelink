...
IMG_PREFIX_EXTRA:=$(if $(EXTRA_IMAGE_NAME),$(call sanitize,$(EXTRA_IMAGE_NAME))-)$(if $(call qstrip,$(CONFIG_EXTRA_IMAGE_NAME)),$(call qstrip,$(CONFIG_EXTRA_IMAGE_NAME))-)
...
define Device/Init
  ...
  IMAGES :=
# CLS_FIRST_IMAGE is used when multi images are compiled but one image is depended by other images
# We need to define this image , or you may see it fails to compile when multi CPU cores are used.
  CLS_FIRST_IMAGE :=
  ARTIFACTS :=
...
define Device/Build/image
  ...
  $(KDIR)/tmp/$(call DEVICE_IMG_NAME,$(1),$(2)): $$(KDIR_KERNEL_IMAGE) $$(ROOTFS/$(1)/$(3)) $(if $(CLS_FIRST_IMAGE),$(KDIR_TMP)/$(DEVICE_IMG_PREFIX)-$(FILESYSTEMS)-$(CLS_FIRST_IMAGE))
	@rm -f $$@
	[ -f $$(word 1,$$^) -a -f $$(word 2,$$^) ]
