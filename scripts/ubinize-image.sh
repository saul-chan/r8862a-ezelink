...
if [ ! -r "$rootfs" -a ! -r "$kernel" -a ! "$outfile" ]; then
	echo "syntax: $0 [--uboot-env] [--part <name>=<file>] [--kernel kernelimage] [--clsfit] [--rootfs rootfsimage] out [ubinize opts]"
	exit 1
fi
...
ubilayout "$ubootenv" "$rootfs" "$kernel" "$clsfit" > "$ubinizecfg"
